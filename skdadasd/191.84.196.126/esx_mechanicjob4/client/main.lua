...
function OpenMechanicActions3Menu()
	local elements = {
	...
	ESX.OpenContext("right", elements, function(menu,element)
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.OpenContext("right", elements2, function(menu2,element2)
						...
						TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mechanic4', vehicleProps)
					end)
				end, 'mechanic')
			else
				local elements2 = {
					{unselectable = true, icon = "fas fa-car", title = TranslateCap('service_vehicle')},
					{icon = "fas fa-truck", title = 'Ford F450',  value = 'f450s'},
					{icon = "fas fa-truck", title = 'Ford F550 Plataforma',  value = 'f550rbc'},
		...
		elseif element.value == 'cloakroom3' then
			ESX.CloseContext()
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
		...
		elseif element.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'mechanic4', function(data, menu)
			      ESX.CloseContext()
			end)
		end
	end, function(menu)
...
function OpenMobileMechanicActions4Menu()
	local elements = {
		{unselectable = true, icon = "fas fa-gear", title = TranslateCap('mechanic')},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						ESX.CloseContext()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic2', TranslateCap('mechanic'), amount)
					end
				end
			end)
...
AddEventHandler('esx_mechanicjob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'MechanicActions4' then
		CurrentAction     = 'mechanic_actions_menu'
		CurrentActionMsg  = TranslateCap('open_actions')
		CurrentActionData = {}
	elseif zone == 'Garage4' then
		CurrentAction     = 'mechanic_harvest_menu'
		CurrentActionMsg  = TranslateCap('harvest_menu')
		CurrentActionData = {}
	elseif zone == 'Craft4' then
		CurrentAction     = 'mechanic_craft_menu'
		CurrentActionMsg  = TranslateCap('craft_menu')
		CurrentActionData = {}
...
AddEventHandler('esx_mechanicjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic4' and not IsPedInAnyVehicle(playerPed, false) then
...
-- Create Blips
CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.MechanicActions4.Pos.x, Config.Zones.MechanicActions4.Pos.y, Config.Zones.MechanicActions4.Pos.z)
...
CreateThread(function()
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic4' then
			Sleep = 500
...
CreateThread(function()
	...
	local sleep = 500
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic4' then
				if CurrentAction == 'mechanic_actions_menu' then
					OpenMechanicActions3Menu()
...
RegisterCommand('mechanicMenu4', function()
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic4' then
			OpenMobileMechanicActions4Menu()
		end
end, false)
...
RegisterKeyMapping('mechanicMenu4', 'Open Mechanic Menu', 'keyboard', 'F6')
