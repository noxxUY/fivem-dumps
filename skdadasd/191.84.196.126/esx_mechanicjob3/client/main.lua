	...
	ESX.OpenContext("right", elements, function(menu,element)
		if element.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements2 = {
					{unselectable = true, icon = "fas fa-car", title = TranslateCap('service_vehicle')},
					{icon = "fas fa-truck", title = 'Ford F450',  value = 'f450c'},
					{icon = "fas fa-truck", title = 'Ford F550 Plataforma',  value = 'f550rb'},
		...
		elseif element.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'mechanic3', function(data, menu)
			      ESX.CloseContext()
			end)
		end
	end, function(menu)
...
function OpenMobileMechanicActions3Menu()
	local elements = {
		{unselectable = true, icon = "fas fa-gear", title = TranslateCap('mechanic')},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						ESX.CloseContext()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanic3', TranslateCap('mechanic'), amount)
					end
				end
			end)
...
AddEventHandler('esx_mechanicjob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'MechanicActions3' then
		...
		CurrentActionMsg  = TranslateCap('open_actions')
		CurrentActionData = {}
	elseif zone == 'Garage3' then
		CurrentAction     = 'mechanic_harvest_menu'
		CurrentActionMsg  = TranslateCap('harvest_menu')
		CurrentActionData = {}
	elseif zone == 'Craft3' then
		CurrentAction     = 'mechanic_craft_menu'
		CurrentActionMsg  = TranslateCap('craft_menu')
		CurrentActionData = {}
...
-- Create Blips
CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.MechanicActions3.Pos.x, Config.Zones.MechanicActions3.Pos.y, Config.Zones.MechanicActions3.Pos.z)
...
RegisterCommand('mechanicMenu3', function()
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic3' then
			OpenMobileMechanicActions3Menu()
		end
end, false)
...
RegisterKeyMapping('mechanicMenu3', 'Open Mechanic Menu', 'keyboard', 'F6')
