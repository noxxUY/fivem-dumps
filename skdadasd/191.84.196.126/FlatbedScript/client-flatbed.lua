...
Citizen.CreateThread(function()
    while true do
        ...
        if veh and GetEntityModel(veh) == GetHashKey(config.flatbed_name[1]) or GetEntityModel(veh) == GetHashKey(config.flatbed_name[2]) or GetEntityModel(veh) == GetHashKey(config.flatbed_name[3]) or GetEntityModel(veh) == GetHashKey(config.flatbed_name[4]) then
           lastTruck = veh
	...
	while true do
        ...
        if IsPedInAnyVehicle(ped, false) then
            if IsEntityAttached(veh) then
                ...
                if vehicleHandle ~= nil and GetEntityModel(vehicleHandle) == GetHashKey(config.flatbed_name[1]) or GetEntityModel(vehicleHandle) == GetHashKey(config.flatbed_name[2]) or GetEntityModel(vehicleHandle) == GetHashKey(config.flatbed_name[3]) or GetEntityModel(vehicleHandle) == GetHashKey(config.flatbed_name[4]) then
                    Citizen.InvokeNative(0x8509B634FBE7DA11, "STRING")
