...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName('Partido politico')
	...
	while true do
        ...
        local dstCheck = #(pedCoords - locationPos)
           	if dstCheck <= 8.0 and on_job_delivery == false then
               	sleepThread = 5
				DrawText3Ds(-1059.40, -240.01, 44.01, '~g~[E] Para trabajar colocando posters', 0.4)
				if dstCheck <= 1.5 and on_job_delivery == false and IsControlJustReleased(0, 38) then
					...
					ESX.ShowNotification('Andá hacia la ubicación marcada a colocar los posters.')
				end
			end
  ...
  while true do
        ...
        local dstCheck = #(pedCoords - poster_location)
           	if dstCheck <= 6.5 and on_job_delivery == true and posted_posted == false then
               	sleepThread = 0
				DrawText3Ds(randomCoords.x, randomCoords.y, randomCoords.z, '~g~[E] Para colocar poster', 0.4)
...
function placeposter()
	...
	PayForJob()
	ESX.ShowNotification('Pago enviado por colocar poster a tu banco')
...
function stopJob()
	if on_job_delivery == true then
		...
		ESX.ShowNotification('Bien papa, volve mañana para mas cartelitos!')
	else
		print('not sure why this posted leme know though MrNewb#6475')
	end
end
