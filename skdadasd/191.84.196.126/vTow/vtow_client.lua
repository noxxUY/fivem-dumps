...
RegisterCommand("tow", function(source, args)
    ...
    if towing then
        ...
        elseif not towSetupMode then
            ShowNotification("Anda a tu ~r~camion de remolque ~s~y cuando veas el", "~y~marcador amarillo ~s~, presiona la tecla ~r~~h~HOME~h~", "~s~para seleccionar el vehiculo a remolcar.")
            towSetupMode = true
            towtruck = nil
            target = nil
            helpstate = 1
                ...
                if hit and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if (IsControlJustPressed(0, 213)) then
                        ...
                        elseif helpstate == 2 and veh ~= towtruck then
                            ShowNotification("El vehiculo ya fue seleccionado.", "~r~/tow ~s~para remolcar, o presiona", "~r~~h~HOME~h~~s~ para cancelar.")
                            target = veh
                            helpstate = 3
                        end
                    end
...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/tow', 'Empezar o parar de remolcar un vehiculo')
    while true do
        Citizen.Wait(0)
        if towSetupMode then
