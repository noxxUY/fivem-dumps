				...
				if IsControlJustReleased(0, 51) then
					ESX.TriggerServerCallback('esx_CountyAlive:CheckPlayerBalance', function(enoughMoney)
						if enoughMoney then
							...
							ESX.ShowNotification('~r~[ERROR]~w~ No tenes suficiente dinero!')
						end
		...
		while true do
			...
			for k,v in pairs(Config.FoodVendors) do
				...
				while #(GetEntityCoords(PlayerPedId()) - markerlocation) <= 1.0 do
					Citizen.Wait(0)
					ESX.Game.Utils.DrawText3D(markerlocation, " ~y~[E]~s~ Para comprar items", 0.6)
					if IsControlJustReleased(0, 51) then
		...
		while true do
			...
			for k,v in pairs(Config.FishingLocations) do
				...
				while #(GetEntityCoords(PlayerPedId()) - markerlocation) <= 1.0 do		
					if isFishing then
						Citizen.Wait(0)
						ESX.Game.Utils.DrawText3D(markerlocation, "~y~[E]~s~ para parar de pescar", 0.6)
						if IsControlJustReleased(0, 51) then
						...
						Citizen.Wait(0)
						ESX.Game.Utils.DrawText3D(markerlocation, "~y~[E]~s~ para empezar a pescar", 0.6)
						if IsControlJustReleased(0, 51) then
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			for k,v in pairs(Config.RentalVehicles) do
				...
				while #(GetEntityCoords(PlayerPedId()) - markerlocation) <= 1.0 do
					Citizen.Wait(0)
					ESX.Game.Utils.DrawText3D(markerlocation, "~o~[E]~s~ para alquilar un vehiculo", 0.6)
					if IsControlJustReleased(0, 51) then
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.RentalVehicleReturnCoord) <= 3.0 do
				Citizen.Wait(0)
				ESX.ShowHelpNotification('~b~E~w~ para eliminar el vehiculo', true)
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.TrailSignInCoord) <= 3.0 do
				Citizen.Wait(0)
				ESX.Game.Utils.DrawText3D(Config.TrailSignInCoord, "~o~[E]~s~ para empezar a subir el monte!", 0.6)
				if IsControlJustReleased(0, 51) then
					if not isHiking then
						TriggerServerEvent('esx_CountyAlive:GlobalAlert', Config.HikingAlertText)
						isHiking = true
					else
						ESX.ShowNotification('~r~[ERROR]~w~ ya iniciaste el camino!')
					end
				end
			end
		end
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			while #(GetEntityCoords(PlayerPedId()) - Config.TrailEndCoords) <= 3.0 do
				Citizen.Wait(0)
				ESX.Game.Utils.DrawText3D(Config.TrailEndCoords, "~o~[E]~s~ para finalizar el viaje!", 0.6)
				if IsControlJustReleased(0, 51) then
					if isHiking then
						...
						ESX.ShowNotification('~r~[ERROR]~w~ Nunca iniciaste el viaje!')
					end
				end
			end
		end
