...
--Version: '4.1'
--Requirements: 'gamebuild 2189 or higher'
--Details: 'Spawns the island into freeroam without requiring a hopper'
...
CreateThread(function()
	SetDeepOceanScaler(0.0) -- NO POLYZONE REQUIRED. 
	...
	while true do
		...
		if #(pCoords - islandCoords) < 2000.0 then
			if not islandLoaded then
				...
				Citizen.InvokeNative(0xF74B1FFA4A15FBEA, 1) -- island path nodes
				SetScenarioGroupEnabled('Heist_Island_Peds', 1)
				-- SetAudioFlag('PlayerOnDLCHeist4Island', 1) -- Disables radio from working in CP. Un-comment if you wish. 
