...
local function loopFullGarage(ped, garage, stop)
    ...
    if distance < 3 then
        if isDriving then
            DrawText3Ds(gcoords.x, gcoords.y, gcoords.z + 1.0, "The Tesla Service Center is ~r~full~w~.")
            return
        end

        SetTextComponentFormat('STRING')
        AddTextComponentString('Press ~INPUT_PICKUP~ pickup your Tesla')
...
local function loopAvailableGarage(ped, garage, stop)
    ...
    if distance < 3 then
        SetTextComponentFormat('STRING')
        AddTextComponentString('Press ~INPUT_PICKUP~ to bring your Tesla to the service')
        DisplayHelpTextFromStringLabel(0, 0, 1, -1)

        DrawText3Ds(gcoords.x, gcoords.y, gcoords.z + 1.0, "Your Tesla gets ~g~charged ~w~and ~g~repaired ~w~at this Service Center.")
