...
local vehicles = "teslax,models,model3,modely,tesroad20,cybertruck,semi" -- define what cars can be used as a tesla (by modelname), set to "" (empty) for all cars
local debug = false
...
RegisterCommand("tesla", function(source, args)
	if(args[1] == "mark") then
		...
		elseif(tesla) then
			...
			minimap("Only available in a Tesla.")
		end
	end
end, false)
