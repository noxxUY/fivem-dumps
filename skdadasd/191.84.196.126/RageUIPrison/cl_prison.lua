...
local quitMenu = RageUI.CreateMenu("Sortie","~y~Prisonnier : ".. GetPlayerName(PlayerId()) .. '')
local subMenu =  RageUI.CreateSubMenu(quitMenu, "Sortie", "~y~Prisonnier : ".. GetPlayerName(PlayerId()) .. '')



local ctMenu = RageUI.CreateMenu("Cantina", "~y~Prisionero : ".. GetPlayerName(PlayerId()) .. '')
local wpMenu = RageUI.CreateMenu("Negro turbina", "Comprar un arma")


ActiverMarkerPrisonTenu = true -- false si vous ne voulez pas les Markers aux sols
local menu = RageUI.CreateMenu("Entrée Prison", "~y~Prisonnier : ".. GetPlayerName(PlayerId()) .. '')	 
ActiverMarkerCivil = true -- false si vous ne voulez pas les Markers aux sols
local menu1 = RageUI.CreateMenu("Prison Tenue", "~y~Prisonnier : ".. GetPlayerName(PlayerId()) .. '')	 

local menu3= RageUI.CreateMenu("Prison Recyclerie","~y~Prisonnier : ".. GetPlayerName(PlayerId()) .. '')
...
Citizen.CreateThread(function()
    ...
    while (true) do
		...
		if Vdist2(GetEntityCoords(PlayerPedId(), false), Positions.positionrecyclerie.x, Positions.positionrecyclerie.y, Positions.positionrecyclerie.z) < 3 then
            ...
            ESX.ShowHelpNotification('~INPUT_TALK~ Para reciclar piezas')
            if IsControlJustPressed(1, 51) then
		...
		if GetDistanceBetweenCoords(coords, 693.10,173.76,80.74, true) < 2.0 then
			ESX.Game.Utils.DrawText3D({ x = 693.17, y = 173.77, z =80.74 }, '~y~[E] ~w~Buscar objetos', 0.6)
		...
		if GetDistanceBetweenCoords(coords, 704.81,169.24,80.72, true) < 2.0 then
			ESX.Game.Utils.DrawText3D({ x = 704.81, y = 169.24, z = 80.72}, '~y~[E] ~w~Buscar objetos', 0.6)
			if recolerbnj == false and maitem == false then
				collecter()
			end					
		end
        ...
        RageUI.IsVisible(ctMenu, function()
            RageUI.Button('Comida', nil, { RightLabel = "~y~20 Tickets" }, true, {
                onSelected = function()
                    TriggerServerEvent('bnj:GivePrsnPain')
                end,
            });
            RageUI.Button('Agua', nil, { RightLabel = "~b~20 Tickets" }, true, {
                onSelected = function()
        ...
        RageUI.IsVisible(wpMenu, function()
            ...
            RageUI.Button('Daga', nil, { RightLabel = "~b~30 Tickets" }, true, {
                onSelected = function()
                    TriggerServerEvent('bnj:BuyItem', "weapon_dagger", 1, 30)
                end,
            });
            RageUI.Button('Palanca', nil, { RightLabel = "~b~40 Tickets" }, true, {
                onSelected = function()
                    TriggerServerEvent('bnj:BuyItem', "weapon_crowbar", 1, 40)
                end,
            });
            end, function()
        end)
        RageUI.IsVisible(subMenu, function()
            ...
            RageUI.Separator("Numero de tickets: ~g~"..count)
		...
		RageUI.IsVisible(menu3, function()
			RageUI.Button('Vender las piezas', nil,  { RightBadge = RageUI.BadgeStyle.Car},  true, {
				onSelected = function()
					skup()
					RageUI.CloseAll()
				end,
     ...
     Citizen.Wait(5)
         ...
         for k, v in pairs(douche) do
             if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 1.5 then
                     ESX.ShowHelpNotification(' ~INPUT_CONTEXT~ para darte una ducha')
