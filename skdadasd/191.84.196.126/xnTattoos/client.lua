	...
	TriggerEvent('skinchanger:getSkin', function()
		if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
			...
			TriggerEvent('skinchanger:loadSkin', {
				...
				shoes_1 = 5,
				glasses_1 = 5,
				bulletproof_vest_1 = 0,
				bulletproof_vest_2 = 0,
				bag = 0
			})
		end
	end)
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Eliminaste ~y~" .. GetLabelText(label) .. "~s~ tattoo")
end
...
function CreateScale(sType)
	...
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Cambiar vista de camara", 21, "Cambiar opacidad", {90, 89}, "Comprar/Eliminar tatto", 191)
		scaleString = sType
	end
end
