...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if distance <= 25.0 and hasjob then
            sleep = 0
            DrawMarker(23, Config.JobCenter, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 255, 255, 150, false, false, 2, false, false, false)
...
AddEventHandler('esx_ets:start', function(data, jobid)
    ...
    local destinationBlip, truckBlip, trailerBlip = addBlip(data.arrive, 38, 3, 0.5, Strings['destination'])
    while true do
        ...
        if not IsPedInVehicle(PlayerPedId(), truck, false) then
            ...
            if not DoesBlipExist(truckBlip) then
                truckBlip = addBlip(GetEntityCoords(truck), 477, 5, 0.5, Strings['truck']) 
            ...
            if not IsVehicleAttachedToTrailer(truck, trailer) then
                ...
                if not DoesBlipExist(trailerBlip) then 
                    trailerBlip = addBlip(GetEntityCoords(trailer), 479, 5, 0.5, Strings['trailer']) 
                ...
                if not DoesBlipExist(destinationBlip) then
                    destinationBlip = addBlip(data.arrive, 38, 3, 0.5, Strings['destination'])
    ...
    while true do
        ...
        if not IsPedInVehicle(PlayerPedId(), truck, false) and distance >= 10.0 then
            ...
            if DoesBlipExist(truckBlip) then RemoveBlip(truckBlip) end
            if not DoesBlipExist(destinationBlip) then
                destinationBlip = addBlip(data.start[1], 38, 3, 0.5, Strings['destination'])
                SetBlipRoute(destinationBlip, true)
            end
