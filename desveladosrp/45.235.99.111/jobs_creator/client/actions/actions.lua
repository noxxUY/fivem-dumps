...
local function checkAllowedActions()
    ...
    TriggerServerCallback(Utils.eventsPrefix .. ':checkAllowedActions', function(data)
        ...
        for action, enabled in pairs(data) do
            if(enabled and not ACTIONS_BLACKLIST[action]) then
                canDoAnyAction = true
                break
            end
        end
        ...
        actionsMenuEnabled = data.actionsMenuEnabled
...
local function openActionsMenu()
    if not canDoAnyAction or not actionsMenuEnabled then return end
    ...
    if(canHandcuff) then
        table.insert(elements, {label = getLocalizedText('actions_put_soft_handcuffs'), value = "soft_handcuff"})
        table.insert(elements, {label = getLocalizedText('actions_put__hard_handcuffs'), value = "hard_handcuff"})
    ...
    function(data, menu) 
        ...
        elseif(action == "soft_handcuff") then
            TriggerEvent(Utils.eventsPrefix .. ':actions:softHandcuff')
        elseif(action == "hard_handcuff") then
            TriggerEvent(Utils.eventsPrefix .. ':actions:hardHandcuff')
