...
local pizzeria = { x = 222.16,  y = -32.24,  z = 68.84} --Configuration marker prise de service
local pizzeriafin = { x = 228.28,  y = -31.76,  z = 68.72} --Configuration marker fin de service
local spawnfaggio = { x = 224.12,  y = -38.6,  z = 68.68 } --Configuration du point de spawn du faggio
...
local blips = {
  {title="Pizzeria", colour=2, id=348, x = 222.16,  y = -32.24,  z = 69.84}, --Configuration du point sur la carte
}

local coefflouze = 5 --Coeficient multiplicateur qui en fonction de la distance definit la paie
  ...
  while true do
    ...
    if isInJobPizz == false then
      ...
      if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Presiona ~INPUT_CONTEXT~ para iniciar ~r~la entrega de pizza",0,1,0.5,0.8,0.6,255,255,255,255)
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        TriggerEvent("pNotify:SendNotification", {
          text = "Dirección : " ..destinol.. " de entrega",
          type = "info",
          queue = "global",
          timeout = 4000,
          layout = "topRight"
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        HelpText("Presiona ~INPUT_CONTEXT~ para entregar la pizza",0,1,0.5,0.8,0.6,255,255,255,255)

        if IsControlJustPressed(1,38) then

          notif2 = true
          posibilidad = math.random(1000, 1500)
          ...
          if (posibilidad > 70) and (posibilidad < 90) then
            ...
            pourboire = math.random(1000, 1500)

            TriggerEvent("pNotify:SendNotification", {
              text = "Pedido : " .. pourboire .. "$",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "topRight"
    ...
    if isToPizzaria == true then
      ...
      if GetDistanceBetweenCoords(pizzeria.x,pizzeria.y,pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 and afaitunepizzamin == true then
        HelpText("Presiona ~INPUT_CONTEXT~ para agarrar más pizzas",0,1,0.5,0.8,0.6,255,255,255,255)

        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("150"))  then
          ...
          if IsControlJustPressed(1,38) then
            ...
            if IsInVehicle() then
              ...
              TriggerEvent("pNotify:SendNotification", {
                text = "Le agradecemos su trabajo, aquí está su pago : " .. paie .. "$",
                type = "success",
                queue = "global",
                timeout = 4000,
                layout = "topRight"
  ...
  while true do
    ...
    if isInJobPizz == true then
      ...
      if GetDistanceBetweenCoords(pizzeriafin.x, pizzeriafin.y, pizzeriafin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        HelpText("Presiona ~INPUT_CONTEXT~ para detener ~r~la entrega de pizza",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          if afaitunepizzamin == true then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Gracias por trabajar, que tengas un buen día",
              type = "success",
              queue = "global",
              timeout = 4000,
              layout = "topRight"
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Gracias de todos modos (por nada), que tengas un buen día",
              type = "error",
              queue = "global",
              timeout = 4000,
              layout = "topRight"
