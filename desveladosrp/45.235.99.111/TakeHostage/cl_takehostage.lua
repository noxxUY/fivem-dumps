...
function callTakeHostage()
	...
	for i=1, #takeHostage.allowedItem do
		if HasPedGotWeapon(PlayerPedId(), takeHostage.allowedItem[i], false) then
			if GetAmmoInPedWeapon(PlayerPedId(), takeHostage.allowedItem[i]) > 0 then
				canTakeHostage = true 
				foundWeapon = takeHostage.allowedItem[i]
				break
			end 					
		end
	end
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas un arma cargada para hacer esto!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~No hay nadie cerca para usar de rehen!")
			end
		else
			drawNativeNotification("~r~No hay nadie cerca para usar de rehen!")
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Presiona [G] para soltar, [H] para matar")
