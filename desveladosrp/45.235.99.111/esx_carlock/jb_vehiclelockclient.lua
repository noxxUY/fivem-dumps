local Keys = {
  ...
  --================ end variables
  --================ begin config
  Config.disableNPC = false    --Carjacking 
  --================ end config
	  ...
	  if DoesEntityExist(vehicle) then
		  if IsPedInAnyVehicle(player) or IsPedInAnyHeli(player) then
			  if vehicleplate ~= nil and clostestvehicleplate ~= nil then
				  if not table.empty(VehicleList) then
					  ...
					  if isvehiclefound then
						  if (islocked == 1)then
							  ...
							  Notify("El vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ fue ~r~cerrado ~w~.")
							  -- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
						  else
							  SetVehicleDoorsLocked(vehicle,1)
							  Notify("El vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ fue ~g~desbloqueado ~w~.")
				  ...
				  Notify("~r~No hay vehículo para bloquear.")
			  end
		  else
			  if not table.empty(VehicleList) then
					  ...
					  if isvehiclefound then
						  if not foundclostestvehicle then 
							  if (islockedHandle == 1)then
								  ...
								  Notify("El vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ fue ~r~cerrado ~w~.")
								  -- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
							  else
								  SetVehicleDoorsLocked(vehicleHandle,1)
								  Notify("El vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicleHandle))) .. "~w~ fue ~g~desbloqueado ~w~.")
								  -- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
							  end
						  else
							  if (islockedclostestvehicle == 1)then
								  SetVehicleDoorsLocked(clostestvehicle, 2)
								  Notify("El vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(clostestvehicle))) .. "~w~ fue ~r~cerrado ~w~.")
								  -- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "lock", 1.0)
								  foundclostestvehicle = false
							  else
								  SetVehicleDoorsLocked(clostestvehicle,1)
								  Notify("El vehiculo ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(clostestvehicle))) .. "~w~ fue ~g~desbloqueado ~w~.")
								  -- TriggerServerEvent("InteractSound_SV:PlayWithinDistance", soundDistance, "unlock", 1.0)
								  foundclostestvehicle = false
							  end
						  ...
						  Notify("El vehículo que quieres abrir es ~r~ no tuyo~w~.")
					  end
				  else
					  Notify("~r~ningún vehículo para bloquear.")
				  end
			  else
				  Notify("~r~No hay vehículo para bloquear o el vehículo está bloqueado.")
			  end
		  end
	  else
		  if DoesEntityExist(vehicleHandle) then
			  if vehicleHandleplate ~= nil then
				  ...
				  if isvehiclefound then
					  ...
					  Notify("El vehículo que desea desbloquear está ~r~bloqueado~w~.")
				  end
			  end
		  end
	  end
