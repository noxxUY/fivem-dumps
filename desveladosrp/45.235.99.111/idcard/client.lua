...
--local LAurl = exports["mugshot"]:getMugshotUrl(PlayerPedId())
...
Citizen.CreateThread(function()
	while true do 
		Wait(0)
		if IsControlJustReleased(0, Keys['F10']) and not open then
			openMenu()
		end
		if IsControlJustReleased(0, Keys['ESC']) and open or IsControlJustReleased(0, Keys['BACKSPACE']) and open or IsControlJustReleased(0, Keys['F3']) and open then --ESC OR BACKSPACE
			SendNUIMessage({
				action = "close"
			})
			open = false
