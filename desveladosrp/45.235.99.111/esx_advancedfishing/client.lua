...
for _, info in pairs(Config.MarkerZones) do
	...
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString("Renta de botes")
			...
			if #(pedCoords - v.Return) < 100.0 and IsPedInAnyVehicle(ped) then
				...
				if #(pedCoords - v.Return) < 5.0 then
					if DoesEntityExist(veh) and GetVehiclePedIsIn(ped) == veh then
						DisplayHelpText('Presiona ~INPUT_CONTEXT~ para devolver el bote')
						if IsControlJustReleased(0, Keys['E']) then
							for _, q in pairs(Config.RentalBoats) do
								if GetEntityModel(veh) == GetHashKey(q.model) then
									...
									ESX.ShowNotification("Te han devuelto tu depósito de ~g~$" .. deposit)
								end
							end
						...
						DisplayHelpText('Este no es un vehículo rentado')
					end
				end
			end
		end
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Has parado de pescar")
			...
			if fishing then
				...
				if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z-3.0)  then
					...
					fishing = false
					ESX.ShowNotification("~r~Has parado de pesca")
				end
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~Se te ha escapado")
				end
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if fishing then
			pause = true
			correct = 1
			ESX.ShowNotification("El pescado mordió el anzuelo\nPresiona ~y~F~s~para atraparlo")
			input = 0
			pausetimer = 0
		end	
	end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~r~No podes hacer esto desde un vehículo")
	else
		if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z-3.0)  then
			ESX.ShowNotification("Has comenzado la pesca\nPresiona ~y~X~s~ para parar")
			TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
			print("La pesca ha comenzado")
		else
			ESX.ShowNotification("Estás muy cerca de la costa, aléjate")
		end
	end
end, false)
    ...
    while true do
        ...
        for k, v in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
				DisplayHelpText('Presiona ~INPUT_CONTEXT~ para rentar un bote')
...
function OpenBoatsMenu(xyz, h)
	...
	for k, v in ipairs(Config.RentalBoats) do
		if data.current.value == v.model then
			...
			ESX.ShowNotification("Has pagado ~r~$" .. deposit .. "~s~ para pagar el seguro ~r~$" .. deposit .. "~s~ del bote.")
			ESX.ShowNotification("Devuelve el ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(v.model)) .. "~s~para que te devueltan el ~r~deposito~s~")
