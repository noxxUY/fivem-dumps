...
AddEventHandler('route68_ruletka:start', function()
	ESX.TriggerServerCallback('esx_roulette:check_money', function(quantity)
		if quantity >= 10 then
			...
			ESX.ShowNotification('¡Necesitas al menos 10 fichas para jugar!')
			SendNUIMessage({
				type = "reset_bet"
			})
		end
...
AddEventHandler('esx_roulette:start_game', function(action, amount)
	...
	if game_during == false then
		...
		TriggerEvent('pNotify:SendNotification', {text = "Tu apuestas "..amount.." Fichas de Casino "..kolorBetu..". La rueda gira..."})
		game_during = true
		...
		if action == 'black' then
			if has_value(black, randomNumber) then
				local win = amount + 3
				ESX.ShowNotification('Tu has ganado '..win..' Fichas de Casino!')
				TriggerServerEvent('esx_roulette:giveasmoney', action, amount)
			else
				ESX.ShowNotification('Perdiste!!. ¡Vuelve a intentarlo! ¡Buena suerte!')
			end
		...
		elseif action == 'green' then
			...
			if randomNumber == 0 then
				ESX.ShowNotification('Tu as ganado '..win..' Fichas de Casino!')
				TriggerServerEvent('esx_roulette:giveasmoney', action, amount)
			else
				ESX.ShowNotification('Perdiste!!. ¡Vuelve a intentarlo! ¡Buena suerte!')
			end
		...
		ESX.ShowNotification('La rueda gira...')
	end
end)