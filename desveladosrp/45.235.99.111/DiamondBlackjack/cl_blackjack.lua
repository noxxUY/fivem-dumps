...
cfg.blackjackTables = {
    ...
    [4] = {
        dealerPos = vector3(-2615.78,1694.35,141.82),
        dealerHeading = 207.0,
        tablePos = vector3(1144.429, 247.3352, 1252.041),
        tableHeading = 135.31,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Presiona ~INPUT_PICKUP~ para jugar al Blackjack")
                    else 
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Tu mano ["..tostring(currentHand).."] - Mano del croupier ["..tostring(dealersHand).."]")
    ...
    while true do 
        if waitingForBetState then
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                local tmpInput = getGenericTextInput("Monto de la apuesta")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "El objetivo del Blackjack es ganarle a la mano del crupier sin pasar de 21. Este juego utiliza cuatro barajas de 52 cartas, que se barajan al comienzo de cada mano. El crupier se plantará con un 17 suave.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
Citizen.CreateThread(function()
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                DrawRect(0.924, 0.928, 0.121, 0.032, 0, 0, 0, 150) 
                DrawAdvancedNativeText(0.993, 0.935, 0.005, 0.0028, 0.29, "Apuesta:", 255, 255, 255, 255, 0, 0)
        ...
        if drawTimerBar then
            DrawRect(0.924, 0.886, 0.121, 0.032, 0, 0, 0, 150)
            DrawAdvancedNativeText(0.993, 0.892, 0.005, 0.0028, 0.29, "Tiempo:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Hagan sus apuestas")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Nadie realizó una apuesta, ronda omitida")
        end
    end)
end)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Que comience el próximo juego ...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Hiciste un Empate !")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:chipsCleanup",function(chairId,tableId)
    if closeToCasino then
        if string.sub(chairId, -5) ~= "Dinero" then
            dealerPed = getDealerFromTableId(tableId)
            local gender = getDealerGenderFromPed(dealerPed)
            if gender == "male" then 
                genderAnimString = "" 
...
function betChipsForNextHand(chipsAmount,chipsProp,something,chairID,someBool,zOffset)
    ...
    if cardObjects[tostring(chairID) .. "Dinero"] ~= nil then
        table.insert(cardObjects[tostring(chairID) .. "Dinero"],chipsObj)
    else 
        cardObjects[tostring(chairID) .. "Dinero"] = {}
        table.insert(cardObjects[tostring(chairID) .. "Dinero"],chipsObj)
    end
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("Pedir carta") --Space
    PopScaleformMovieFunctionVoid()  
