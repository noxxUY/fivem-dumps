...
local casinoEntranceVector = vector3(923.76627685547,47.726057739258,81.104507446289)
...
RMenu:Get('casino_exit', 'casino'):SetSubtitle("~b~Salir")

Citizen.CreateThread(function()
    local blip = AddBlipForCoord(923.7822, 46.3247, 81.10634)
    SetBlipAsShortRange(blip, true)
    SetBlipSprite(blip, 382)
    SetBlipColour(blip, 3)
    SetBlipScale(blip, 1.5)
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentSubstringPlayerName("~b~Diamond Casino")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('casino_exit', 'casino')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.Button("Salir del Casino Diamond", "",{ RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('casino_enter', 'casino')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.Button("Ingreso al Casino Diamond", "",{ RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
Citizen.CreateThread(function()
    while true do 
        ...
        DrawMarker(27, casinoEntranceVector.x, casinoEntranceVector.y, casinoEntranceVector.z-1.0, 0, 0, 0, 0, 0, 0, 6.001, 6.0001, 0.5001, 255,255,255, 200, 0, 0, 0, 0)
        DrawMarker(27, casinoExitVector.x, casinoExitVector.y, casinoExitVector.z-1.0, 0, 0, 0, 0, 0, 0, 3.001, 3.0001, 0.5001, 255,255,255, 200, 0, 0, 0, 0)
        Wait(0)
    end
end)
