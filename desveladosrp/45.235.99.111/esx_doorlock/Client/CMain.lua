...
TriggerEvent("chat:addSuggestion", Config['commands'].CreateDoor, ("Agregar una puerta"), {})
TriggerEvent("chat:addSuggestion", Config['commands'].RemoveDoor, ("Quitar una puerta"), {})
...
RegisterNetEvent("guille_doorlock:client:setUpDoor", function()
    local elements = {}
    table['insert'](elements, {label = "Tipo de puerta: " .._doorType, value = "doortype"})
    table['insert'](elements, {label = "Distancia: " .._distToDoor, value = "doordist"})
    table['insert'](elements, {label = "Añadir trabajo", value = "addjob"})
    table['insert'](elements, {label = "Añadir pin: " ..doorPin, value = "doorpin"})
    table['insert'](elements, {label = "Añadir item: " ..item, value = "dooritem"})
    for k, v in pairs(allowedJobs) do
        table['insert'](elements, {label = v, value = k})
    end
    table['insert'](elements, {label = "Confirmar creacion", value = "conf"})
    ...
    elements = elements, 
        ...
        elseif v == "doordist" then
            ESX['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'dist_to_door', {
                title = 'Distancia a la puerta'
            }, function(data2, menu2)
                local dist = tonumber(data2['value'])
                if dist == nil then
                ESX['ShowNotification']('Invalido, prueba de nuevo')
				else
                    menu2['close']()
                    _distToDoor = dist
                    ExecuteCommand(Config['commands'].CreateDoor)
        ...
        elseif v == "addjob" then
            ESX['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'new_job', {
                title = 'Introduce el nombre del trabajo'
            }, function(data2, menu2)
        ...
        elseif v == "doorpin" then
            ESX['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'new_job', {
                ...
                if pin == nil then
                    ESX['ShowNotification']('Invalid, prueba de nuevo')
                else
                    doorPin = pin
                    ESX['UI']['Menu']['CloseAll']()
                    ExecuteCommand(Config['commands'].CreateDoor)
        ...
        elseif v == "dooritem" then
            ESX['UI']['Menu']['Open']('dialog', GetCurrentResourceName(), 'new_jobitem', {
                title = 'Introduce el nombre del item'
            }, function(data2, menu2)
                local pin = data2['value']
                if pin == nil then
                    ESX['ShowNotification']('Invalido, prueba de nuevo')
...
RegisterNetEvent("guille_doorlock:client:deleteDoor", function()
    _CreateThread(function()
        while true do
            ...
            ESX['ShowHelpNotification']("Presiona ~INPUT_CONTEXT~ para quitar la puerta")
            ...
            if IsControlJustPressed(1, 38) then
                ...
                if _found then
                    ESX['ShowNotification']("Has eliminado la puerta")
                    break
                else
                    ESX['ShowNotification']("La puerta no se ha eliminado")
                end
            end
            _Wait(_wait)
        end
        ...
        _CreateThread(function()
            while true do
                ...
                if IsEntityAnObject(entity) then
                    ESX['ShowHelpNotification']("Presiona ~INPUT_CONTEXT~ para añadir la puerta")
            ...
            while true do
                ...
                if IsEntityAnObject(entity) then
                    ...
                    if #_doorsDobule ~= 2 then
                        DrawLine(_coords, coords, 0, 255, 34, 255)
                        ESX['ShowHelpNotification']("Presiona ~INPUT_CONTEXT~ para añadir una puerta")
                    else
                        DrawLine(_coords, coords, 0, 255, 34, 255)
                        ESX['ShowHelpNotification']("Presiona ~INPUT_CONTEXT~ para confirmar el pin y el texto (importante)")
                    end
                    showedEntity = entity
                    if IsControlJustPressed(1, 38) then
                        local _doorCoords = GetEntityCoords(entity)
