	...
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("CarryPeoplepapito:sync",targetSrc)
				ensureAnimDict(carry.personCarrying.animDict)
				carry.type = "carrying"
			else
				drawNativeNotification("~r~No hay jugadores serca!")
			end
		else
			drawNativeNotification("~r~No hay jugadores serca!")
		end
		...
		TriggerServerEvent("CarryPeoplepapito:stop",carry.targetSrc)
		carry.targetSrc = 0
	end
end,false)

RegisterNetEvent("CarryPeoplepapito:syncTarget")
AddEventHandler("CarryPeoplepapito:syncTarget", function(targetSrc)
...
RegisterNetEvent("CarryPeoplepapito:cl_stop")
AddEventHandler("CarryPeoplepapito:cl_stop", function()
	carry.InProgress = false
	ClearPedSecondaryTask(PlayerPedId())
	DetachEntity(PlayerPedId(), true, false)
end)
