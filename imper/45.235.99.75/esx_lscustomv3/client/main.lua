...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_lscustomv3:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_lscustomv3:installMod')
AddEventHandler('esx_lscustomv3:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustomv3:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustomv3:cancelInstallMod')
AddEventHandler('esx_lscustomv3:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_lscustomv3:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_lscustomv3:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_lscustomv3:cancelInstallMod')
...
if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv2' and PlayerData.job.grade_name == 'experimente') or Config.IsMechanicv2JobOnly == false then
