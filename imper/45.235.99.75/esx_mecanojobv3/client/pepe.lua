...
function OpenMobilemechanicv2ActionsMenu()

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mobile_mechanicv2_actions',
    {
      title    = _U('mechanic'),
      align    = 'right',
      elements = {
    ...
    function(data, menu)
          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanicv2', _U('mechanic'), amount)
              end
            end
          end,
        function(data, menu)
      ...
      if data.current.value == 'object_spawner' then
        ...
        ESX.UI.Menu.Open(
          'default', GetCurrentResourceName(), 'mobile_mechanicv2_actions_spawn',
          {
            title    = _U('objects'),
            align    = 'right',
            elements = {
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_mechanicv2job:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_mechanicv2job:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
 ...
 ESX.TriggerServerCallback('esx_mechanicv2job:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_mechanicv2job:putStockItems', itemName, count)
            end
...
RegisterNetEvent('esx_mechanicv2job:onHijack')
AddEventHandler('esx_mechanicv2job:onHijack', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mechanicv2job:onCarokit')
AddEventHandler('esx_mechanicv2job:onCarokit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mechanicv2job:onFixkit')
AddEventHandler('esx_mechanicv2job:onFixkit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
AddEventHandler('esx_mechanicv2job:hasEnteredMarker', function(zone)

  if zone == 'mechanicv2Actions' then
    CurrentAction     = 'mechanicv2_actions_menu'
    CurrentActionMsg  = _U('open_actions')
    CurrentActionData = {}
  end

  if zone == 'Garage' then
    CurrentAction     = 'mechanicv2_harvest_menu'
    CurrentActionMsg  = _U('harvest_menu')
    CurrentActionData = {}
  end

  if zone == 'Craft' then
    CurrentAction     = 'mechanicv2_craft_menu'
...
AddEventHandler('esx_mechanicv2job:hasExitedMarker', function(zone)

  if zone == 'Craft' then
    TriggerServerEvent('esx_mechanicv2job:stopCraft')
    TriggerServerEvent('esx_mechanicv2job:stopCraft2')
    TriggerServerEvent('esx_mechanicv2job:stopCraft3')
  end

  if zone == 'Garage' then
    TriggerServerEvent('esx_mechanicv2job:stopHarvest')
    TriggerServerEvent('esx_mechanicv2job:stopHarvest2')
    TriggerServerEvent('esx_mechanicv2job:stopHarvest3')
  end
...
AddEventHandler('esx_mechanicv2job:hasEnteredEntityZone', function(entity)

  local playerPed = GetPlayerPed(-1)

  if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv2' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_mechanicv2job:hasExitedEntityZone', function(entity)
...
-- Create Blips
Citizen.CreateThread(function()
  local blip = AddBlipForCoord(Config.Zones.mechanicv2Actions.Pos.x, Config.Zones.mechanicv2Actions.Pos.y, Config.Zones.mechanicv2Actions.Pos.z)
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv2' then
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv2' then
      ...
      if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
        ...
        TriggerEvent('esx_mechanicv2job:hasEnteredMarker', currentZone)
      end
      if not isInMarker and HasAlreadyEnteredMarker then
        HasAlreadyEnteredMarker = false
        TriggerEvent('esx_mechanicv2job:hasExitedMarker', LastZone)
      end
    end
  end
end)
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_mechanicv2job:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_mechanicv2job:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv2' then

            if CurrentAction == 'mechanicv2_actions_menu' then
                Openmechanicv2ActionsMenu()
            end

            if CurrentAction == 'mechanicv2_harvest_menu' then
                Openmechanicv2HarvestMenu()
            end

            if CurrentAction == 'mechanicv2_craft_menu' then
                Openmechanicv2CraftMenu()
            end
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('esx_society:putVehicleILovePizzaInGarage', 'mechanicv2', vehicleProps)

              else

                if
                  GetEntityModel(vehicle) == GetHashKey('Towbennys')   or
                  GetEntityModel(vehicle) == GetHashKey('slamvan3')
                then
                  TriggerServerEvent('esx_service:disableService', 'mechanicv2')
                end
        ...
        if IsControlJustReleased(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv2' then
            OpenMobilemechanicv2ActionsMenu()
        end
    end
end)
...
function Openmechanicv2ActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mechanicv2_actions',
    {
      title    = _U('mechanic'),
      align    = 'right',
      elements = elements
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
              ...
              ESX.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mechanicv2', vehicleProps)
            ...
            local elements = {
              {label = ('Auto de bennys'), value = 'Towbennys'},
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'mechanicv2', function(data, menu)
          menu.close()
        end)
      end
...
function Openmechanicv2HarvestMenu()
  ...
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'mechanicv2_harvest',
      {
        title    = _U('harvest'),
        align    = 'right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'gaz_bottle' then
          menu.close()
          TriggerServerEvent('esx_mechanicv2job:startHarvest')
        end

        if data.current.value == 'fix_tool' then
          menu.close()
          TriggerServerEvent('esx_mechanicv2job:startHarvest2')
        end

        if data.current.value == 'caro_tool' then
          menu.close()
          TriggerServerEvent('esx_mechanicv2job:startHarvest3')
        end
...
function Openmechanicv2CraftMenu()
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'mechanicv2_craft',
      {
        title    = _U('craft'),
        align    = 'right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'blow_pipe' then
          menu.close()
          TriggerServerEvent('esx_mechanicv2job:startCraft')
        end

        if data.current.value == 'fix_kit' then
          menu.close()
          TriggerServerEvent('esx_mechanicv2job:startCraft2')
        end

        if data.current.value == 'caro_kit' then
          menu.close()
          TriggerServerEvent('esx_mechanicv2job:startCraft3')
        end
