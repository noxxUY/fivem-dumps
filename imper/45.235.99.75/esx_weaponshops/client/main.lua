...
Citizen.CreateThread(function()
    while GCM == nil do
        TriggerEvent('gcm:getSharedObject', function(obj) GCM = obj end)
        Citizen.Wait(0)
    end        
end)
...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_weaponshop2:getShop', function(shopItems)
...
RegisterNetEvent('esx_weaponshop2:sendShop')
AddEventHandler('esx_weaponshop2:sendShop', function(shopItems)
	for k,v in pairs(shopItems) do
		Config.Zones[k].Items = v
	end
end)
...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_weaponshop2:buyLicense', function(bought)
				if bought then
					menu.close()
					OpenShopMenu(zone)
				end
...
function OpenShopMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		ESX.TriggerServerCallback('esx_weaponshop2:buyWeapon', function(bought)
			if bought then
...
AddEventHandler('esx_weaponshop2:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_weaponshop2:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_weaponshop2:hasEnteredMarker', currentZone)
		end
		
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_weaponshop2:hasExitedMarker', LastZone)
		end
	end
end)
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) then
                if CurrentAction == 'shop_menu' then
                    GCM.TriggerServerCallback('gcm_vips:obtenerTipoDeVip', function(vipType)
                        if vipType then
							OpenShopMenu(CurrentActionData.zone)
                        else
                            ESX.ShowNotification("Tu no eres VIP ")
                        end
                    end)
                end
