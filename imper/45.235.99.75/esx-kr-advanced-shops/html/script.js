...
$(function() {
		window.addEventListener('message', function(event) {
			...
			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>Lojinha</h6>`
			)
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Ações do chefe</button>
					`
				);
			}
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<h6>${cb[i].label}</h4>
							<h6>$${cb[i].price} por item</h4>
							<h6>En Stock: ${cb[i].count}</h4>
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Cantidades"></textarea>
							</div>`
								);
							};
							...
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">Voltar</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$.post('http://esx-kr-advanced-shops/notify', JSON.stringify({msg : "~r~Um dos itens não tem estoque suficiente ou o valor é inválido."}));
					}
				}
			});
