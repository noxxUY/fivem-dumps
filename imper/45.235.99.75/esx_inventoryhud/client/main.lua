...
Citizen.CreateThread(
    function()
        while ESX == nil do
            TriggerEvent(
                "esx:getShILovePizzaaredObjILovePizzaect",
                function(obj)
                    ESX = obj
                end
            )
    ...
    function(data, cb)
        ...
        for i = 1, #players, 1 do
            if players[i] ~= PlayerId() then
				...
				ExecuteCommand("me Pasa un objeto")
                    foundPlayer = true
                end
            end
        end
        ...
        if foundPlayer then
            ...
            if data.item.type == "item_weapon" then
				...
				ExecuteCommand("me Pasa un arma")
				
            end

            TriggerServerEvent("esx:giILovePizzaveInventoryItem", data.player, data.item.type, data.item.name, count)
