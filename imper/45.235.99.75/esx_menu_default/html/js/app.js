(function(){
	...
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" class="menu{{#align}} align-{{align}}{{/align}}">' +
			'<div class="gif">' +
			'<div class="head"><span class="head-title">{{{title}}}</span><span class="head-subtitle"></span></div></div>' +
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}}">' +
							'<div class="menu-item-selection {{#selected}}active{{/selected}}"></div > ' +
							'<div class="menu-item-icon"><i class="{{icon}}" ></i></div>' +
							'<div class="menu-item-text">{{{label}}}{{#isSlider}} : &lt;{{{sliderLabel}}}&gt;{{/isSlider}}</div>' +
						'</div>' +
					'{{/elements}}' +
				'</div>'+
			'</div>' +
	...
	ESX_MENU.open = function(namespace, name, data) {
		...
		for (let i=0; i<data.elements.length; i++) {
			if (typeof data.elements[i].type == 'undefined') {
				...
				if (typeof data.elements[i].icon == 'undefined') {
						data.elements[i].icon = "fas fa-angle-right"
					}
			}
		}
