...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_lscustomv2:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_lscustomv2:installMod')
AddEventHandler('esx_lscustomv2:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustomv2:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustomv2:cancelInstallMod')
AddEventHandler('esx_lscustomv2:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_lscustomv2:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_lscustomv2:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_lscustomv2:cancelInstallMod')
...
if (PlayerData.job ~= nil and PlayerData.job.name == 'mechanic' and PlayerData.job.grade_name == 'experimente') or Config.IsMechanicJobOnly == false then
