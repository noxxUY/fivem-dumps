...
RegisterNetEvent('m7_mafias:openAdminMenu')
AddEventHandler('m7_mafias:openAdminMenu', function(gangsData)
	openAdminMenu(gangsData)
end)

RegisterNetEvent('m7_mafias:openGradeMenu')
AddEventHandler('m7_mafias:openGradeMenu', function(gangData , grades , playerId , PlayerName )
			...
			RageUI.IsVisible(gradeMenu, function()
				for _key, grade in pairs(grades) do
					RageUI.Button(grade.name, '', {}, true, {
						onSelected = function()
							whileGo = false
							TriggerServerEvent("m7_mafias:addPlayerToGang", gangData.id , playerId , grade.grade  )
						end,
					});
				end
			end, function() end)
					...
					RageUI.Button( _U('admin_add_new_gang'), '' , {}, true, {
						...
						onSelected = function()
							...
							if ( name ~= nil ) then
								...
								if name ~= ''  then 
									whileGo = false
									TriggerServerEvent("m7_mafias:creatNewGang", name )
								end
							end
						end,
					});
					...
					onSelected = function()
						...
						if ( name ~= nil ) then
							...
							if name ~= ''  then 
								...
								TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'gangName' , name )
								Citizen.Wait(50)
					...
					onSelected = function()
						...
						if ( accountMoney ~= nil ) then
							...
							if accountMoney ~= '' and accountMoney ~= nil and accountMoney >= 0  then 
								whileGoSubMenu = false
								TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'accountMoney' , accountMoney )
								Citizen.Wait(50)
								openGangDataSubMenu(gang.id,_key )
							end
						end
					...
					onSelected = function()
						...
						if ( expireTime ~= nil ) then
							...
							if expireTime ~= '' and expireTime ~= nil and expireTime >= 0  then 
								whileGoSubMenu = false
								TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'expireTime' , expireTime )
								Citizen.Wait(50)
								openGangDataSubMenu(gang.id,_key )
							end
						end
						...
						while true do
							...
							if ( playerID ~= nil ) then
								...
								if playerID ~= '' and playerID ~= nil and playerID >= 0 and GetPlayerFromServerId(playerID) ~= -1 then 
									whileGoSubMenu = false
									TriggerServerEvent("m7_mafias:getRankForPlayer", gang.id , playerID )
									break
								else
									TriggerEvent('m7_mafias:sendNotfication','[Mafia]', ''  , _U('inCorrectId') , 'CHAR_BLOCKED' , 2 )
								end
							end
						end
					end,
			...
			RageUI.IsVisible(subMenuOfGang[_key].blips, function()
				RageUI.Button(_U('set_blips_location') , _U('edit_blips_location'), {}, true, {
					onSelected = function()
						...
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'blip' , coords  )
					end,
				});
					...
					onSelected = function()
						...
						if ( blipRadius ~= nil ) then
							...
							if blipRadius ~= '' and blipRadius ~= nil and blipRadius >= 0  then 
								...
								TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'blipRadius' , blipRadius )
								Citizen.Wait(50)
				...
				RageUI.Checkbox(_U('enable_gang_area' ), '', gang.gangColor , {}, {
					onChecked = function()
						whileGoSubMenuBlips = false
						TriggerServerEvent("m7_mafias:updateGangData",  gang.id , 'gangColor' , '1' )
						Citizen.Wait(50)
						openGangBlipsSubMenu(gang.id,_key )
					end,
					onUnChecked = function()
						whileGoSubMenuBlips = false
						TriggerServerEvent("m7_mafias:updateGangData",  gang.id , 'gangColor' , '0' )
						Citizen.Wait(50)
						openGangBlipsSubMenu(gang.id,_key )
					end,
		...
		while whileGoSubMenuSetting do
			...
			RageUI.IsVisible(subMenuOfGang[_key].setting, function()
					...
					onSelected = function()
						...
						if ( slotPlayer ~= nil ) then
							...
							if slotPlayer ~= '' and slotPlayer ~= nil and slotPlayer >= 0  then 
								...
								TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'slotPlayer' , slotPlayer )
								Citizen.Wait(50)
					...
					onSelected = function()
						...
						if ( maxArmor ~= nil ) then
							...
							if maxArmor ~= '' and maxArmor ~= nil and maxArmor >= 0  then 
								whileGoSubMenuSetting = false
								TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'maxArmor' , maxArmor )
								Citizen.Wait(50)
								openGangsSetttingSubMenu(gang.id,_key )
							end
						end
				...
				RageUI.Checkbox(_U('enable_search_player' ), _U('enable_search_player_info' ),  (gang.canSearch == 1 and  true or  false  ), {}, {
					onChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canSearch' , '1' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
					onUnChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canSearch' , '0' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
				});
				RageUI.Checkbox(_U('enable_cuff_player' ), _U('enable_cuff_player_info' ),  (gang.canCuff == 1 and  true or  false  ), {}, {
					onChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canCuff' , '1' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
					onUnChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canCuff' , '0' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
				});
				RageUI.Checkbox(_U('enable_move_player' ), _U('enable_move_player_info' ),  (gang.canMove == 1 and  true or  false  ), {}, {
					onChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canMove' , '1' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
					onUnChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canMove' , '0' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
				});
				RageUI.Checkbox(_U('enable_open_car' ), _U('enable_open_car_info' ),  (gang.canOpenCarsDoor == 1 and  true or  false  ), {}, {
					onChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canOpenCarsDoor' , '1' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
					onUnChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'canOpenCarsDoor' , '0' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
				});
				RageUI.Checkbox(_U('enable_GPS' ), _U('enable_GPS_info' ),  (gang.haveGPS == 1 and  true or  false  ), {}, {
					onChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'haveGPS' , '1' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
					onUnChecked = function()
						whileGoSubMenuSetting = false
						TriggerServerEvent("m7_mafias:updateGangData", gang.id , 'haveGPS' , '0' )
						Citizen.Wait(50)
						openGangsSetttingSubMenu(gang.id,_key )
					end,
				});
		...
		while whileGoSubMenucoords do
			...
			RageUI.IsVisible(subMenuOfGang[_key].coords, function()
				RageUI.Button(_U('set_boss_action') , _U('edit_boss_action'), {}, true, {
					onSelected = function()
						local coords = GetEntityCoords(PlayerPedId(), true) 
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'boss' , coords  )
					end,
				});
				RageUI.Button(_U('set_locker') , _U('edit_locker'), {}, true, {
					onSelected = function()
						local coords = GetEntityCoords(PlayerPedId(), true) 
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'locker' , coords  )
					end,
				});
				RageUI.Button(_U('set_armory') , _U('edit_armory'), {}, true, {
					onSelected = function()
						local coords = GetEntityCoords(PlayerPedId(), true) 
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'armory' , coords  )
					end,
				});
				RageUI.Button(_U('set_vehicle') , _U('edit_vehicle'), {}, true, {
					onSelected = function()
						local coords = GetEntityCoords(PlayerPedId(), true) 
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'vehicle' , coords  )
					end,
				});
				RageUI.Button(_U('set_helicopter') , _U('edit_helicopter'), {}, true, {
					onSelected = function()
						local coords = GetEntityCoords(PlayerPedId(), true) 
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'helicopter' , coords  )
					end,
				});
				RageUI.Button(_U('set_boat') , _U('edit_boat'), {}, true, {
					onSelected = function()
						local coords = GetEntityCoords(PlayerPedId(), true) 
						TriggerServerEvent("m7_mafias:updateGangcoords", gang.id , 'boat' , coords  )
					end,
				});
			end, function() end)
		end
		...
		while whileGoDeleteMenu do
			...
			RageUI.IsVisible(subMenuOfGang[_key].Delete, function()
				...
				RageUI.Button(_U('delete_gang'), _U('delete_gang'), { Color = { HightLightColor = { 255, 0, 0, 150 }, BackgroundColor = { 150, 0, 0, 160 } }}, true, {
					onSelected = function() 
						whileGoDeleteMenu = false
						TriggerServerEvent("m7_mafias:deleteGang", gang.id )
