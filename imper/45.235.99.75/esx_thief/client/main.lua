...
function IsAbleToSteal(targetSID, err)
	ESX.TriggerServerCallback('esx_thieflelele:getValue', function(result)
		local result = result
		if result.value then
			err(false)
		else
			exports['mythic_notify']:SendAlert('inform', 'No tiene las manos levantadas', 2500, { ['background-color'] = '#FF2D00', ['color'] = '#FFFFFF' })
...
function OpenStealMenu(target, target_id)
	...
	ESX.TriggerServerCallback('esx_thieflelele:getOtherPlayerData', function(data)
		local elements = {}
					...
					if data2.current.action == 'steal' then
						...
						if itemType == 'item_standard' then
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'steal_inventory_item_standard', {
								...
								TriggerServerEvent('esx_thieflelele:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, quantity)
								ExecuteCommand("me Le roba los objetos")
							...
							TriggerServerEvent('esx_thieflelele:stealPlayerItem', GetPlayerServerId(target), itemType, itemName, amount)
