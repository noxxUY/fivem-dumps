...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", 240)
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] fue priorizado y colocado %d/%d en la cola", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        if tmp.priority then
            -- Queue:DebugPrint(string_format("%s[%s] Posee prioridad %s, fue puesto %d/%d en la cola", tmp.name, ids[1], inicialMayuscula(_Queue.ListaVip[string.sub(ids[2], 9)]), _pos, queueCount))
        else
            -- Queue:DebugPrint(string_format("%s[%s] No posee prioridad, fue puesto %d/%d en la cola", tmp.name, ids[1], _pos, queueCount))
        end
    end
...
function inicialMayuscula(palabraRecibida)
    return palabraRecibida:sub(1,1):upper()..palabraRecibida:sub(2)
end
...
function Queue:LoadVips(listaVip)
    _Queue.Priority = {}
    _Queue.ListaVip = {}
    if listaVip == nil then
        Citizen.Wait(5000)
        for a, b in pairs(exports.gcm_vips:obtenerListaVip()) do
            _Queue.Priority[b.steam] = b.priority
            _Queue.ListaVip[b.identifier] = b.type
        end
    else
        for a, b in pairs(listaVip) do
            _Queue.Priority[b.steam] = b.priority
            _Queue.ListaVip[b.identifier] = b.type
        end
    end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Se te bloqueó para unirte")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] se le impidió unirse, Razón: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] se ha unido nuevamente a la cola después de cancelar", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Fantasy] Removido de la cola, data invalida :(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] fue removido de la cola porque posee datos inválidos"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Fantasy] Se agotó el tiempo de espera")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] fue removido de la cola porque se agotó el tiempo de espera")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] fue removido de la cola de conexión porque se agotó el tiempo de espera")
            else
                i = i + 1
            end
        end
...
RegisterServerEvent("Queue:LoadVips")
AddEventHandler("Queue:LoadVips", function(listaVip)
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [connectqueue] Deshabilitando hardcap ^7")
