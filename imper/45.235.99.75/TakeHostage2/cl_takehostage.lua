...
RegisterNetEvent('takeh:openM')
AddEventHandler('takeh:openM', function()
	takeHostage()
end)
...
function takeHostage()
	...
	if not holdingHostageInProgress and canTakeHostage then
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			holdingHostage = true
			ESX.TriggerServerCallback('cmg3_animationspapa:sync', function(cb) end, closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, 'take')
		else
...
RegisterNetEvent('cmg3_animationspapa:syncTarget')
AddEventHandler('cmg3_animationspapa:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('cmg3_animationspapa:syncMe')
AddEventHandler('cmg3_animationspapa:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('cmg3_animationspapa:cl_stop')
AddEventHandler('cmg3_animationspapa:cl_stop', function()
	holdingHostageInProgress = false
	beingHeldHostage = false
	...
	while true do
		if holdingHostage then
			if IsEntityDead(GetPlayerPed(-1)) then
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("cmg3_animationspapa:stop",target)
				Wait(100)
				releaseHostage()
			end
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z," [G] Para soltarlo, [H] Para matarlo")
...
function releaseHostage()
	...
	if closestPlayer ~= 0 then
		ESX.TriggerServerCallback('cmg3_animationspapa:sync', function(cb) end, closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, 'release')
...
function killHostage()
	...
	if target ~= 0 then
		-- LOG KILL
		ESX.TriggerServerCallback('cmg3_animationspapa:sync', function(cb) end, closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag, 'kill')
	end
end
