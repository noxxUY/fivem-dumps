...
function StockVehicleMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_aircraftgarage:stockv',function(valid)

			if (valid) then
				TriggerServerEvent('eden_aircraftgarage:debug', vehicle)
...
function ReturnVehicleMenu()
	...
	ESX.TriggerServerCallback('eden_aircraftgarage:getOutVehicles', function(vehicles)
		...
		function(data, menu)
			...
			ESX.TriggerServerCallback('eden_aircraftgarage:checkMoney', function(hasEnoughMoney)
				if hasEnoughMoney then
					
					if times == 0 then
						TriggerServerEvent('eden_aircraftgarage:pay')
						SpawnPoundedVehicle(data.current.value, data.current.value.plate)
						times=times+1
					elseif times > 0 then
						ESX.SetTimeout(600000, function()
						times=0
						end)
					end
				else
