Locales['fr'] = {
  -- Inventory
  ['cash'] = 'Dinero',
  ['ca_coins'] = 'CA Coins',
  ['inventory'] = 'Inventario',
  ['use'] = 'Usar',
  ['give'] = 'Dar',
  ['remove'] = 'Tirar',
  ['return'] = 'Volver',
  ['give_to'] = 'Dar',
  ['amount'] = 'Cantidad',
  ['giveammo'] = 'Dar Municion',
  ['amountammo'] = 'Cantidad de municion',
  ['noammo'] = 'No tienes esa cantidad de munición',
  ['gave_item'] = 'Le ha dado ~y~%sx~s~ ~b~%s~s~ a ~y~%s~s~',
  ['received_item'] = 'Has recibido ~y~%sx~s~ ~b~%s~s~ por ~b~%s~s~',
  ['gave_weapon'] = 'Has dado ~y~1x~s~ ~b~%s~s~ a ~y~%s~s~.',
  ['gave_weapon_ammo'] = 'Has dado ~y~1x~s~ ~b~%s~s~ con ~o~%sx~s~ balas a ~y~%s~s~.',
  ['gave_weapon_hasalready'] = '~y~%s~s~ ya tiene ~y~%s~s~',
  ['received_weapon'] = 'Has recibido ~y~1x~s~ ~b~%s~s~ de ~b~%s~s~.',
  ['received_weapon_ammo'] = 'Has recibido ~y~1x~s~ ~b~%s~s~ con ~o~%sx~s~ balas de ~b~%s~s~.',
  ['received_weapon_hasalready'] = '~b~%s~s~ ha tratado de darte~y~%s~s~, pero ya tenias una.',
  ['gave_ammo'] = 'Ha dado ~o~%sx~s~ municion a ~y~%s~s~.',
  ['received_ammo'] = 'Has recibido ~o~%sx~s~ municion de ~b~%s~s~.',
  ['gave_money'] = 'Le has dado ~g~$ ARS %s~s~ a ~y~%s~s~',
  ['received_money'] = 'Has recibido ~g~$ ARS %s~s~ por ~b~%s~s~',
  ['gave_account_money'] = 'Has dado ~g~$ ARS %s~s~ (%s) a ~y~%s~s~',
  ['received_account_money'] = 'Has recibido ~g~$ ARS %s~s~ (%s) por ~b~%s~s~',
  ['amount_invalid'] = 'Cantidad invalida',
  ['players_nearby'] = 'No hay jugadores cercanos',
  ['ex_inv_lim'] = 'Acción imposible, superando el limite de inventario para ~y~%s~s~',
  ['imp_invalid_quantity'] = 'Accion imposible, ~r~cantidad invalida',
  ['imp_invalid_amount'] = 'Accion imposible, ~r~cantidad invalida',
  ['threw_standard'] = 'Has tirado ~y~%sx~s~ ~b~%s~s~',
  ['threw_money'] = 'Has tirado ~r~$ ARS %s~s~ ~b~cash~s~',
  ['threw_account'] = 'Has tirado ~r~$ ARS %s~s~ ~b~%s~s~',
  ['threw_weapon'] = 'Has tirado ~y~1x~s~ ~b~%s~s~',
  ['threw_weapon_ammo'] = 'Has tirado ~y~1x~s~ ~b~%s~s~ con ~o~%sx~s~ balas',
  -- Salary related
  ['received_salary'] = 'Tu Trabajo Te a Transferido: ~g~$ ARS %s~s~ ',
  ['received_help'] = 'Has recibido ayuda del estado: ~g~$ ARS %s~s~',
  ['company_nomoney'] = 'Tu empresa no tiene mas dinero para pagarte',
  ['received_paycheck'] = 'Pago recibido',
  ['bank'] = 'Banco',
  ['black_money'] = 'Dinero negro',

  ['act_imp'] = 'Accion imposible',
  ['in_vehicle'] = 'No se puede dar nada a alguien en un vehiculo',
  ['cannot_pickup_room'] = 'No tienes mas espacio para ~y~%s~s~!',

  -- Commands
  ['setjob'] = 'Asignar trabajo',
  ['id_param'] = 'Identificacion del jugador',
  ['setjob_param2'] = 'Trabajo para asignar',
  ['setjob_param3'] = 'El nivel del empleado',
  ['load_ipl'] = 'cargar IPL',
  ['unload_ipl'] = 'descargar IPL',
  ['play_anim'] = 'Hacer animacion',
  ['play_emote'] = 'Hacer emoticono',
  ['spawn_car'] = 'Spawnear un vehiculo',
  ['spawn_car_param'] = 'Nombre del vehiculo',
  ['delete_vehicle'] = 'Borrar el vehiculo',
  ['spawn_object'] = 'Generar objeto',
  ['spawn_ped'] = 'Generar ped',
  ['spawn_ped_param'] = 'example a_m_m_hillbilly_01',
  ['givemoney'] = 'Dar dinero',
  ['setmoney'] = 'Establecer dinero por jugador',
  ['money_type'] = 'Tipos de moneda validos: efectivo, banco, negro',
  ['money_set'] = 'quelqu\'un de ~y~haut placé~s~ vous a attribué ~g~$ ARS%s~s~ (%s) !',
  ['money_amount'] = 'cantidad de dinero',
  ['invalid_account'] = 'cuenta invalida',
  ['account'] = 'cuenta',
  ['giveaccountmoney'] = 'dar dinero a la cuenta',
  ['invalid_item'] = 'Articulo invalido',
  ['item'] = 'articulo',
  ['giveitem'] = 'dar un articulo',
  ['weapon'] = 'Arma',
  ['giveweapon'] = 'Dar el arma',
  ['disconnect'] = 'Desconectado del servidor',
  ['chat_clear'] = 'Vaciar el chat',
  ['chat_clear_all'] = 'vider le chat pour tous le monde',
  ['command_clearinventory'] = 'Elimina todos los articulos del inventario',
  ['command_clearloadout'] = 'Remover todas las armas del equipamento',
  ['command_playerid_param'] = 'Especificar ID o dejar en blanco si es para ti',
  -- Locale settings
  ['locale_digit_grouping_symbol'] = ',',
  ['locale_currency'] = '$%s',

  -- Weapons
  ['weapon_pistol_mk2'] = 'Pistola MK2',
   ['weapon_assaultrifle_mk2'] = 'Ak47 MK2',
 ['weapon_smg_mk2'] = 'Smg MK2',
  ['weapon_knife'] = 'Cuchillo',
  ['weapon_nightstick'] = 'Porra',
  ['weapon_hammer'] = 'Martillo',
  ['weapon_bat'] = 'Bate',
  ['weapon_golfclub'] = 'Palo de golf',
  ['weapon_crowbar'] = 'palanca',
  ['weapon_pistol'] = 'pistola',
  ['weapon_combatpistol'] = 'G17',
  ['weapon_appistol'] = 'pistola automatica',
  ['weapon_pistol50'] = 'pistola calibre 50',
  ['weapon_microsmg'] = 'micro smg',
  ['weapon_smg'] = 'MP5',
  ['weapon_assaultsmg'] = 'smg de asalto',
  ['weapon_assaultrifle'] = 'fusil de asalto',
  ['weapon_carbinerifle'] = 'M4A1',
  ['weapon_advancedrifle'] = 'fusil avanzado',
  ['weapon_mg'] = 'Ametralladora',
  ['weapon_combatmg'] = 'Ametralladora de combate',
  ['weapon_pumpshotgun'] = 'Escopeta',
  ['weapon_sawnoffshotgun'] = 'Rifle recortado',
  ['weapon_assaultshotgun'] = 'carabina de asalto',
  ['weapon_bullpupshotgun'] = 'carabina bullup',
  ['weapon_stungun'] = 'Tazer X26',
  ['weapon_sniperrifle'] = 'Francotirador',
  ['weapon_heavysniper'] = 'Francotirador pesado',
  ['weapon_grenadelauncher'] = 'Lanza granadas',
  ['weapon_rpg'] = 'Lanza-cohetes',
  ['weapon_stinger'] = 'Lanza misiles',
  ['weapon_minigun'] = 'minigun',
  ['weapon_grenade'] = 'Granada',
  ['weapon_stickybomb'] = 'Bomba adhesiva',
  ['weapon_smokegrenade'] = 'Granada de humo',
  ['weapon_bzgas'] = 'Granada gas lacrimogeno',
  ['weapon_molotov'] = 'Coctel molotov',
  ['weapon_fireextinguisher'] = 'Extintor',
  ['weapon_petrolcan'] = 'Bidon de gasolina',
  ['weapon_digiscanner'] = 'Radar',
  ['weapon_ball'] = 'Pelota',
  ['weapon_snspistol'] = 'pistola SNS',
  ['weapon_bottle'] = 'Botella',
  ['weapon_gusenberg'] = 'Ametralladora Gusenberg',
  ['weapon_specialcarbine'] = 'carabina especial',
  ['weapon_heavypistol'] = 'Arma pesada',
  ['weapon_bullpuprifle'] = 'fusil bullpup',
  ['weapon_dagger'] = 'Daga',
  ['weapon_vintagepistol'] = 'pistola vintage',
  ['weapon_firework'] = 'Fuego artificiales',
  ['weapon_musket'] = 'Mosquete',
  ['weapon_heavyshotgun'] = 'escopeta pesada',
  ['weapon_marksmanrifle'] = 'fusil de tirador',
  ['weapon_hominglauncher'] = 'lanza cazador de cabezas',
  ['weapon_proxmine'] = 'Mina de proximidad',
  ['weapon_snowball'] = 'bola de nieve',
  ['weapon_flaregun'] = 'lanza cohete de socorro',
  ['weapon_garbagebag'] = 'bolsa de basura',
  ['weapon_handcuffs'] = 'esposas',
  ['weapon_combatpdw'] = 'UMP45',
  ['weapon_marksmanpistol'] = 'pistola de tirador',
  ['weapon_knuckle'] = 'puño americano',
  ['weapon_hatchet'] = 'hacha',
  ['weapon_railgun'] = 'cañon',
  ['weapon_machete'] = 'Machete',
  ['weapon_machinepistol'] = 'TEC-9',
  ['weapon_switchblade'] = 'Cuchillo mariposa',
  ['weapon_revolver'] = 'revolver',
  ['weapon_dbshotgun'] = 'escopeta de doble cañon',
  ['weapon_compactrifle'] = 'rifle compacto',
  ['weapon_autoshotgun'] = 'Ametralladora corta',
  ['weapon_battleaxe'] = 'Hacha de combate',
  ['weapon_compactlauncher'] = 'lanzador compacto',
  ['weapon_minismg'] = 'mini smg',
  ['weapon_pipebomb'] = 'Bomba "pipe"',
  ['weapon_poolcue'] = 'taco de billar',
  ['weapon_wrench'] = 'clave',
  ['weapon_flashlight'] = 'Linterna',
  ['gadget_nightvision'] = 'vision nocturna',
  ['gadget_parachute'] = 'paracaidas',
  ['weapon_flare'] = 'Bengalas',
  ['weapon_doubleaction'] = 'Revolver (VIP)',
  ['weapon_heavysniper_mk2'] = 'Rifle (VIP)',
  ['weapon_carbinerifle_mk2'] = 'Carabina mk2',

 

  -- Weapon Components
  ['component_clip_default'] = 'Empuñadura por defecto',
  ['component_clip_extended'] = 'Empuñadura extendida',
  ['component_clip_drum'] = 'drum Magazine',
  ['component_clip_box'] = 'box Magazine',
  ['component_flashlight'] = 'Linterna',
  ['component_scope'] = 'mira',
  ['component_scope_advanced'] = 'mira avanzada',
  ['component_suppressor'] = 'supresor',
  ['component_grip'] = 'empuñadora',
  ['component_luxary_finish'] = 'acabado lujoso',
}
