...
function OpenMobilemechanicv4ActionsMenu()

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mobile_mechanicv4_actions',
    {
      title    = _U('mechanic'),
      align    = 'right',
      elements = {
    ...
    function(data, menu)
          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanicv4', _U('mechanic'), amount)
              end
            end
          end,
        function(data, menu)
      ...
      if data.current.value == 'object_spawner' then
        ...
        ESX.UI.Menu.Open(
          'default', GetCurrentResourceName(), 'mobile_mechanicv4_actions_spawn',
          {
            title    = _U('objects'),
            align    = 'right',
            elements = {
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('mechanicv4:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('mechanicv4:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
 ...
 ESX.TriggerServerCallback('mechanicv4:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('mechanicv4:putStockItems', itemName, count)
            end
...
RegisterNetEvent('mechanicv4:onHijack')
AddEventHandler('mechanicv4:onHijack', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('mechanicv4:onCarokit')
AddEventHandler('mechanicv4:onCarokit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('mechanicv4:onFixkit')
AddEventHandler('mechanicv4:onFixkit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
AddEventHandler('mechanicv4:hasEnteredMarker', function(zone)

  if zone == 'mechanicv4Actions' then
    CurrentAction     = 'mechanicv4_actions_menu'
    CurrentActionMsg  = _U('open_actions')
    CurrentActionData = {}
  end

  if zone == 'Garage' then
    CurrentAction     = 'mechanicv4_harvest_menu'
    CurrentActionMsg  = _U('harvest_menu')
    CurrentActionData = {}
  end

  if zone == 'Craft' then
    CurrentAction     = 'mechanicv4_craft_menu'
...
AddEventHandler('mechanicv4:hasExitedMarker', function(zone)

  if zone == 'Craft' then
    TriggerServerEvent('mechanicv4:stopCraft')
    TriggerServerEvent('mechanicv4:stopCraft2')
    TriggerServerEvent('mechanicv4:stopCraft3')
  end

  if zone == 'Garage' then
    TriggerServerEvent('mechanicv4:stopHarvest')
    TriggerServerEvent('mechanicv4:stopHarvest2')
    TriggerServerEvent('mechanicv4:stopHarvest3')
  end
...
AddEventHandler('mechanicv4:hasEnteredEntityZone', function(entity)

  local playerPed = GetPlayerPed(-1)

  if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv4' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('mechanicv4:hasExitedEntityZone', function(entity)
...
-- Create Blips
Citizen.CreateThread(function()
  local blip = AddBlipForCoord(Config.Zones.mechanicv4Actions.Pos.x, Config.Zones.mechanicv4Actions.Pos.y, Config.Zones.mechanicv4Actions.Pos.z)
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv4' then
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv4' then
      ...
      if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
        ...
        TriggerEvent('mechanicv4:hasEnteredMarker', currentZone)
      end
      if not isInMarker and HasAlreadyEnteredMarker then
        HasAlreadyEnteredMarker = false
        TriggerEvent('mechanicv4:hasExitedMarker', LastZone)
      end
    end
  end
end)
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('mechanicv4:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('mechanicv4:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv4' then

            if CurrentAction == 'mechanicv4_actions_menu' then
                Openmechanicv4ActionsMenu()
            end

            if CurrentAction == 'mechanicv4_harvest_menu' then
                Openmechanicv4HarvestMenu()
            end

            if CurrentAction == 'mechanicv4_craft_menu' then
                Openmechanicv4CraftMenu()
            end
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('esx_society:putVehicleILovePizzaInGarage', 'mechanicv4', vehicleProps)

              else

                if
                  GetEntityModel(vehicle) == GetHashKey('terror2')   or
                  GetEntityModel(vehicle) == GetHashKey('slamvan3')
                then
                  TriggerServerEvent('esx_service:disableService', 'mechanicv4')
                end
        ...
        if IsControlJustReleased(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv4' then
            OpenMobilemechanicv4ActionsMenu()
        end
    end
end)
...
function Openmechanicv4ActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mechanicv4_actions',
    {
      title    = _U('mechanic'),
      align    = 'right',
      elements = elements
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
              ...
              ESX.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mechanicv4', vehicleProps)
            ...
            local elements = {
              {label = ('Camioneta Razer'), value = 'terror2'},
              {label = ('Grua Razer'), value = 'sadlerrt'}
            }
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'mechanicv4', function(data, menu)
          menu.close()
        end)
      end
...
function Openmechanicv4HarvestMenu()
  ...
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'mechanicv4_harvest',
      {
        title    = _U('harvest'),
        align    = 'right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'gaz_bottle' then
          menu.close()
          TriggerServerEvent('mechanicv4:startHarvest')
        end

        if data.current.value == 'fix_tool' then
          menu.close()
          TriggerServerEvent('mechanicv4:startHarvest2')
        end

        if data.current.value == 'caro_tool' then
          menu.close()
          TriggerServerEvent('mechanicv4:startHarvest3')
        end
...
function Openmechanicv4CraftMenu()
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'mechanicv4_craft',
      {
        title    = _U('craft'),
        align    = 'right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'blow_pipe' then
          menu.close()
          TriggerServerEvent('mechanicv4:startCraft')
        end

        if data.current.value == 'fix_kit' then
          menu.close()
          TriggerServerEvent('mechanicv4:startCraft2')
        end

        if data.current.value == 'caro_kit' then
          menu.close()
          TriggerServerEvent('mechanicv4:startCraft3')
        end
