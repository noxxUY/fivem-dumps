...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback('esx_weaponshop3:getShop', function(shopItems)
...
RegisterNetEvent('esx_weaponshop3:sendShop')
AddEventHandler('esx_weaponshop3:sendShop', function(shopItems)
    for k,v in pairs(shopItems) do
        Config.Zones[k].Items = v
    end
end)
...
function OpenBuyLicenseMenu(zone)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_weaponshop3:buyLicense', function(bought)
				if bought then
					menu.close()
					OpenShopMenu(zone)
				end
...
AddEventHandler('esx_weaponshop3:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_weaponshop3:hasExitedMarker', function(zone)
    CurrentAction = nil
    ESX.UI.Menu.CloseAll()
end)
        ...
        for k,v in pairs(Config.Zones) do
            for i = 1, #v.Locations, 1 do
                if (Config.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Locations[i], true) < Config.DrawDistance) then
                    ...
                    DrawText3Ds(v.Locations[i].x, v.Locations[i].y, v.Locations[i].z + 1.0, '~g~E~w~ - Para Comprar armas')
                end
            end
        end
    end
...
Citizen.CreateThread(function()
    while true do
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerEvent('esx_weaponshop3:hasEnteredMarker', currentZone)
        end
        
        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('esx_weaponshop3:hasExitedMarker', LastZone)
        end
    end
end)
            ...
            if IsControlJustReleased(0, 38) then
                if CurrentAction == 'shop_menu' then
                    if Config.LicenseEnable and Config.Zones[CurrentActionData.zone].Legal then
                        ...
                        end, GetPlayerServerId(PlayerId()), 'armeria')
                    else
                        OpenShopMenu(CurrentActionData.zone)
                    end
                end
...
RegisterNUICallback('buyItem', function(data, cb)
    ESX.TriggerServerCallback('esx_weaponshop3:buyWeapon', function(success)
        if success then
