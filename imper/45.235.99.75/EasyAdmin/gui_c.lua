		...
		if (RedM and IsControlJustReleased(0, Controls[settings.button]) ) or (not RedM and IsControlJustReleased(0, tonumber(settings.button)) and GetLastInputMethod( 0 )) then
			...
			if strings and isAdmin then
				...
				if permissions["ban.temporary"] then
					table.insert(banLength, {label = GetLocalisedText("30 Minutos"), time = 2100})
					table.insert(banLength, {label = GetLocalisedText("1 Hora"), time = 4000})
					table.insert(banLength, {label = GetLocalisedText("2 Horas"), time = 8000})
					table.insert(banLength, {label = GetLocalisedText("5 Horas"), time = 20000})
