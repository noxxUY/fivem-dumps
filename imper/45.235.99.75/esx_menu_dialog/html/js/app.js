(function () {
	...
	let MenuTpl =
		...
		'<div class="gif">' +'<div class="head"><span class="head-subtitle">{{title}}</span></div></div>' +
				'{{#isDefault}}<input type="text" placeholder="{{placeholder}}" name="value" id="inputText"/>{{/isDefault}}' +
				'{{#isBig}}<textarea name="value"/>{{/isBig}}' +
				'<button type="button" name="submit">Confirmar</button>' +
