...
function OpenMobilemechanicv5ActionsMenu()

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mobile_mechanicv5_actions',
    {
      title    = _U('mechanic'),
      align    = 'right',
      elements = {
    ...
    function(data, menu)
          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              if closestPlayer == -1 or closestDistance > 3.0 then
                ...
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mechanicv5', _U('mechanic'), amount)
              end
            end
          end,
        function(data, menu)
      ...
      if data.current.value == 'object_spawner' then
        ...
        ESX.UI.Menu.Open(
          'default', GetCurrentResourceName(), 'mobile_mechanicv5_actions_spawn',
          {
            title    = _U('objects'),
            align    = 'right',
            elements = {
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_mechanicv5job:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_mechanicv5job:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
 ...
 ESX.TriggerServerCallback('esx_mechanicv5job:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_mechanicv5job:putStockItems', itemName, count)
            end
...
RegisterNetEvent('esx_mechanicv5job:onHijack')
AddEventHandler('esx_mechanicv5job:onHijack', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mechanicv5job:onCarokit')
AddEventHandler('esx_mechanicv5job:onCarokit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mechanicv5job:onFixkit')
AddEventHandler('esx_mechanicv5job:onFixkit', function()
  local playerPed = GetPlayerPed(-1)
  local coords    = GetEntityCoords(playerPed)
...
AddEventHandler('esx_mechanicv5job:hasEnteredMarker', function(zone)

  if zone == 'mechanicv5Actions' then
    CurrentAction     = 'mechanicv5_actions_menu'
    CurrentActionMsg  = _U('open_actions')
    CurrentActionData = {}
  end

  if zone == 'Garage' then
    CurrentAction     = 'mechanicv5_harvest_menu'
    CurrentActionMsg  = _U('harvest_menu')
    CurrentActionData = {}
  end

  if zone == 'Craft' then
    CurrentAction     = 'mechanicv5_craft_menu'
...
AddEventHandler('esx_mechanicv5job:hasExitedMarker', function(zone)

  if zone == 'Craft' then
    TriggerServerEvent('esx_mechanicv5job:stopCraft')
    TriggerServerEvent('esx_mechanicv5job:stopCraft2')
    TriggerServerEvent('esx_mechanicv5job:stopCraft3')
  end

  if zone == 'Garage' then
    TriggerServerEvent('esx_mechanicv5job:stopHarvest')
    TriggerServerEvent('esx_mechanicv5job:stopHarvest2')
    TriggerServerEvent('esx_mechanicv5job:stopHarvest3')
  end
...
AddEventHandler('esx_mechanicv5job:hasEnteredEntityZone', function(entity)

  local playerPed = GetPlayerPed(-1)

  if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv5' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_mechanicv5job:hasExitedEntityZone', function(entity)
...
-- Create Blips
Citizen.CreateThread(function()
  local blip = AddBlipForCoord(Config.Zones.mechanicv5Actions.Pos.x, Config.Zones.mechanicv5Actions.Pos.y, Config.Zones.mechanicv5Actions.Pos.z)
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv5' then
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv5' then
      ...
      if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
        ...
        TriggerEvent('esx_mechanicv5job:hasEnteredMarker', currentZone)
      end
      if not isInMarker and HasAlreadyEnteredMarker then
        HasAlreadyEnteredMarker = false
        TriggerEvent('esx_mechanicv5job:hasExitedMarker', LastZone)
      end
    end
  end
end)
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_mechanicv5job:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_mechanicv5job:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv5' then

            if CurrentAction == 'mechanicv5_actions_menu' then
                Openmechanicv5ActionsMenu()
            end

            if CurrentAction == 'mechanicv5_harvest_menu' then
                Openmechanicv5HarvestMenu()
            end

            if CurrentAction == 'mechanicv5_craft_menu' then
                Openmechanicv5CraftMenu()
            end
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('esx_society:putVehicleILovePizzaInGarage', 'mechanicv5', vehicleProps)

              else

                if
                  GetEntityModel(vehicle) == GetHashKey('gruamidnight')   or
                  GetEntityModel(vehicle) == GetHashKey('slamvan3')
                then
                  TriggerServerEvent('esx_service:disableService', 'mechanicv5')
                end
        ...
        if IsControlJustReleased(0, Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'mechanicv5' then
            OpenMobilemechanicv5ActionsMenu()
        end
    end
end)
...
function Openmechanicv5ActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'mechanicv5_actions',
    {
      title    = _U('mechanic'),
      align    = 'right',
      elements = elements
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
              ...
              ESX.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mechanicv5', vehicleProps)
            ...
            local elements = {
              {label = ('Auto de meca'), value = 'gruamidnight'},
          ...
          local Male = GetHashKey("mp_m_freemode_01")
           ...
           TriggerEvent('skinchanger:getSkin', function(skin)
            if GetHashKey(GetEntityModel(PlayerPedId())) == Male then
              local clothesSkin = {
                ['tshirt_1'] = 116,  ['tshirt_2'] = 0,
                ['torso_1']  = 252, ['torso_2']  = 18,
                ['decals_1'] = 0,   ['decals_2'] = 0,
                ['arms']     = 33,   ['pants_1']  = 100,
                ['pants_2']  = 16,   ['shoes_1']  = 73,
                ['shoes_2']  = 16,   ['bags_1']  = 45,
                ['bags_2']  = 0
              }
              TriggerEvent('skinchanger:loadClothes', skin, clothesSkin)
                ...
                local clothesSkin = {
                ['tshirt_1'] = 116,  ['tshirt_2'] = 0,
                ['torso_1']  = 244, ['torso_2']  = 18,
                ['decals_1'] = 0,   ['decals_2'] = 0,
                ['arms']     = 33,   ['pants_1']  = 97,
                ['pants_2']  = 16,   ['shoes_1']  = 70,
                ['shoes_2']  = 16, ['bags_1']  = 45,
                ['bags_2']  = 0
              }
              TriggerEvent('skinchanger:loadClothes', skin, clothesSkin)
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:openBossMenu', 'mechanicv5', function(data, menu)
          menu.close()
        end)
      end
...
function Openmechanicv5HarvestMenu()
  ...
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'mechanicv5_harvest',
      {
        title    = _U('harvest'),
        align    = 'right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'gaz_bottle' then
          menu.close()
          TriggerServerEvent('esx_mechanicv5job:startHarvest')
        end

        if data.current.value == 'fix_tool' then
          menu.close()
          TriggerServerEvent('esx_mechanicv5job:startHarvest2')
        end

        if data.current.value == 'caro_tool' then
          menu.close()
          TriggerServerEvent('esx_mechanicv5job:startHarvest3')
        end
...
function Openmechanicv5CraftMenu()
  if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.grade_name ~= 'recrue' then
    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'mechanicv5_craft',
      {
        title    = _U('craft'),
        align    = 'right',
        elements = elements
      ...
      function(data, menu)
        if data.current.value == 'blow_pipe' then
          menu.close()
          TriggerServerEvent('esx_mechanicv5job:startCraft')
        end

        if data.current.value == 'fix_kit' then
          menu.close()
          TriggerServerEvent('esx_mechanicv5job:startCraft2')
        end

        if data.current.value == 'caro_kit' then
          menu.close()
          TriggerServerEvent('esx_mechanicv5job:startCraft3')
        end
