...
--Menu function
function OpenMenuGarageMafia(PointType, coords, head)
    ESX.UI.Menu.CloseAll()
    local elements = {}
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'garage_menu', {
        ...
        if (data.current.value == 'list_vehicles') then
            ListVehiclesMenuMafia(coords, head)
        end
        ...
        if coords ~= nil then
            local playerPed = GetPlayerPed(-1)
            SpawnVehicleMafia(data.current.value, coords, head)
        end
...
function ListVehiclesMenuMafia(coords, head)
    local elements = {}
    ...
    ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
            ...
            if (data.current.value.state) then
                menu.close()
                SpawnVehicleMafia(data.current.value.vehicle, coords, head)
            else
...
function ListVehiclesMenu()
    ...
    ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
        ...
        for _,v in pairs(vehicles) do
            ...
            table.insert(elements, {label = labelvehicle .. ' ['..v.plate..']', value = v})
...
function SpawnVehicleMafia(vehicle, coords, head)
    ...
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlPressed(0, 38) and (GetGameTimer() - GUI.Time) > 150 then
				...
				ExecuteCommand("me Abre el garage")
                    OpenMenuGarage('spawn')
					
                end

                if CurrentAction == 'delete' then
				ExecuteCommand("me Guarda el vehiculo")
...
RegisterNetEvent('eden_garage:spawn_vehicles_mafias')
AddEventHandler('eden_garage:spawn_vehicles_mafias', function(coords, head)
    OpenMenuGarageMafia('spawn', coords, head)
end)

RegisterNetEvent('eden_garage:delete_vehicles_mafias')
AddEventHandler('eden_garage:delete_vehicles_mafias', function()
    OpenMenuGarageMafia('delete', nil, nil)
end)
...
function SpawnVehicle(vehicle)
    ESX.Game.SpawnVehicle(vehicle.model, {
        ...
        local plate = GetVehicleNumberPlateText(callback_vehicle)
        TriggerServerEvent('ls:mainCheck', plate, callback_vehicle, true)
		ExecuteCommand("me Saca el vehiculo")
    end)
