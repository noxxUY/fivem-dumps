...
AddEventHandler('esx_holdup:tooFar', function()
	holdingUp, store = false, ''
		...
		exports['mythic_notify']:SendAlert('inform', 'El robo a sido cancelado', 2500, { ['background-color'] = '#FF2D00', ['color'] = '#FFFFFF' })
...
AddEventHandler('esx_holdup:robberyComplete', function(award)
	holdingUp, store = false, ''
	exports['mythic_notify']:SendAlert('inform', 'El robo salió bien, has robado '..award..'', 2500, { ['background-color'] = '#FF2D00', ['color'] = '#FFFFFF' })
...
AddEventHandler('esx_holdup:startTimer', function()
	...
	Citizen.CreateThread(function()
		while holdingUp do
			Citizen.Wait(0)
			drawTxt(0.66, 1.44, 1.0, 1.0, 0.4, 'Robo: ~r~'..timer..'~s~ segundos restantes', 255, 255, 255, 255)
		end
	end)
end)
...
Citizen.CreateThread(function()
	for k,v in pairs(Stores) do
		...
		elseif v.type == 'special' then
			...
			SetBlipAsShortRange(blip, true)
			BeginTextCommandSetBlipName("STRING")
			AddTextComponentString('Robo especial')
			EndTextCommandSetBlipName(blip)
		end
	end
end)
			...
			if distance < Config.Marker.DrawDistance then
				if not holdingUp then
					...
					if distance < 0.5 then
						ESX.ShowHelpNotification('Pulsa ~INPUT_CONTEXT~ para ~o~robar~s~ ~b~'..v.nameOfStore..'~s~')
...
exports['mythic_notify']:SendAlert('inform', 'Sin pistola el encargado no te tiene miedo...', 2500, { ['background-color'] = '#FF2D00', ['color'] = '#FFFFFF' })
