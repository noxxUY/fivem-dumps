...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		...
		notowned = 0
		if #cars == 0 then
			exports['mythic_notify']:SendAlert('inform', 'No hay vehículos para bloquear cerca', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							exports['mythic_notify']:SendAlert('inform', 'Has bloqueado tu vehiculo', 2500, { ['background-color'] = '#FF0000', ['color'] = '#000000' }) 
                            ExecuteCommand("me Bloquea el vehiculo")
						...
						elseif lock == 2 then
							...
							exports['mythic_notify']:SendAlert('inform', 'Tienes desbloqueado tu vehiculo', 2500, { ['background-color'] = '#AEE600', ['color'] = '#000000' })
                            ExecuteCommand("me Desbloquea el vehiculo")
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
