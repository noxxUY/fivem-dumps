...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		align    = 'right',
		elements = {
			{label = ('Menu de same'), value = 'citizen_interaction'}
		}
	}, function(data, menu)
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = ('♥️Iniciar RCP♥️'), value = 'revive'},
					{label = ('👋🏻Colocar Vendaje👋🏻'), value = 'big'},
					{label = ('🚑Meterlo al Vehiculo🚑'), value = 'put_in_vehicle'}
				}
			}, function(data, menu)
...
-- barcos Spawners
			for k,v in ipairs(hospital.barco) do
				local distance = GetDistanceBetweenCoords(playerCoords, v.Spawner, true)
				...
				if distance < v.Marker.x then
					isInMarker, currentHospital, currentPart, currentPartNum = true, hospitalNum, 'barco', k
				end
			end
...
AddEventHandler('esx_ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' then
		...
		elseif part == 'Helicopters' then
			...
			CurrentActionData = {hospital = hospital, partNum = partNum}
			elseif part == 'barco' then
			CurrentAction = part
			CurrentActionMsg = _U('barco_prompt')
			CurrentActionData = {hospital = hospital, partNum = partNum}
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'Helicopters' then
					...
					elseif CurrentAction == 'barco' then
					OpenbarcoSpawnerMenu(CurrentActionData.hospital, CurrentActionData.partNum)
...
function OpenbarcoSpawnerMenu(hospital, partNum)
	local playerCoords = GetEntityCoords(PlayerPedId())
	ESX.PlayerData = ESX.GetPlayerData()
	local elements = {
		{label = _U('barco_garage'), action = 'garage'},
		{label = _U('barco_store'), action = 'store_garage'},
		{label = _U('barco_buy'), action = 'buy_barco'}
	}

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'barco_spawner', {
		title    = _U('barco_title'),
		align    = 'right',
		elements = elements
	}, function(data, menu)

		if data.current.action == 'buy_barco' then
			local shopCoords = Config.Hospitals[hospital].barco[partNum].InsideShop
			local shopElements = {}

			local Authorizedbarco = Config.Authorizedbarco[ESX.PlayerData.job.grade_name]

			if #Authorizedbarco > 0 then
				for k,barco in ipairs(Authorizedbarco) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(barco.label, _U('shop_item', ESX.Math.GroupDigits(barco.price))),
						name  = barco.label,
						model = barco.model,
						price = barco.price,
						type  = 'barco'
				...
				ESX.ShowNotification(_U('barco_notauthorized'))
				return
			end
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('esx_vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'barco_garage', {
						title    = _U('helicopter_garage_title'),
						align    = 'right',
						elements = garage
					}, function(data2, menu2)
						if data2.current.stored then
							local foundSpawn, spawnPoint = GetAvailableVehicleSpawnPoint(hospital, 'barco', partNum)
