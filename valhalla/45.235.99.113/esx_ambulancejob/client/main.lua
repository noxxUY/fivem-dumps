		...
		while bleedoutTimer > 0 and isDead do
			...
			if not Config.EarlyRespawnFine then
				...
				if IsControlPressed(0, 38) and timeHeld > 60 then
					RemoveItemsAfterRPDeath()
					TriggerServerEvent('bm_deathloot:playerDied', GetEntityCoords(PlayerPedId()))
					break
				end
...
function RemoveItemsAfterRPDeath()
	...
	Citizen.CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
			local respawns = {
				-- @ Ciudad
				vec4(295.61, -590.75, 43.25, 70.63),

				-- @ Sandy
				vec4(1837.53, 3671.24, 34.28, 205.15)
			}
			local closestHospital
			for i,v in pairs(respawns) do
					if (closestHospital) then
					if #(GetEntityCoords(PlayerPedId()) - v.xyz) < #(GetEntityCoords(PlayerPedId()) - closestHospital.xyz) then
						closestHospital = v
					end
				else
					closestHospital = v
			...
			RespawnPed(PlayerPedId(), formattedCoords, closestHospital.w)
			
			SetTimeout(2000, function()
				if #(GetEntityCoords(PlayerPedId()) - closestHospital.xyz) > 15 then
					RespawnPed(PlayerPedId(), formattedCoords, closestHospital.w)
				end
			end)
