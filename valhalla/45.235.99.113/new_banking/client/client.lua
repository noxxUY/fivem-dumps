...
local banks = {
	...
	{name="Banco Central", id=106, x=241.727, y=220.706, z=106.286, principal = true},
...
Citizen.CreateThread(function()
	while true do
		local msec = 1000
		if nearBank() or IsPlayerNearATM and not IsPedInAnyVehicle(PlayerPedId() , 1) then
			msec = 0
			
			local ped                         = PlayerPedId()
			local playerX , playerY , playerZ = table.unpack(GetEntityCoords(ped))
			
			CAJERO(playerX , playerY , playerZ , "¡Presiona ~g~[E]~w~ para interaccionar!" , 7 , 0.2 , 0.1)
			...
			if IsControlJustPressed(1, 38) then
				...
				TriggerEvent("mythic_progressbar:client:progress", {
					...
					animation = {
						animDict = "anim@amb@prop_human_atm@interior@male@enter",
						anim = "enter",
					},
					prop = {
						model = "",
