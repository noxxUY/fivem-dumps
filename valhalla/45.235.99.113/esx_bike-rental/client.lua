...
Citizen.CreateThread(function()
    while true do
		...
		for k in pairs(Config.MarkerZones) do
			if (GetEntityCoords(PlayerPedId()) - vec3(Config.MarkerZones[k].x, Config.MarkerZones[k].y, Config.MarkerZones[k].z)).length() < 10.0 then
				msec = 0
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
				...
				if havebike == false then
                    ...
                    ESX.ShowFloatingHelpNotification(_U('press_e'), vector3(playerX, playerY, playerZ))
				...
				elseif havebike == true then
					local playerPed = PlayerPedId()
                    local playerX, playerY, playerZ = table.unpack(GetEntityCoords(playerPed))
                    ESX.ShowFloatingHelpNotification(_U('storebike'), vector3(playerX, playerY, playerZ))
...
function OpenBikesMenu()
	...
	if data.current.value == 'bike2' then
		if Config.EnablePrice then
			TriggerServerEvent("esx:bike:lowmoney", Config.Pricefaggio2)
			TriggerEvent("chatMessage", _U('bikes'), {255,0,255}, _U('bike_pay', Config.Pricefaggio2))
		end

		if Config.EnableEffects then
			spawn_effect("faggio2")
		else
			TriggerEvent('esx:spawnVehicle', "faggio2")
		end
