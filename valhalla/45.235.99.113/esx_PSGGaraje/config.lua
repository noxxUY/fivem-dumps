...
Config.psgDatabaseName = 'psg'	-- set the exact name from your jobs database for psg

--psg Car Garage:
Config.CarZones = {
	psgCarGarage = {
		Pos = {
			{x = 1864.3,  y = 3702.1, z =  33.8},
		}
	}
}
...
-- psg Car Garage Marker Settings:
Config.psgCarMarker = 36 												-- marker type
Config.psgCarMarkerColor = { r = 255, g = 255, b = 0, a = 100 } 			-- rgba color of the marker
Config.psgCarMarkerScale = { x = 1.5, y = 1.5, z = 1.0 }  				-- the scale for the marker on the x, y and z axis
Config.CarDraw3DText = "[E] Presione para abrir el garage"			-- set your desired text here

-- psg Heli Garage Marker Settings:
Config.psgHeliMarker = 27 												-- marker type
Config.psgHeliMarkerColor = { r = 50, g = 50, b = 204, a = 100 } 		-- rgba color of the marker
Config.psgHeliMarkerScale = { x = 5.5, y = 5.5, z = 1.0 }  				-- the scale for the marker on the x, y and z axis
Config.HeliDraw3DText = "[E] Para abrir el garaje"		-- set your desired text here

-- psg Boat Garage Marker Settings:
Config.psgBoatMarker = 27 												-- marker type
Config.psgBoatMarkerColor = { r = 50, g = 50, b = 204, a = 100 } 		-- rgba color of the marker
Config.psgBoatMarkerScale = { x = 3.0, y = 3.0, z = 1.0 }  				-- the scale for the marker on the x, y and z axis
Config.BoatDraw3DText = "[E] Para abrir el garaje"		-- set your desired text here

-- psg Extra Garage Marker Settings:
Config.psgExtraMarker = 27 												-- marker type
Config.psgExtraMarkerColor = { r = 50, g = 50, b = 204, a = 100 } 		-- rgba color of the marker
Config.psgExtraMarkerScale = { x = 2.5, y = 2.5, z = 1.0 }  				-- the scale for the marker on the x, y and z axis
Config.ExtraDraw3DText = "Pulse ~ g ~ [E] ~ s ~ para ~ y ~ Menú extra de EMS ~ s ~"		-- set your desired text here

-- psg Cars:
Config.psgVehicles = {
	{ model = 'spacer', label = '── Low PSG'},
			{ model = 'prosegur1', label = '<i class="fas fa-car"></i> Patrulla #1 BMW'},
			{ model = 'prosegur2', label = '<i class="fas fa-car"></i> Patrulla #2 - Maserati'},
			{ model = 'prosegur3', label = '<i class="fas fa-car"></i> Patrulla #3 - Dodge Challenger'},
			{ model = 'prosegur4', label = '<i class="fas fa-truck"></i> Camioneta #1 - Ford F-150'},
			{ model = 'spacer', label = '── High PSG'},
			{ model = 'prosegur5', label = '<i class="fas fa-car"></i> Patrulla #1 - Chevrolet Camaro'},
			{ model = 'prosegur6', label = '<i class="fas fa-car"></i> Patrulla #2 - Audi R8'},
			{ model = 'prosegur7', label = '<i class="fas fa-truck"></i> Camioneta #1 - Ford Explorer'},
			{ model = 'prosegur8', label = '<i class="fas fa-truck"></i> Camioneta #2 - Dodge Durango'},
			{ model = 'spacer', label = '── Head PSG'},
			{ model = 'prosegur9', label = '<i class="fas fa-car"></i> Patrulla #1 - Fenyr Lykan'},
			{ model = 'spacer', label = '── Camiones'},
			{ model = 'stockade', label = '<i class="fas fa-truck"></i> Caudal'},
			{ model = 'riotpsg', label = '<i class="fas fa-truck"></i> Blindado'},
			{ model = 'spacer', label = '── Infanteria'},
			{ model = 'prosegur10', label = '<i class="fas fa-motorcycle"></i> Unidad Motorizada #1'},
			{ model = 'prosegur11', label = '<i class="fas fa-motorcycle"></i> Unidad Motorizada #2'},
			{ model = 'prosegur12', label = '<i class="fas fa-truck"></i> Camioneta #1 - Ford Ram Waggon'},
			{ model = 'spacer', label = '── Rapida Accion'},
			{ model = 'prosegur13', label = '<i class="fas fa-car"></i> Rapida Accion #1 - Lamborghini Centenario'},
			{ model = 'prosegur14', label = '<i class="fas fa-car"></i> Rapida Accion #2 - Lamborghini Aventador'},
			{ model = 'spacer', label = '── Brigada Delta'},
			{ model = 'prosegur15', label = '<i class="fas fa-truck"></i> Camioneta #1 - Chevrolet Tahoe'},
			{ model = 'prosegur16', label = '<i class="fas fa-truck"></i> Camioneta #2 - Hummer'},
		
}

-- psg Helicopters:
Config.psgHelicopters = {
	{ model = 'polmav', label = 'Prosegur helicoptero', livery = 0, price = 0 }
}

-- psg Boats:
Config.psgBoats = {
	{ model = 'predator', label = 'psg Predator', livery = 0, price = 0 }
}
...
-- Menu Labels & Titles:
Config.LabelStoreVeh = "Guardar el vehiculo"
Config.LabelGetVeh = "Toma un vehículo"
Config.LabelPrimaryCol = "Primary"
Config.LabelSecondaryCol = "Secondary"
Config.LabelExtra = "Extra"
Config.LabelLivery = "Livery"
Config.TitlepsgGarage = "Garaje de Prosegur"
Config.TitlepsgExtra = "Extra"
Config.TitlepsgLivery = "Livery"
Config.TitleColorType = "Color Type"
Config.TitleValues = "Value"

-- ESX.ShowNotifications:
Config.VehicleParked = "¡Su vehículo está Guardado!"
Config.NoVehicleNearby = "¡No tienes vehículo!"
Config.CarOutFromPolGar ="¡Sacaste un Vehículo de Garage de Prosegur!"
Config.HeliOutFromPolGar = "¡Sacaste un helicóptero de Prosegur Helicoptero Garage!"
Config.BoatOutFromPolGar = "¡Sacaste un Barco de Garage de barcos de Prosegur!"
Config.VehRepNotify ="¡Su vehículo está siendo reparado, por favor espere!"
Config.VehRepDoneNotify ="¡Su vehículo ha sido reparado!"
Config.VehCleanNotify ="¡Su vehículo está siendo limpiado, por favor espere!"
Config.VehCleanDoneNotify ="¡Su vehículo ha sido limpiado!"
...
Config.VehicleLoadText = "Espera..."			-- text on screen when vehicle model is being loaded
