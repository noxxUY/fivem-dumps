...
function ToggleSeatbelt(state)
    ...
    if vehclass ~= 8 and vehclass ~= 13 then
        if IsPedInAnyVehicle(ped, true) then
            if state then
                QBCore.Functions.Notify('Te has puesto el cinturon', 'success', 5000)
            else
                QBCore.Functions.Notify('Te has quitado el cinturon', 'success', 5000)
            end
        end
    else
        QBCore.Functions.Notify('No estas en un vehiculo para ponerte el cinturon', 'error', 5000)
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsPedInAnyVehicle(PlayerPedId()) then
            Wait(2000)
            QBCore.Functions.Notify('Escriba /carhud para editar el hud del vehiculo', 'primary', 10000)
            Wait(5000)
            QBCore.Functions.Notify('Ponte el cinturon con la K', 'primary', 5000)
            break
        end
    end
end)
