...
RegisterNetEvent('pfa_panico:client:PanicUrgente')
AddEventHandler('pfa_panico:client:PanicUrgente', function(player, s1, s2)
	if ESX.PlayerData.job.name == Config.PoliceName then
...
RegisterNetEvent('pfa_panico:client:BlipUrgente')
AddEventHandler('pfa_panico:client:BlipUrgente', function(tx, ty, tz)
	if ESX.PlayerData.job.name == Config.PoliceName then
	local transT = 250
...
RegisterNetEvent('pfa_panico:client:PanicImportante')
AddEventHandler('pfa_panico:client:PanicImportante', function(player, s1, s2)
	if ESX.PlayerData.job.name == Config.PoliceName then
			local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(player)))
...
RegisterNetEvent('pfa_panico:client:BlipImportante')
AddEventHandler('pfa_panico:client:BlipImportante', function(tx, ty, tz)
	if ESX.PlayerData.job.name == Config.PoliceName then
	local transT = 250
	local Blip = AddBlipForCoord(tx, ty, tz)
	SetBlipSprite(Blip,  161)
...
RegisterNetEvent('pfa_panico:client:PanicLeve')
AddEventHandler('pfa_panico:client:PanicLeve', function(player, s1, s2)
	if ESX.PlayerData.job.name == Config.PoliceName then
			local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(player)))
...
RegisterNetEvent('pfa_panico:client:BlipLeve')
AddEventHandler('pfa_panico:client:BlipLeve', function(tx, ty, tz)
	if ESX.PlayerData.job.name == Config.PoliceName then
	local transT = 250
	local Blip = AddBlipForCoord(tx, ty, tz)
	SetBlipSprite(Blip,  161)
...
---------------------------------------------------------------------------------
 ...
 RegisterNetEvent('pfa_panico:client:EntroServ')
 AddEventHandler('pfa_panico:client:EntroServ', function(player)
 	if ESX.PlayerData.job.name == Config.PoliceName then
...
-- AddEventHandler('pfa_panico:client:EntroServ', function(player)
-- 	if ESX.PlayerData.job.name == Config.PoliceName then
-- 		ESX.TriggerServerCallback('pfa_panico:obtengoNombre', function(data)
-- 			local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(GetPlayerFromServerId(player)))
-- 			ESX.ShowAdvancedNotification('NOTIFICACIÓN P.F.A', '~g~Entrada en servicio', 'El oficial ~b~'..GetPlayerName(GetPlayerFromServerId(player))..'~w~ entró en servicio.', mugshotStr, 4)
-- 			UnregisterPedheadshot(mugshot)
-- 			PlaySound(-1, "Kill_List_Counter", "GTAO_FM_Events_Soundset", 0, 0, 1)
...
RegisterNetEvent('pfa_panico:client:SalgoServ')
AddEventHandler('pfa_panico:client:SalgoServ', function(player)
	if ESX.PlayerData.job.name == Config.PoliceName then
		--ESX.TriggerServerCallback('pfa_panico:obtengoNombre', function(data)
			TriggerEvent('skinchanger:getSkin', function(skin)
				if skin.sex == 0 then
