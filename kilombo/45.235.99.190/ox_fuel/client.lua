...
local function startFueling(vehicle, isPump)
	...
	if 100 - fuel < Config.refillValue then
		return exports['okokNotify']:Alert("SERVIDOR", locale('tank_full'), 5500, 'error')
	end
	...
	if isPump then
		...
		if Config.priceTick > moneyAmount then
			return exports['okokNotify']:Alert("SERVIDOR", locale('not_enough_money', Config.priceTick), 5500, 'error')
		end
	elseif Config.durabilityTick > fuelingCan.metadata.ammo then
		return exports['okokNotify']:Alert("SERVIDOR", locale('petrolcan_not_enough_fuel'), 5500, 'error')
	end
	...
	RegisterCommand('startfueling', function()
		...
		if nearestPump then
			...
			if not vehicleInRange then
				...
				return exports['okokNotify']:Alert("SERVIDOR", locale('petrolcan_cannot_afford'), 5500, 'error')
			elseif moneyAmount >= Config.priceTick then
				return startFueling(lastVehicle, true)
			else
				return exports['okokNotify']:Alert("SERVIDOR", locale('refuel_cannot_afford'), 5500, 'error')
			end

			return exports['okokNotify']:Alert("SERVIDOR", locale('vehicle_far'), 5500, 'error')
			...
			options = {
				{
					action = function (entity)
						if getMoneyAmount() >= Config.priceTick then
							...
							exports['okokNotify']:Alert("SERVIDOR", locale('refuel_cannot_afford'), 5500, 'error')
						end
					end,
				...
				},
				{
					action = function (entity)
						...
						if moneyAmount < Config.petrolCan.price then
							exports['okokNotify']:Alert("SERVIDOR", locale('petrolcan_cannot_afford'), 5500, 'error')
						end
					...
					action = function (entity)
						if getMoneyAmount() >= Config.priceTick then
							if GetVehicleFuelLevel(lastVehicle) >= 100 then
								exports['okokNotify']:Alert("SERVIDOR", locale('vehicle_full'), 5500, 'error')
							end
							startFueling(lastVehicle, 1)
						else
							exports['okokNotify']:Alert("SERVIDOR", locale('refuel_cannot_afford'), 5500, 'error')
			...
			options = {
				{
					action = function (entity)
						...
						if not petrolCan then return exports['okokNotify']:Alert("SERVIDOR", locale('petrolcan_not_equipped'), 5500, 'error') end
