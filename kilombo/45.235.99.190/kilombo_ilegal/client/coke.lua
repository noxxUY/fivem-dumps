...
Citizen.CreateThread(function()
	while true do
		...
		local distancia = #(coords - Config.CircleZones.CokeField.coords)
		local sleep = 500
...
Citizen.CreateThread(function()
	while true do
		...
		local distancia = #(coords - Config.CircleZones.CokeProcessing.coords)
		local sleep = 500
...
function ProcessCoke()
	...
	exports['okokNotify']:Alert("SERVIDOR", _U('coke_processingstarted'), 3500, 'info')
	TriggerServerEvent('kilombo_ilegal:processCocaLeaf')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.CokeProcessing.coords, false) > 5 then
			exports['okokNotify']:Alert("SERVIDOR", _U('coke_processingtoofar'), 3500, 'warning')
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #CocaPlants, 1 do
			local distancia = #(coords - GetEntityCoords(CocaPlants[i]))
			if distancia < 1 then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('kilombo_ilegal:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('kilombo_ilegal:pickedUpCocaLeaf')
					else
						exports['okokNotify']:Alert("SERVIDOR", _U('coke_inventoryfull'), 3500, 'error')
					end
