		...
		var eventCallback = {
			...
			updateVehicle: function(data) {
				...
				if (data.status == true) {
					...
					// var speedometerCircle = document.querySelector('.veh .num');
					...
					// document.querySelector('.veh .num').style.strokeDashoffset = data.nail;
					// saferInnerHTML(document.querySelector('#vehicle-speed span'), data.speed);
					
					var speed_class = $('.veh .num')
					var speed = Math.floor(Math.floor(data.speed*100)/data.config.maxSpeed);

					speed_class.removeClass('zero');
					speed_class.removeClass('twentyfive');
					speed_class.removeClass('fifty');
					speed_class.removeClass('seventyfive');

					if (speed >= 0 && speed <= 25) { speed_class.addClass('cero'); }
					else if (speed > 50 && speed <= 75) { speed_class.addClass('twentyfive'); }
					else if (speed > 75 && speed <= 100) { speed_class.addClass('fifty'); }
					else if (speed > 150) { speed_class.addClass('seventyfive'); }
