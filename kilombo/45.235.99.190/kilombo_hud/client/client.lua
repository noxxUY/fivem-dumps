...
local Keys = {
	...
	["."] = 81, ["LEFTCTRL"] = 36, ["LEFTALT"] = 19, ["SPACE"] = 22, ["RIGHTCTRL"] = 70, ["HOME"] = 213, ["PAGEUP"] = 10,
	["PAGEDOWN"] = 11, ["DELETE"] = 178, ["LEFT"] = 174, ["RIGHT"] = 175, ["TOP"] = 27, ["DOWN"] = 173, ["NENTER"] = 201,
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if ESX.PlayerData.job then
			...
			local bank
			local money
			local src = GetPlayerServerId(NetworkGetEntityOwner(PlayerPedId()))
			local year, month, day, hour, minute, second = GetLocalTime()
			local t = { d = day, ms = month, y = year, h = hour, m = minute }
			local fecha = string.format("%02d/%02d/%02d", t.d, t.ms, t.y)
			local hora = string.format("%02d:%02d", t.h, t.m)
			...
			SendNUIMessage({ action = 'setText', id = 'PJID', value = src })
			SendNUIMessage({ action = 'setText', id = 'fecha', value = fecha })
			SendNUIMessage({ action = 'setText', id = 'hora', value = hora })
...
ESX.RegisterInput("cinturon", "Cinturón de seguridad", "keyboard", Config.vehicle.keys.seatbelt, function()
...
ESX.RegisterInput("limitador", "Limitador de velocidad", "keyboard", Config.vehicle.keys.cruiser, function()
	local player = ESX.PlayerData.ped
	local vehicle = GetVehiclePedIsIn(player, false)
	local vehicleSpeedSource = GetEntitySpeed(vehicle)
	...
	while true do
		...
		--[[if IsPedInAnyVehicle(player, false) and GetIsVehicleEngineRunning(vehicle) then
			if IsControlJustReleased(0, Keys[Config.vehicle.keys.seatbelt]) and
				(has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~= 8) then
				seatbeltIsOn = not seatbeltIsOn
			end
		end
