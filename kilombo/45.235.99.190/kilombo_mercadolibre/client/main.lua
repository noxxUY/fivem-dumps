	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		...
		if CurrentStatus == Status.PLAYER_REMOVED_GOODS_FROM_VEHICLE then
			...
			if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, DeliveryLocation.Item2.x, DeliveryLocation.Item2.y, DeliveryLocation.Item2.z, true) < 1.5 then
				...
				ESX.TriggerServerCallback('kilombo_mercadolibre:finishDelivery:server', function(deliveryMoney)
					-- Aquí puedes hacer lo que quieras con la respuesta del servidor
					if deliveryMoney >= 1 then
						TriggerServerEvent("kilombo_mercadolibre:finishDelivery:server", CurrentType)
						TriggerServerEvent('InteractSound_SV:PlayOnSource', 'mercadolibre', 0.1)
						PlaySound(-1, "Menu_Accept", "Phone_SoundSet_Default", false, 0, true)
...
function HandleMarkers()
	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, deleter.x, deleter.y, deleter.z) < 20 then
		...
		if CurrentStatus == Status.PLAYER_STARTED_DELIVERY then
			if not IsPlayerInsideDeliveryVehicle() and CurrentVehicle ~= nil then
				...
				if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, dl.x, dl.y, dl.z, true) < 30 then
		...
		if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, bCoords.x, bCoords.y, bCoords.z, true) < 20.0 then
			...
			DrawMarker(1, VanPos.x, VanPos.y, VanPos.z, 0, 0, 0, 0, 0, 0, 2.0, 2.0, 0.8,255,255,51, 200, 0, 0, 0, 0)
...
function StartDelivery(deliveryType)
	TriggerServerEvent("kilombo_mercadolibre:removeSafeMoney:server", deliveryType)
end
...
function FinishDelivery(deliveryType, safeReturn)
	...
	TriggerServerEvent("kilombo_mercadolibre:returnSafe:server", deliveryType, safeReturn)
...
AddEventHandler("esx:playerLoaded", function()
	...
	TriggerServerEvent("kilombo_mercadolibre:getPlayerJob:server")
...
RegisterNetEvent('kilombo_mercadolibre:setPlayerJob:client')
RegisterNetEvent('kilombo_mercadolibre:startJob:client')

AddEventHandler('esx:setJob', function(job)
	PlayerJob = job.name
end)

AddEventHandler('kilombo_mercadolibre:setPlayerJob:client', function(job)
	PlayerJob = job
end)

AddEventHandler('kilombo_mercadolibre:startJob:client', function(deliveryType)
