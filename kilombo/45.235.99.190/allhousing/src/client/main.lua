...
LoadInterior = function(d)
  exports['okokNotify']:Alert("SERVIDOR", Labels["InteractDrawText"]..Labels["AccessHouseMenu"], 6500, 'info')
...
Invited = function(house)
  ...
  if Vdist(plyPos,house.Entry.xyz) < 5.0 then
    exports['okokNotify']:Alert("SERVIDOR", Labels["AcceptDrawText"]..Labels["InvitedInside"], 7500, 'info')
    BeingInvited = true
    ...
    BeingInvited = false
    exports['okokNotify']:Alert("SERVIDOR", Labels["MovedTooFar"], 3500, 'error')
  else    
    exports['okokNotify']:Alert("SERVIDOR", Labels["MovedTooFar"], 3500, 'error')
  end
end
...
KnockAtDoor = function(entry)
  if InsideHouse and InsideHouse.Entry == entry and InsideHouse.Owner and InsideHouse.Owner == GetPlayerIdentifier() then
    exports['okokNotify']:Alert("SERVIDOR", Labels["KnockAtDoor"], 7500, 'info')
  end
end
