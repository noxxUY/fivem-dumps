RegisterCommand("notif", function()
  ...
  exports['okokNotify']:Alert("SERVIDOR", Labels["AcceptDrawText"]..Labels["SetGarage"].."\nㅤㅤㅤㅤㅤ"..Labels["CancelDrawText"]..Labels["CancelGarage"], 6500, 'info')

end)


if Config.CreateHouses then
  RegisterCommand("crearcasa", function(...)
    ...
    exports['okokNotify']:Alert("SERVIDOR", Labels["AcceptDrawText"]..Labels["SetEntry"], 6500, 'info')
    ...
    exports['okokNotify']:Alert("SERVIDOR", Labels["SetSalePrice"], 6500, 'info')
    exports["input"]:Open("Precio de compra",(Config.UsingESX and Config.UsingESXMenu and "ESX" or "Native"),function(data)
          ...
          ESX.UI.Menu.Open('default', GetCurrentResourceName(), "select_int_type",{
            ...
            function(data,menu)
              ...
              if data.current.value == "interior" then
                exports['okokNotify']:Alert("SERVIDOR", Labels['AcceptDrawText']..Labels['SetInterior'], 6500, 'info')
                while true do
                  ...
                  if IsControlJustReleased(0,Config.Controls.Accept) then
                    if int and int >= 1 then
                      is_interior = int
                      exports['okokNotify']:Alert("SERVIDOR", Labels['Interior']..": "..int, 6500, 'info')
                      return
                    end
                  end
                  Wait(0)
              ...
              elseif data.current.value == "shell" then            
                exports['okokNotify']:Alert("SERVIDOR", Labels["SelectDefaultShell"], 6500, 'info')
                  local elements = {}
      ...
      local shells,doors = {},{}
      if is_shell then
        exports['okokNotify']:Alert("SERVIDOR", Labels['ToggleShells'], 6500, 'info')
        if Config.UsingESX and Config.UsingESXMenu then
          local elements = {}
          for k,v in pairs(ShellModels) do
            table.insert(elements,{label = k})
          ...
          ESX.UI.Menu.Open('default', GetCurrentResourceName(), "create_house_shell",{
            ...
            function(submitData,submitMenu)
              if submitData.current.label == "Done" then
                exports['okokNotify']:Alert("SERVIDOR", Labels['CreationComplete'], 6500, 'success')
                submitMenu.close()
                ...
                exports['okokNotify']:Alert("SERVIDOR", submitData.current.label..": "..(shells[submitData.current.label] == true and Labels['Enabled'] or Labels['Disabled']), 6500, 'info')
              end
            end,
            function(data,menu)
              menu.close()
  ...
  RegisterCommand('delhouse', function(...)  
    ...
    for _,thisHouse in pairs(Houses) do
      ...
      if not closestDist or dist < closestDist then
        ESX.ShowNotification("No hay ninguna casa cerca", "error")
        closest = thisHouse
        closestDist = dist
      end
    end
