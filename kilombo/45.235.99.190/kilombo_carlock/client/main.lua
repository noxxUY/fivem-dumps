	...
	ESX.TriggerServerCallback('euk-carlock:requestPlayerCars', function(isOwnedVehicle)
		...
		if isOwnedVehicle then
			...
			if lockStatus == 1 then -- unlocked
				...
				exports['okokNotify']:Alert("SERVIDOR", "Vehículo bloqueado: "..(GetVehicleNumberPlateText(vehicle)).." ", 4500, 'info')

				-- ESX.ShowNotification('Locked'..(GetVehicleNumberPlateText(vehicle))..' ')

				-- lib.notify({
				-- 	title = 'Vehicle',
				-- 	description = 'Locked'..(GetVehicleNumberPlateText(vehicle))..' ',
				-- 	type = 'error'
				-- })
			...
			elseif lockStatus == 2 then -- locked
				...
				exports['okokNotify']:Alert("SERVIDOR", "Vehículo desbloqueado: "..(GetVehicleNumberPlateText(vehicle)).." ", 4500, 'info')
				
				-- ESX.ShowNotification('Unlocked'..(GetVehicleNumberPlateText(vehicle))..' ')
				-- lib.notify({
				-- 	title = 'Vehicle',
				-- 	description = 'Unlocked'..(GetVehicleNumberPlateText(vehicle))..' ',
				-- 	type = 'success'
				-- })
			end
		end
