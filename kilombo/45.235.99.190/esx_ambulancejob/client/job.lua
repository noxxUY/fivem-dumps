...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		title    = '<i class="fa-solid fa-truck-medical fa-bounce"></i> SAME - Menu General',
		align    = 'top-left',
		elements = {
			{label = _U('ems_menu'), value = 'citizen_interaction'},
			{label = _U('ems_facturacion'), value = 'facturacion_same'}
	}}, function(data, menu)
		if data.current.value == 'facturacion_same' then
			local elements2 = {
				{unselectable = true, icon = "fas fa-scroll", title = "Facturación | SAME"},
				{icon = 'fa-solid fa-money-bill', title = "Importe", input = true, inputType = "number", inputMin = 1, inputMax = 1000000, inputPlaceholder = "Importe de la factura..", name="cantidadfactura"},
				{icon = 'fa-solid fa-comments', title = "Razón", input = true, inputType = "text", inputPlaceholder = "Razón de la factura..", name="razonfactura"},
				{icon = "fas fa-check-double", title = "Confirmar", value = "confirm"}
			}

			ESX.OpenContext("right", elements2, function(menu2,element2)
				local amount = tonumber(menu2.eles[2].inputValue)
				local razon = menu2.eles[3].inputValue

				if amount == nil or amount < 0 then
					ESX.ShowNotification("Cantidad inválida", "error")
				elseif razon == nil then
					ESX.ShowNotification("Razón inválida", "error")
				else
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						ESX.CloseContext()
						ESX.ShowNotification("Hiciste una factura a [#"..GetPlayerServerId(closestPlayer).."] por $"..ESX.Math.GroupDigits(amount).." con la razón "..razon.."")
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'ambulance', "Same: "..razon.." ", amount)
					end
				end
			end)
	...
	deadPlayers[Player] = 'distress'
		...
		if Config.Debug then 
			...
			exports.ox_lib:notify({
				title = 'Central 911',
				description = "Se registró una nueva persona inconsciente en "..zone.."",
