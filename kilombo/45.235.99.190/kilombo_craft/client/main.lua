...
local function HasItem(table, blueprint)
    for k,v in pairs(table) do
        ...
        if hasitem < v.value then
            CollectiveC.Notification(3, 'Necesitas '..v.value..'x '..v.label..' para poder realizar esta acción')
            return false
        end
    end
...
AddEventHandler('kilombo_craft:Craft', function(data)
    local context = data.context
    local method = data.method
    if context == 'ox_lib' then
        lib.registerContext({
            id = 'cfx_hu_craft_option',
            title = '📄 Opciones de Crafteo',
            options = {
                {
                    title = '🔨 Craftear',
                    description = 'Craftear este objeto',
                    event = 'kilombo_craft:Craft',
                    args = {
                        method = 'craft',
    ...
    elseif method == 'craft' then
        ...
        if HasItem(data.input_items, data.blueprint_data) then
            ...
            elseif data.craft_type == 'ox-progress' then
                if libLoaded then
                    success  = lib.progressBar({
                        duration = data.craft_time,
                        label = 'Crafteando '..data.output_label,
                        useWhileDead = false,
                        canCancel = false,
                        disable = {
                            car = true,
            ...
            if not success then
                isCrafting = false
                CollectiveC.Notification(3, 'Fallaste, intentalo nuevamente.')
                if data.craft_table then
            ...
            if success then
                isCrafting = false
                TriggerServerEvent('kilombo_craft:Finish', {
                    input_items = data.input_items,
                    output_qyt = data.output_qyt,
                    output_item = data.output_item,
                    output_label = data.output_label,
    ...
    if ESX.PlayerData.job.name == self.Job or not self.Job then
        ...
        elseif self.method == 'text' then
            ...
            if self.text == 'ox_lib' then
                lib.showTextUI('[E] - Craftear', {icon = 'fas fa-wrench'})
            elseif self.text == 'ps-ui' then
                exports['ps-ui']:DisplayText("[E] Craftear", "primary")
            elseif self.text == 'esx_textui' then
                ESX.TextUI("Pulsá ~b~[E]~s~ para acceder al menú de crafteo", "info")
            end
        end
    end
end
...
RegisterKeyMapping('craft_system', 'Abrir menu crafteo', 'keyboard', 'E')