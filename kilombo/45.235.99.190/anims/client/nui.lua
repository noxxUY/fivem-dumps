...
RegisterCommand(cfg.commandNameEmote, function(_, args)
    if args and args[1] and not IsPedFalling(PlayerPedId()) and not IsPedJumping(PlayerPedId()) then
...
RegisterCommand(cfg.defaultCommand, function()
    if cfg.defaultEmote and not IsPedFalling(PlayerPedId()) and not IsPedJumping(PlayerPedId()) then
...
if cfg.defaultEmoteUseKey then
    CreateThread(function()
        while cfg.defaultEmoteKey do
            if IsControlJustPressed(0, cfg.defaultEmoteKey) and not IsPedFalling(PlayerPedId()) and not IsPedJumping(PlayerPedId()) then
                findEmote(cfg.defaultEmote)
            end
            Wait(5)
        end
