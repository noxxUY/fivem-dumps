...
function OpenLSMenu(elems, menuName, menuTitle, parent, precio, vehiculo)
        ...
        for k, v in pairs(Config.Menus) do
            ...
            if k == data.current.modType or isRimMod then
                ...
                if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
                    ...
                    if isRimMod then
                        return TriggerServerEvent('esx_lscustom:buyMod', 0, nombre, label)
...
function GetAction(data)
    ...
    OpenLSMenu(elements, menuName, menuTitle, parent, vehiclePrice, vehicle)
end
    ...
    while true do
        ...
        if IsPedInAnyVehicle(playerPed, false) then
            ...
            if (ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic') or not Config.IsMechanicJobOnly then
                for k, v in pairs(Config.Zones_Bennys) do
