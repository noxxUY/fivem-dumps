...
AddEventHandler('esx:playerLoaded', function()
    ESX.TriggerServerCallback('esx_lscustom3:getVehiclesPrices', function(vehicles)
        Vehicles = vehicles
    end)
end)

RegisterNetEvent('esx_lscustom3:installMod')
AddEventHandler('esx_lscustom3:installMod', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    local NetId = NetworkGetNetworkIdFromEntity(vehicle)
    myCar = ESX.Game.GetVehicleProperties(vehicle)
    TriggerServerEvent('esx_lscustom3:refreshOwnedVehicle', myCar, NetId)
end)

RegisterNetEvent('esx_lscustom3:restoreMods', function(netId, props)
...
RegisterNetEvent('esx_lscustom3:cancelInstallMod')
AddEventHandler('esx_lscustom3:cancelInstallMod', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
AddEventHandler('onClientResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		if lsMenuIsShowed then
			TriggerEvent('esx_lscustom3:cancelInstallMod')
		end
	end
end)
...
function OpenLSMenuRL(elems, menuName, menuTitle, parent, precio, vehiculo)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName, {
        ...
        for k, v in pairs(Config.Menus) do
            ...
            if k == data.current.modType or isRimMod then
                ...
                if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
                    ...
                    myCar = ESX.Game.GetVehicleProperties(vehicle)
                    TriggerServerEvent('esx_lscustom3:refreshOwnedVehicle', myCar, NetworkGetNetworkIdFromEntity(vehicle))
                else
                    ...
                    if isRimMod then
                        return TriggerServerEvent('esx_lscustom3:buyMod', 0, nombre, label)
        ...
        if parent == nil then
            ...
            TriggerServerEvent('esx_lscustom3:stopModing', myCar.plate)
            myCar = {}
        end
    end, function(data, menu) -- on change
        UpdateModsRL(data.current)
    end)
end

function UpdateModsRL(data)
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function GetActionRL(data)
    ...
    OpenLSMenuRL(elements, menuName, menuTitle, parent, vehiclePrice, vehicle)
end
...
CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(playerPed, false) then
            ...
            if (ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic3') or not Config.IsMechanicJobOnly then
                for k, v in pairs(Config.Zones_RedLine) do
                    ...
                    if #(coords - zonePos) < 3.0 then
                        ...
                        if not lsMenuIsShowed then
                            ...
                            if IsControlJustReleased(0, 38) then
                                ...
                                local netId = NetworkGetNetworkIdFromEntity(vehicle)
                                TriggerServerEvent('esx_lscustom3:startModing', myCar, netId)
