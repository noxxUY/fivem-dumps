                ...
                for i = 1, #Config.AtmModels do
                    ...
                    if atm ~= 0 then
                        ...
                        if not isInAtmMarker and atmDistance <= 1.5 then
                            isInAtmMarker = true
                            ESX.TextUI(_U('press_e_banking'), 'info')
                            Listen4Key()
...
RegisterNUICallback('close', function(data, cb)
    CloseUi()
    cb('ok')
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'cierrobanco', 0.1)
end)
    ...
    if data ~= nil and isInMenu then
        ESX.TriggerServerCallback("esx_banking:checkPincode", function(pincode)
            if pincode then
                ...
                TriggerServerEvent('InteractSound_SV:PlayOnSource', 'accesobanco', 0.1)
