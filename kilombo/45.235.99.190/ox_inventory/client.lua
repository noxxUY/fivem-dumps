...
function client.openInventory(inv, data)
	...
	if inv == 'dumpster' and cache.vehicle then
		return lib.notify({ id = 'inventory_right_access', type = 'error', position = 'center-right',  description = locale('inventory_right_access') })
	end
	...
	if canOpenInventory() then
		...
		if inv == 'player' then
			...
			if not targetCoords or #(targetCoords - GetEntityCoords(playerPed)) > 1.8 or not (client.hasGroup(shared.police) or not client.hasGroup(shared.gna) or not client.hasGroup({ ['gna'] = 0 }) or canOpenTarget(targetPed)) then
				return lib.notify({ id = 'inventory_right_access', type = 'error', position = 'center-right',  description = locale('inventory_right_access') })
			end
		end
		...
		if inv == 'shop' and invOpen == false then
			if cache.vehicle then
				return lib.notify({ id = 'cannot_perform', type = 'error', position = 'center-right',  description = locale('cannot_perform') })
			end
		...
		if left then
			...
			if invOpen == false then lib.notify({ id = 'inventory_right_access', type = 'error', position = 'center-right',  description = locale('inventory_right_access') }) end
			if invOpen then client.closeInventory() end
		end
	elseif invBusy then lib.notify({ id = 'inventory_player_access', type = 'error', position = 'center-right',  description = locale('inventory_player_access') }) end
end
...
local function useSlot(slot)
	...
	if canUseItem(data.ammo and true) then
		if data.component and not currentWeapon then
			return lib.notify({ id = 'weapon_hand_required', type = 'error', position = 'center-right',  description = locale('weapon_hand_required') })
		end
		...
		if durability and durability <= 100 and consume then
			if durability <= 0 then
				return lib.notify({ type = 'error', position = 'center-right',  description = locale('no_durability', label) })
			elseif consume ~= 0 and consume < 1 and durability < consume * 100 then
				return lib.notify({ type = 'error', position = 'center-right',  description = locale('not_enough_durability', label) })
			end
		end
			...
			elseif data.component then
				...
				for componentIndex = 1, #weaponComponents do
					if componentType == Items[weaponComponents[componentIndex]].type then
						return lib.notify({ id = 'component_slot_occupied', type = 'error', position = 'center-right',  description = locale('component_slot_occupied', componentType) })
					end
				end
				...
				for i = 1, #components do
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							lib.notify({ id = 'component_has', type = 'error', position = 'center-right',  description = locale('component_has', label) })
						else
				...
				lib.notify({ id = 'component_invalid', type = 'error', position = 'center-right',  description = locale('component_invalid', label) })
...
local function registerCommands()
	...
	lib.addKeybind({
		...
		onPressed = function()
			...
			if invBusy then
				return lib.notify({ id = 'inventory_player_access', type = 'error', position = 'center-right',  description = locale('inventory_player_access') })
			end
			...
			if GetVehicleDoorLockStatus(entity) > 1 then
				return lib.notify({ id = 'vehicle_locked', type = 'error', position = 'center-right',  description = locale('vehicle_locked') })
			end
		...
		onPressed = function(self)
			...
			if currentWeapon.ammo then
				if currentWeapon.metadata.durability > 0 then
					...
					lib.notify({ id = 'no_durability', type = 'error', position = 'center-right',  description = locale('no_durability', currentWeapon.label) })
				end
			end
		end
	})
			...
			if not canOpenInventory() then
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 1.8 or not (client.hasGroup(shared.police) or not client.hasGroup(shared.gna) or not client.hasGroup({ ['gna'] = 0 }) or canOpenTarget(ped)) then
							client.closeInventory()
							lib.notify({ id = 'inventory_lost_access', type = 'error', position = 'center-right',  description = locale('inventory_lost_access') })
						else
...
RegisterNUICallback('removeComponent', function(data, cb)
	cb(1)
	if currentWeapon then
		if data.slot ~= currentWeapon.slot then return lib.notify({ id = 'weapon_hand_wrong', type = 'error', position = 'center-right',  description = locale('weapon_hand_wrong') }) end
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	elseif response then
		lib.notify({ type = 'error', position = 'center-right',  description = locale(response) })
	end
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		...
		if not success then
			if response then lib.notify({ type = 'error', position = 'center-right',  description = locale(response or 'cannot_perform') }) end
			break
		end
	end
