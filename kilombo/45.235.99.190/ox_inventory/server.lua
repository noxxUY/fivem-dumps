...
lib.addCommand({'additem', 'giveitem'}, {
	help = 'Dar objetos al inventario de un jugador',
	params = {
		{ name = 'target', type = 'playerId', help = 'ID del jugador' },
		{ name = 'item', type = 'string', help = 'Nombre del objeto' },
		{ name = 'count', type = 'number', help = 'Cantidad de objetos', optional = true },
		{ name = 'type', help = 'Tipo de metadata (opcional)', optional = true },
	},
	...
	if item then
		...
		if server.loglevel > 0 then
			envioInvADiscord("15844367", "Logs | Items", " Item entregado por: ".. GetPlayerName(src) .." (#"..src..")", "\nHa dado: **".. item.name .."** \nCantidad: **".. count .."** \nObjetivo: "..GetPlayerName(args.target).." **(#".. args.target ..")**", "Kilombo RP - Staff")
			lib.logger(source.owner, 'admin', ('"%s" gave %sx %s to "%s"'):format(source.label, count, item.name, inventory.label))
		end
	end
end)

function envioInvADiscord(color, name, title, message, footer)
	local embed = {
		  {
			  ["color"] = color,
			  ["title"] = "**".. title .."**",
      ...
      PerformHttpRequest("https://discord.com/api/webhooks/1099058056815448064/_2i29qF5lp30BVZnpsiCX9ETHkvx4TKh39fnrPkn1s72dPnFZ-WjCjL_hfMaJU-kvs8m", function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end

lib.addCommand('removeitem', {
	help = 'Sacar objetos del inventario de un jugador',
	params = {
		{ name = 'target', type = 'playerId', help = 'ID del jugador' },
		{ name = 'item', type = 'string', help = 'Nombre del objeto' },
		{ name = 'count', type = 'number', help = 'Cantidad de objetos' },
		{ name = 'type', help = 'Eliminar sólo los elementos con metadata coincidentes "type" (opcional)', optional = true },
	},
	restricted = {'group.admin', 'group.jradmin'},
}, function(source, args)
	local item = Items(args.item)
...
lib.addCommand('setitem', {
	help = 'Establece la cantidad de objetos de un jugador, eliminándolos o añadiéndolos según sea necesario.',
	params = {
		{ name = 'target', type = 'playerId', help = 'ID del jugador' },
		{ name = 'item', type = 'string', help = 'Nombre del objeto' },
		{ name = 'count', type = 'number', help = 'Cantidad de objetos', optional = true },
		{ name = 'type', help = 'Añadir o eliminar elementos con la metadata "type" (opcional)', optional = true },
	},
	restricted = {'group.admin', 'group.jradmin'},
}, function(source, args)
	local item = Items(args.item)
...
lib.addCommand('borrarevidencia', {
	help = 'Borra un armario de pruebas de la policía con un ID',
	params = {
		{ name = 'locker', type = 'number', help = 'ID del locker' },
	},
}, function(source, args)
...
lib.addCommand({'takeinv', 'confiscar'}, {
	help = 'Confiscar un inventario, devolver con /devolver',
	params = {
		{ name = 'target', type = 'playerId', help = 'ID del jugador' },
	},
	restricted = {'group.admin', 'group.jradmin'},
...
lib.addCommand({'restoreinv', 'devolver'}, {
	help = 'Devolver un inventario confiscado anteriormente',
	params = {
		{ name = 'target', type = 'playerId', help = 'ID del jugador' },
	},
	restricted = {'group.admin', 'group.jradmin'},
...
lib.addCommand('borrarinv', {
	help = 'Borrar todo el inventario de un jugador',
	params = {
		{ name = 'invId', help = 'ID del jugador o inventario' },
	},
	restricted = {'group.admin', 'group.jradmin'},
}, function(source, args)
...
lib.addCommand('saveinv', {
	help = 'Guardar todos los cambios de inventario pendientes en la base de datos',
	params = {
		{ name = 'lock', help = 'Bloquear el acceso al inventario, hasta que se reinicie o se guarde sin bloqueo', optional = true },
	},
	restricted = {'group.admin', 'group.jradmin'},
}, function(source, args)
...
lib.addCommand('verinv', {
	help = 'Ver el inventario de un jugador',
	params = {
		{ name = 'invId', help = 'ID del jugador o inventario' },
	},
	restricted = {'group.admin', 'group.jradmin'},
