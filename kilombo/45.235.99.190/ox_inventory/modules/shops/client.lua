...
local function nearbyShop(point)
	...
	if point.isClosest and point.currentDistance < 1.2 and IsControlJustReleased(0, 38) or IsControlJustPressed(0, 289) or IsControlJustReleased(0, 289) then
		-- print("PRESIONA E O F2")
		if point.type ~= "ArmeriaVIP" and point.type ~= "TiendaMafias" and point.currentDistance < 1.2 then
			client.openInventory('shop', { id = point.invId, type = point.type })
		elseif point.type == "ArmeriaVIP" then
			...
			if vip then
				...
				if point.currentDistance < 1.2 then
					Wait(90)
					TriggerEvent('ox_inventory:closeInventory', point.invId, true)
					exports['okokNotify']:Alert("SERVIDOR", "Tenés que ser VIP para usar esta armería.", 3500, 'error')
				end
			end 
		elseif point.type == "TiendaMafias" then
			local mafia = exports['kilombo_mafias']:getGangID(source)
			if mafia >= 0 then
				client.openInventory('shop', { id = point.invId, type = point.type })
			else
				if point.currentDistance < 1.2 then
					Wait(90)
					TriggerEvent('ox_inventory:closeInventory', point.invId, true)
					exports['okokNotify']:Alert("SERVIDOR", "Tenés que ser Mafia para usar este punto.", 3500, 'error')
				end
			end
		end
	end
