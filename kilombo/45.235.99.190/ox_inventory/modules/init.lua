...
shared = {
	...
	gna = json.decode(GetConvar('inventory:gna', '["gna"]')),
}
...
do
	if type(shared.gna) == 'string' then
		shared.gna = { shared.gna }
	end

	local gna = table.create(0, shared.gna and #shared.gna or 0)

	for i = 1, #shared.gna do
		gna[shared.gna[i]] = 0
	end
