...
AddEventHandler('esx_garbagecrew:selectnextjob', function()
	if currentstop < Config.MaxStops then
		...
		albetogetbags = false
		exports['okokNotify']:Alert("SERVIDOR", _U('return_depot'), 4500, 'info')
	end
end)
...
function CollectBagFromBin(currentZone)
	...
	if DoesEntityExist(worktruck) and GetDistanceBetweenCoords(GetEntityCoords(worktruck), GetEntityCoords(PlayerPedId()), true) < 25.0 then
		...
		exports['okokNotify']:Alert("SERVIDOR", _U('not_near_truck'), 4500, 'info')
...
function SelectBinAndCrew(location)
	...
	if bin ~= 0 then
		...
		exports['okokNotify']:Alert("SERVIDOR", _U('no_trash_aviable'), 4500, 'info')
		SetBlipRoute(Blips['endmission'], true)
		FindDeliveryLoc()
	end
end
...
function FindDeliveryLoc()
	...
	oncollection = true
	exports['okokNotify']:Alert("SERVIDOR", _U('drive_to_collection'), 4500, 'info')
end
...
function MenuVehicleSpawner()
	...
	for i=1, #Config.Trucks, 1 do
		table.insert(elements, {label = '<i style="color:powderblue;" class="fa-solid fa-truck"></i> - '..GetLabelText(GetDisplayNameFromVehicleModel(Config.Trucks[i]))..'', value = Config.Trucks[i]})
	end
