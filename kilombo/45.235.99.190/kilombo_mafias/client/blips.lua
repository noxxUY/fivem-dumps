		...
		for _key, activeGang in pairs(activeGangs) do
			...
			if gangCoords ~= nil and gangCoords.blip and gangCoords.blip ~= nil then
				...
				if myGangId ~= nil and myGangId == activeGang.id then
					...
					SetBlipScale(blipGangPlace, 0.7)
...
local blipsMafias = {}
local versedes = false

RegisterCommand("versedes", function()
    ESX.TriggerServerCallback('mafias:veoGrupo', function(group)
        if group == "admin" then
			if versedes == false then
				exports['okokNotify']:Alert("MAFIAS", '<span style="color:green;font-weight:bold">Activaste</span> los <span style="color:cyan;font-weight:bold">BLIPS</span> de sedes', 5500, 'error')
				for _key, activeGang in pairs(activeGangs) do
					local gangCoords = json.decode(activeGang.coords)
					if gangCoords ~= nil and gangCoords.blip and gangCoords.blip ~= nil then
						...
						table.insert(blipsMafias, blipGangPlace)
					end
				end
				versedes = true
			else
				exports['okokNotify']:Alert("MAFIAS", '<span style="color:red;font-weight:bold">Desactivaste</span> los <span style="color:cyan;font-weight:bold">BLIPS</span> de sedes', 5500, 'error')

				for k, existingBlip in pairs(blipsMafias) do
					RemoveBlip(existingBlip)
				end
				versedes = false
			end
		else
			exports['okokNotify']:Alert("MAFIAS", 'No tenes suficientes permisos para usar este comando.', 4500, 'error')
		end
	end)
end)
...
-- 		SetBlipScale(blipsVehicelImpound[_key], 0.7)
-- 		BeginTextCommandSetBlipName('STRING')
