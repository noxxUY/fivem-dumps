	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if (Killer == PlayerId()) then
				DeathReason = 'se suicidó'
			elseif (Killer == nil) then
				DeathReason = 'murió'
			else
				if IsMelee(DeathCauseHash) then
					DeathReason = 'trompeó a'
				elseif IsTorch(DeathCauseHash) then
					DeathReason = 'quemó a'
				elseif IsKnife(DeathCauseHash) then
					DeathReason = 'acuchilló a'
				elseif IsPistol(DeathCauseHash) then
					DeathReason = 'mató con pistola a'
				elseif IsSub(DeathCauseHash) then
					DeathReason = 'mató con subfusil a'
				elseif IsRifle(DeathCauseHash) then
					DeathReason = 'mató con rifle a'
				elseif IsLight(DeathCauseHash) then
					DeathReason = 'cagó a tiros a'
				elseif IsShotgun(DeathCauseHash) then
					DeathReason = 'mató con escopeta a'
				elseif IsSniper(DeathCauseHash) then
					DeathReason = 'mató con sniper a'
				elseif IsHeavy(DeathCauseHash) then
					DeathReason = 'mató con armas ilegales a'
				elseif IsMinigun(DeathCauseHash) then
					DeathReason = 'mató con minigun a'
				elseif IsBomb(DeathCauseHash) then
					DeathReason = 'mató con bomba a'
				elseif IsVeh(DeathCauseHash) then
					DeathReason = 'mató con arma de vehículo a'
				elseif IsVK(DeathCauseHash) then
					DeathReason = 'atropelló a'
				else
					DeathReason = 'mató a'
				end
			end
			
			if DeathReason == 'se suicidó' or DeathReason == 'murió' then
				TriggerServerEvent('kilombo_logs:playerDied', GetPlayerName(PlayerId()) .. ' **['..GetPlayerServerId(PlayerId())..']** ' .. DeathReason .. '.', Weapon)
			else
				TriggerServerEvent('kilombo_logs:playerDied', GetPlayerName(Killer) .. ' **['..GetPlayerServerId(Killer)..']** ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. ' **['..GetPlayerServerId(PlayerId())..']** .', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
