...
local defaultParts = {
						...
						["leva"] = {type = "stock_leva", health = 100},
						["tapa"] = {type = "stock_tapa", health = 100},
						["piston"] = {type = "stock_piston", health = 100},
						["inyectores"] = {type = "stock_inyectores", health = 100},
						["bobina"] = {type = "stock_bobina", health = 100},
						["embreague"] = {type = "stock_embreague", health = 100},
						["fulltech"] = {type = "stock_fulltech", health = 100}
                    }
...
local tapaPower = 0
local pistonPower = 0
local inyectoresPower = 0
local bobinaPower = 0
local embreaguePower = 0
local fulltechPower = 0
        ...
        while true do
            Citizen.Wait(1)
				if repairMode then
								...
								Citizen.CreateThread(
									function()
										...
										if repairInstall then
											...
											currentVehicleParts[repairPartType].health = repairHealth
                    ...
                    elseif repairPartType == "leva" then
					if DoesVehicleHaveDoor(currentVehicle, 4) then
						if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
							local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
							...
							if not repairing then
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_leva"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_leva"])
								end
							else
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_leva"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_leva"])
								end
							end
							...
							if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
								...
								Citizen.CreateThread(
									function()
										...
										if repairInstall then
											Citizen.Wait(Config.levaInstallTime)
										else
											Citizen.Wait(Config.levaRepairTime)
										end
						...
						local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
						 ...
						 elseif repairPartType == "tapa" then
					if DoesVehicleHaveDoor(currentVehicle, 4) then
						if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
							local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
							...
							if not repairing then
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_tapa"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_tapa"])
								end
							else
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_tapa"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_tapa"])
								end
							end
							...
							if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
								...
								Citizen.CreateThread(
									function()
										...
										if repairInstall then
											Citizen.Wait(Config.tapaInstallTime)
										else
											Citizen.Wait(Config.tapaRepairTime)
										end
						...
						local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
						 ...
						 elseif repairPartType == "piston" then
					if DoesVehicleHaveDoor(currentVehicle, 4) then
						if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
							local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
							...
							if not repairing then
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_piston"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_piston"])
								end
							else
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_piston"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_piston"])
								end
							end
							...
							if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
								...
								Citizen.CreateThread(
									function()
										...
										if repairInstall then
											Citizen.Wait(Config.pistonInstallTime)
										else
											Citizen.Wait(Config.pistonRepairTime)
										end
					...
					elseif repairPartType == "inyectores" then
						if DoesVehicleHaveDoor(currentVehicle, 4) then
							if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
								local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
								...
								if not repairing then
									if repairInstall then
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_inyectores"])
									else
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_inyectores"])
									end
								else
									if repairInstall then
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_inyectores"])
									else
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_inyectores"])
									end
								end
								...
								if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
									...
									Citizen.CreateThread(
										function()
											...
											if repairInstall then
												Citizen.Wait(Config.inyectoresInstallTime)
											else
												Citizen.Wait(Config.inyectoresRepairTime)
											end
						 ...
						 elseif repairPartType == "bobina" then
					if DoesVehicleHaveDoor(currentVehicle, 4) then
						if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
							local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
							...
							if not repairing then
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_bobina"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_bobina"])
								end
							else
								if repairInstall then
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_bobina"])
								else
									DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_bobina"])
								end
							end
							...
							if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
								...
								Citizen.CreateThread(
									function()
										...
										if repairInstall then
											Citizen.Wait(Config.bobinaInstallTime)
										else
											Citizen.Wait(Config.bobinaRepairTime)
										end
					...
					elseif repairPartType == "embreague" then
						if DoesVehicleHaveDoor(currentVehicle, 4) then
							if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
								local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
								...
								if not repairing then
									if repairInstall then
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_embreague"])
									else
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_embreague"])
									end
								else
									if repairInstall then
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_embreague"])
									else
										DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_embreague"])
									end
								end
								...
								if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
									...
									Citizen.CreateThread(
										function()
											...
											if repairInstall then
												Citizen.Wait(Config.embreagueInstallTime)
											else
												Citizen.Wait(Config.embreagueRepairTime)
											end
						...
						elseif repairPartType == "fulltech" then
							if DoesVehicleHaveDoor(currentVehicle, 4) then
								if GetVehicleDoorAngleRatio(currentVehicle, 4) ~= 0 then
									local coords = GetOffsetFromEntityInWorldCoords(currentVehicle, off.x, off.y + 0.8, off.z - 0.1)
									...
									if not repairing then
										if repairInstall then
											DrawText3D(coords.x, coords.y, coords.z, Config.Text["install_fulltech"])
										else
											DrawText3D(coords.x, coords.y, coords.z, Config.Text["repair_fulltech"])
										end
									else
										if repairInstall then
											DrawText3D(coords.x, coords.y, coords.z, Config.Text["installing_fulltech"])
										else
											DrawText3D(coords.x, coords.y, coords.z, Config.Text["repairing_fulltech"])
										end
									end
									...
									if IsControlJustReleased(0, Keys["E"]) and #(pedCoords - coords) < 2.0 then
										...
										Citizen.CreateThread(
											function()
												...
												if repairInstall then
													Citizen.Wait(Config.fulltechInstallTime)
												else
													Citizen.Wait(Config.fulltechRepairTime)
												end
								...
								function()
									...
									if done == 3 then
										...
										if repairInstall then
											...
											currentVehicleParts[repairPartType].health = repairHealth
											currentVehicleParts[repairPartType].type = repairPart
											for i=0, 5, 1 do SetVehicleTyreFixed(currentVehicle, i) end
										else
											currentVehicleParts[repairPartType].health = 100.0
											for i=0, 5, 1 do SetVehicleTyreFixed(currentVehicle, i) end
										end
...
function checkVehicleCompatability(veh)

local class = GetVehicleClass(veh)

if class == 16 or class == 14 or class == 13 or class == 15 or class == 8 then
	return false
else
	return true
end
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if seat == ped and not inVehicle and checkVehicleCompatability(veh) then
            ...
            if inVehicle and currentVehicleParts ~= nil then
				...
				--TURBO WEAR
                if currentVehicleParts["leva"] then
                    if currentVehicleParts["leva"].health < 0 then
                        currentVehicleParts["leva"] = nil
                        levaPower = 0
                    else
                        if currentVehicleParts["leva"].health < 30 then
                            checkEngine = true
                        end

                        if levaPower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (levaPower * rpm))

                                currentVehicleParts["leva"].health =
                                    currentVehicleParts["leva"].health -
                                    ((100 - Config.leva[currentVehicleParts["leva"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.leva[currentVehicleParts["leva"].type].power))
                            end
                        end
                    end
                end
				...
				--TURBO WEAR
                if currentVehicleParts["tapa"] then
                    if currentVehicleParts["tapa"].health < 0 then
                        currentVehicleParts["tapa"] = nil
                        tapaPower = 0
                    else
                        if currentVehicleParts["tapa"].health < 30 then
                            checkEngine = true
                        end

                        if tapaPower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (tapaPower * rpm))

                                currentVehicleParts["tapa"].health =
                                    currentVehicleParts["tapa"].health -
                                    ((100 - Config.tapa[currentVehicleParts["tapa"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.tapa[currentVehicleParts["tapa"].type].power))
                            end
                        end
                    end
                end
				...
				--TURBO WEAR
                if currentVehicleParts["piston"] then
                    if currentVehicleParts["piston"].health < 0 then
                        currentVehicleParts["piston"] = nil
                        pistonPower = 0
                    else
                        if currentVehicleParts["piston"].health < 30 then
                            checkEngine = true
                        end

                        if pistonPower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (pistonPower * rpm))

                                currentVehicleParts["piston"].health =
                                    currentVehicleParts["piston"].health -
                                    ((100 - Config.piston[currentVehicleParts["piston"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.piston[currentVehicleParts["piston"].type].power))
                            end
                        end
                    end
                end
								...
								--TURBO WEAR
                if currentVehicleParts["inyectores"] then
                    if currentVehicleParts["inyectores"].health < 0 then
                        currentVehicleParts["inyectores"] = nil
                        inyectoresPower = 0
                    else
                        if currentVehicleParts["inyectores"].health < 30 then
                            checkEngine = true
                        end

                        if inyectoresPower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (inyectoresPower * rpm))

                                currentVehicleParts["inyectores"].health =
                                    currentVehicleParts["inyectores"].health -
                                    ((100 - Config.inyectores[currentVehicleParts["inyectores"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.inyectores[currentVehicleParts["inyectores"].type].power))
                            end
                        end
                    end
                end
											...
											--TURBO WEAR
                if currentVehicleParts["bobina"] then
                    if currentVehicleParts["bobina"].health < 0 then
                        currentVehicleParts["bobina"] = nil
                        bobinaPower = 0
                    else
                        if currentVehicleParts["bobina"].health < 30 then
                            checkEngine = true
                        end

                        if bobinaPower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (bobinaPower * rpm))

                                currentVehicleParts["bobina"].health =
                                    currentVehicleParts["bobina"].health -
                                    ((100 - Config.bobina[currentVehicleParts["bobina"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.bobina[currentVehicleParts["bobina"].type].power))
                            end
                        end
                    end
                end
										...
										--TURBO WEAR
                if currentVehicleParts["embreague"] then
                    if currentVehicleParts["embreague"].health < 0 then
                        currentVehicleParts["embreague"] = nil
                        embreaguePower = 0
                    else
                        if currentVehicleParts["embreague"].health < 30 then
                            checkEngine = true
                        end

                        if embreaguePower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (embreaguePower * rpm))

                                currentVehicleParts["embreague"].health =
                                    currentVehicleParts["embreague"].health -
                                    ((100 - Config.embreague[currentVehicleParts["embreague"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.embreague[currentVehicleParts["embreague"].type].power))
                            end
                        end
                    end
                end

				if currentVehicleParts["fulltech"] then
                    if currentVehicleParts["fulltech"].health < 0 then
                        currentVehicleParts["fulltech"] = nil
                        fulltechPower = 0
                    else
                        if currentVehicleParts["fulltech"].health < 30 then
                            checkEngine = true
                        end

                        if fulltechPower > 0 then
                            if rpm > 0.4 and not nitroActive then
                                SetVehicleEnginePowerMultiplier(currentVehicle, currentMulti + (fulltechPower * rpm))

                                currentVehicleParts["fulltech"].health =
                                    currentVehicleParts["fulltech"].health -
                                    ((100 - Config.fulltech[currentVehicleParts["fulltech"].type].durability) *
                                        (((kmh / (currentWear * 200)) + (rpm / (currentWear * 200))) *
                                            Config.fulltech[currentVehicleParts["fulltech"].type].power))
                            end
                        end
                    end
                end
...
for _,v in ipairs(Config.WhitelistedJobs) do
    ...
    function(tool)
        ...
        if tool == "mechanic_tools" then
            if hasJob() then
				...
				if localVehLockStatus == 1 or localVehLockStatus == 0  then
				--VehicleClass
				...
				if IsOnLift then
                    ...
                    for k, v in pairs(parts) do
						...
						if Config.leva[v.type] ~= nil then
                            v.label = Config.leva[v.type].label
                        end
						if Config.tapa[v.type] ~= nil then
                            v.label = Config.tapa[v.type].label
                        end
						if Config.piston[v.type] ~= nil then
                            v.label = Config.piston[v.type].label
                        end
						if Config.inyectores[v.type] ~= nil then
                            v.label = Config.inyectores[v.type].label
                        end
						if Config.bobina[v.type] ~= nil then
                            v.label = Config.bobina[v.type].label
                        end
						if Config.embreague[v.type] ~= nil then
                            v.label = Config.embreague[v.type].label
                        end
						if Config.fulltech[v.type] ~= nil then
                            v.label = Config.fulltech[v.type].label
                        end
					...
					allparts["leva"] = Config.leva
					allparts["tapa"] = Config.tapa
					allparts["piston"] = Config.piston
					allparts["inyectores"] = Config.inyectores
					allparts["bobina"] = Config.bobina
					allparts["embreague"] = Config.embreague
					allparts["fulltech"] = Config.fulltech
...
RegisterNUICallback(
    ...
    function(data)
        ...
        if defaultParts[partType] then 

        if defaultParts[partType].type ~= currentVehicleParts[partType].type then
         TriggerServerEvent('core_vehicle:removePart', currentVehicleParts[partType].type, currentVehicleParts[partType].health)
     	end
         currentVehicleParts[partType] = defaultParts[partType]
...
function setleva(veh)
    local leva = Config.leva[currentVehicleParts["leva"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    levaPower = leva.power
end

function settapa(veh)
    local tapa = Config.tapa[currentVehicleParts["tapa"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    tapaPower = tapa.power
end

function setpiston(veh)
    local piston = Config.piston[currentVehicleParts["piston"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    pistonPower = piston.power
end

function setinyectores(veh)
    local inyectores = Config.inyectores[currentVehicleParts["inyectores"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    inyectoresPower = inyectores.power
end

function setbobina(veh)
    local bobina = Config.bobina[currentVehicleParts["bobina"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    bobinaPower = bobina.power
end
function setembreague(veh)
    local embreague = Config.embreague[currentVehicleParts["embreague"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    embreaguePower = embreague.power
end

function setfulltech(veh)
    local fulltech = Config.fulltech[currentVehicleParts["fulltech"].type]

    SetVehicleModKit(veh, 0)
    ToggleVehicleMod(veh, 18, true)

    fulltechPower = fulltech.power
end
...
function setTransmission(veh)
    ...
    if currentVehicleParts["transmission"].health > 0 then
     ...
     if trans.manual then
  		if trans.manual ~= true then modifyGears = trans.manual end
    	SetupManual()
    end
end
