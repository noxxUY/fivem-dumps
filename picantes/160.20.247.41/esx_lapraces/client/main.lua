...
RegisterNetEvent('esx_lapraces:client:StartRaceEditor')
AddEventHandler('esx_lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        TriggerEvent('esx:showNotification','You are already making a race.', 2)
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        TriggerEvent('esx:showNotification','You have not posted any checkpoints yet.', 2)
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        TriggerEvent('esx:showNotification','You must have at least 10 checkpoints.', 2)
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["LEFT"]) or IsDisabledControlJustPressed(0, Keys["LEFT"]) then
                    ...
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        TriggerEvent('esx:showNotification','You can not exceed 15.')
                    end
                end

                if IsControlJustPressed(0, Keys["RIGHT"]) or IsDisabledControlJustPressed(0, Keys["RIGHT"]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        CreatorData.TireDistance = CreatorData.TireDistance - 1.0
                    else
                        TriggerEvent('esx:showNotification','You can not lower than 2.')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    TriggerEvent('esx:showNotification','Press [9] again to confirm', 2)
                else
                    ...
                    TriggerEvent('esx:showNotification','Race editor canceled!', 2)
...
function SaveRace()
    ...
    TriggerServerEvent('esx_lapraces:server:SaveRace', CreatorData)

    TriggerEvent('esx:showNotification','Race: '..CreatorData.RaceName..' has been saved!', 1)
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            TriggerEvent('esx:showNotification','You can not go too fast.', 2)
        end
    else
        TriggerEvent('esx:showNotification','You can not go too fast.', 2)
    end
...
RegisterNetEvent('esx_lapraces:client:UpdateRaceRacerData')
AddEventHandler('esx_lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('esx_lapraces:client:JoinRace')
AddEventHandler('esx_lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('esx_lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        TriggerEvent('esx:showNotification','You are already in a race.', 2)
    end
end)

RegisterNetEvent('esx_lapraces:client:LeaveRace')
AddEventHandler('esx_lapraces:client:LeaveRace', function(data)
    TriggerEvent('esx:showNotification','You have leaved the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('esx_lapraces:client:RaceCountdown')
AddEventHandler('esx_lapraces:client:RaceCountdown', function()
    TriggerServerEvent('esx_lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    TriggerEvent('esx:showNotification','The race will start in 10 seconds.', 2, 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    TriggerEvent('esx:showNotification',Countdown, 2, 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            TriggerEvent('esx:showNotification','GO!', 1, 1000)
        ...
        TriggerEvent('esx:showNotification','You are not in a race right now.', 2)
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('esx_lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('esx_lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('esx_lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        TriggerEvent('esx:showNotification','Race finished in '..SecondsToClock(CurrentRaceData.TotalTime)..', with the best: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        TriggerEvent('esx:showNotification','Race finished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('esx_lapraces:client:finishh')
AddEventHandler('esx_lapraces:client:finishh', function(RaceData, RaceId, BestLap)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            if BestLap > 0 then
                local deneme = (BestLap * 100) / 10
                local second = deneme / 1000
                TriggerEvent('esx:showNotification', ' The race will be over in ' .. second ..' seconds.', 2, 3500)
                Citizen.Wait((BestLap * 100) / 10)
                TriggerServerEvent('esx_lapraces:server:LeaveRace', RaceData)
            else
                Citizen.Wait(60000)
                TriggerServerEvent('esx_lapraces:server:LeaveRace', RaceData)
            end
...
RegisterNetEvent('esx_lapraces:client:PlayerFinishs')
AddEventHandler('esx_lapraces:client:PlayerFinishs', function(RaceId, Place, esxChar)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            TriggerEvent('esx:showNotification',' place: '..Place, 2, 3500)
        end
    end
end)
...
RegisterNetEvent('esx_lapraces:client:WaitingDistanceCheck')
AddEventHandler('esx_lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerEvent('esx:showNotification','Go back to the start or you will be kicked over: '..ToFarCountdown..'s', 2, 500)
                        else
                            TriggerServerEvent('esx_lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
