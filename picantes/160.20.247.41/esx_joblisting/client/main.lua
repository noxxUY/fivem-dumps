...
function ShowJobListingMenu()
	ESX.TriggerServerCallback('esx_joblisting:getJobsList', function(jobs)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'joblisting', {
			...
			exports['mythic_notify']:DoCustomHudText('inform' , ('INFO: Tu nuevo empleo es ' .. data.current.label .. '') , 5000)
			menu.close()
	...
	while true do
		...
		for i=1, #Config.Zones, 1 do
			...
			if distance < (Config.ZoneSize.x / 2) then
                ...
                ESX.ShowFloatingHelpNotification(_U('access_job_center'), vector3(playerX, playerY, playerZ))
