...
RegisterNetEvent('ry_trucker2job:start_job')
AddEventHandler('ry_trucker2job:start_job', function()
	start_job()
end)
	...
	ESX.UI.Menu.Open(
		...
		function (data,menu)
			...
			if data.current.value == 'confirm' then
				start_travel()
				TriggerEvent('ry_trucker2job:updatworking', true)
				working = true
			end
...
function start_travel()
	ESX.Game.SpawnVehicle(truck_details.truck, Config.SpawnPointtrucker2, 270.0, function(vehicle)
			...
			ESX.Game.SpawnVehicle(truck_details.trailer, Config.SpawnPointtrucker2, 270.0, function(trailer)
				AttachVehicleToTrailer(vehicle, trailer, 1.1)
				for k,v in pairs(Config.Menus['trailers'].options) do 
					if truck_details.trailer == k then
						TriggerEvent('ry_trucker2job:' .. v.route, v.route)
					end
				end
			end)
	end)
...
function start_route(route)
	if route == 'route1' then
		TriggerEvent('ry_trucker2job:start_route', 'route1')
	elseif route == 'route2' then
		TriggerEvent('ry_trucker2job:start_route', 'route2')
	elseif route == 'route3' then
		TriggerEvent('ry_trucker2job:start_route', 'route3')
	elseif route == 'route4' then
		TriggerEvent('ry_trucker2job:start_route', 'route4')
	end
end
...
RegisterNetEvent('ry_trucker2job:delivery')
AddEventHandler('ry_trucker2job:delivery', function(x,y,z)
	RemoveBlip(Blips['delivery'])
	Blips['delivery'] = nil

	progress(3000)
	TriggerServerEvent('ry_trucker2job:pay', Config.MoneyPerDelivery)
	TriggerEvent('ry_updateblip', x,y,z)
	ESX.ShowNotification(Config.ReceiveMoney)
end)

RegisterNetEvent('ry_trucker2job:last_delivery')
AddEventHandler('ry_trucker2job:last_delivery', function(k)
	RemoveBlip(Blips['delivery'])
	Blips['delivery'] = nil
	progress(3000)
	...
	if Config.ActivateDamage then
		if vehicle_life >= 650 then
			TriggerServerEvent('ry_trucker2job:pay', Config.DeliveryFinalMoney)
			...
			if vehicle_life < 649 and vehicle_life >= 450 then
				...
				TriggerServerEvent('ry_trucker2job:pay', final_reward)
	...
	TriggerEvent('ry_trucker2job:state', false, k)
		...
		if delivery_truck then
			...
			if dist < 5 and isATruck then
				...
				if IsControlJustReleased(0, 38) then
					...
					TriggerEvent('ry_trucker2job:statelast', false)
					TriggerEvent('ry_trucker2job:updatworking', false)
					TriggerEvent('ry_trucker2job:clearblips')
				end
			end
...
RegisterNetEvent('ry_trucker2job:clearblips')
AddEventHandler('ry_trucker2job:clearblips', function()
	RemoveBlip(Blips['delivery'])
	Blips['delivery'] = nil
end)

RegisterNetEvent('ry_trucker2job:updatworking2')
AddEventHandler('ry_trucker2job:updatworking2', function(option)
    working = option
	ESX.Game.DeleteVehicle(truck_details.info)
	TriggerEvent('ry_trucker2job:updatworking', option)
end)

RegisterNetEvent('ry_trucker2job:route1')
AddEventHandler('ry_trucker2job:route1', function(route)
    start_route(route)
end)

RegisterNetEvent('ry_trucker2job:route2')
AddEventHandler('ry_trucker2job:route2', function(route)
	start_route(route)
end)

RegisterNetEvent('ry_trucker2job:route3')
AddEventHandler('ry_trucker2job:route3', function(route)
	start_route(route)
end)

RegisterNetEvent('ry_trucker2job:route4')
AddEventHandler('ry_trucker2job:route4', function(route)
	start_route(route)
end)
