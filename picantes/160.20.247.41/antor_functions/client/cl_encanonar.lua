...
RegisterCommand("encañonar",function()
	callTakeHostage()
end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas Una Pistola Para hacer esto!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("Nadie cerca para tomar como rehén!")
			end
		else
			drawNativeNotification("No hay nadie cerca para tomar como rehén!")
		end
	end
end 
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Presiona [G] para liberar, [H] para matar")
