...
RegisterNetEvent('esx_towing:openTowingMenu')
AddEventHandler('esx_towing:openTowingMenu', function()
    OpenTowingMenu()
end)
...
RegisterNUICallback('ConnectRear', function(data, cb)
    ...
    if (found) and entity1 ~= veh and not Contains(Config.blacklistedClasses, GetVehicleClass(veh)) then
        ...
        if (lock == 0 or lock == 1 or lock == 7 or lock == 8) or not Config.checkForLocks then
            ...
            ShowTooltip('Este vehículo está bloqueado y por lo tanto no puede ser remolcado')
            entity2 = nil
            cb(false)
        end
    else
...
function DetachRope()
    if entity1 and entity2 then
        CloseTowingMenu()
        TriggerServerEvent('esx_towing:stopTow')
...
function AttemptAttachRope()
    if entity1 and entity2 then
        ...
        TriggerServerEvent('esx_towing:tow', veh1, veh2)
        CloseTowingMenu()
...
RegisterNetEvent('esx_towing:removeRope')
AddEventHandler('esx_towing:removeRope', function(id, veh1_, veh2_)
    for k, rope in pairs(ropes) do
        if rope.id == id then
...
RegisterNetEvent('esx_towing:makeRope')
AddEventHandler('esx_towing:makeRope', function(veh1_, veh2_, id, owner)
    for k, rope in pairs(ropes) do
        if rope.id == id then
            DeleteRope(rope.rope)
            ropes[k] = nil
