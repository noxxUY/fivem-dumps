ESX = nil
local PlayerData = {}
local reviveCost = 500000
local grandmasLocationx = 321.8329  -- coords as a float ex 112.3
local grandmasLocationy = -590.508 -- coords as a float ex 112.3
local grandmasLocationz = 42.283 -- coords as a float ex 112.3 
...
AddEventHandler('esx_grandmas:hasEnteredMarker', function(zone)
	...
	CurrentActionMsg  = 'Presiona [E] para revivir a tu compañero por $500.000'
...
function healThem()
	...
	if(closestDistance ~= -1 and closestDistance < 5) then
	    ...
	    if 	(closestPlayer) then
					...
					print("Running heal script")
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
								...
								if health == 0 then
									...
									IsBusy = true
									ESX.ShowNotification("Reviviendo Persona")
									...
									ESX.ShowNotification("Persona Revivida", GetPlayerName(closestPlayer))
									IsBusy = false
									TriggerServerEvent("esx_grandmas:moneyTake", reviveCost)
								else
									ESX.ShowNotification("No esta muerto")
								end
			...
			print("El Ciudadano Tiene Pulso Aun")
		end -- end if statement of if dead
	else
		print("No hay nadie cerca")
