    ...
    while true do
        ...
        if inTrunk then
            ...
            if DoesEntityExist(vehicle) or not IsPedDeadOrDying(PlayerPedId()) or not IsPedFatallyInjured(PlayerPedId()) then
                ...
                DrawText3D(coords, '[K] Salir del maletero')
        ...
        if DoesEntityExist(vehicle) then
            ...
            if trunk ~= -1 then
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), coords, true) <= 1.5 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicle, 5) < 0.9 then
                            DrawText3D(coords, '[K] Entrar en el maletero [H] Abrir maletero')
                            if IsControlJustReleased(0, 74) then
                                -- SetVehicleDoorOpen(vehicle, 5, false, false)
                                SetCarBootOpen(vehicle)
                            end
                        else
                            DrawText3D(coords, '[K] Ocultar en el maletero [H] Cerrar maletero')
                            if IsControlJustReleased(0, 74) then
                                SetVehicleDoorShut(vehicle, 5)
                            end
                        end
                    end
                    if IsControlJustReleased(0, 311) and not inTrunk then
                        ...
                        if DoesEntityExist(playerPed) then
                            if not IsEntityAttached(playerPed) or GetDistanceBetweenCoords(GetEntityCoords(playerPed), GetEntityCoords(PlayerPedId()), true) >= 5.0 then
                                ...
                                ESX.ShowNotification('Alguien ya está escondido en ese vehículo.!')
                            end
                        end
                    end
                end
