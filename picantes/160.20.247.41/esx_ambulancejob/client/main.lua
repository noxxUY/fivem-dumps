...
AddEventHandler('esx_ambulancejob:useItem', function(itemName)
	...
	if itemName == 'medikit' then
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			exports['mythic_notify']:DoCustomHudText('cajaverde', ('Has curado a '..GetPlayerName(closestPlayer).. ""), 7000)
			ESX.ShowNotification(_U('used_medikit'))
		end)

	elseif itemName == 'bandage' then
		ExecuteCommand "me utilizó un/a Vendaje Farmaceútico"
		local lib, anim = 'move_m@_idles@shake_off', 'shakeoff_1' -- TODO better animations
		local playerPed = PlayerPedId()
		TriggerEvent("mythic_progressbar:client:progress" , {
			name            = "bandage",
			duration        = 2000,
			label           = "Utilizando Vendaje Farmaceútico",
			useWhileDead    = false,
			canCancel       = true,
			controlDisables = {
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and isDead do
			...
			if not Config.EarlyRespawnFine then
				DrawGenericTextThisFrame()
				BeginTextCommandDisplayText('STRING')
				AddTextComponentSubstringPlayerName("Manten [C] para respawnear en la ciudad")
				EndTextCommandDisplayText(0.5, 0.83)
				DrawGenericTextThisFrame()
				BeginTextCommandDisplayText('STRING')
				AddTextComponentSubstringPlayerName("Manten [P] para respawnear en Paleto")
				EndTextCommandDisplayText(0.5, 0.86)
				DrawGenericTextThisFrame()
				BeginTextCommandDisplayText('STRING')
				AddTextComponentSubstringPlayerName("Manten [S] para respawnear en Sandy")
				...
				if IsControlPressed(0, 79) and timeHeld > 120 then
				selectedOption = "C"
				RemoveItemsAfterRPDeath()
				break
				end
				if IsControlPressed(0, 199) and timeHeld > 120 then
				selectedOption = "P"
				RemoveItemsAfterRPDeath()
				break
				end
				if IsControlPressed(0, 31) and timeHeld > 120 then
				selectedOption = "S"
				RemoveItemsAfterRPDeath()
				break
				end
			...
			if IsControlPressed(0, 31) or IsControlPressed(0, 199) or IsControlPressed(0, 79) then
				timeHeld = timeHeld + 1
			else
				timeHeld = 0
			end
...
function RemoveItemsAfterRPDeath()
	...
	Citizen.CreateThread(function()
		...
		ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
			...
			if selectedOption == "C" then
				RespawnCoords = Config.RespawnPoints[1].coords
				ClosestHospital = Config.RespawnPoints[1]
			elseif selectedOption == "P" then
				RespawnCoords = Config.RespawnPoints[3].coords
				ClosestHospital = Config.RespawnPoints[3]
			elseif selectedOption == "S" then
				RespawnCoords = Config.RespawnPoints[2].coords
				ClosestHospital = Config.RespawnPoints[2]
			end
