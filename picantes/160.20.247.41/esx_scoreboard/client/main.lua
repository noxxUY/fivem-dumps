...
function UpdatePlayerTable(connectedPlayers)
	...
	local ems, police, psg, michelin, mechanic, aca, players = 0, 0, 0, 0, 0, 0, 0
	...
	for k,v in pairs(connectedPlayers) do
		...
		elseif v.job == 'mechanic' then
			mechanic = mechanic + 1
		elseif v.job == 'michelin' then
			michelin = michelin + 1
		elseif v.job == 'aca' then
			aca = aca + 1
		end
	end
	...
	SendNUIMessage({
		...
		jobs   = {ems = ems, police = police, psg = psg, michelin = michelin, mechanic = mechanic, aca = aca, player_count = players}
	})
