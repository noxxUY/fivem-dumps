	...
	ESX.TriggerServerCallback('esx_thief:getOtherPlayerData', function(data)
		...
		if Config.EnableCash then
			table.insert(elements, {
				label = (('%s: 「<span style="color:yellow;">$%s</span>」'):format(_U('cash'), ESX.Math.GroupDigits(data.money))),
				value = 'money',
				type = 'item_money',
				amount = data.money,
				haveImage = true,
		...
		if Config.EnableBlackMoney then
			...
			table.insert(elements, {
				label = (('%s: 「<span style="color:yellow;">$%s</span>」'):format(_U('black_money'), ESX.Math.GroupDigits(blackMoney))),
				value = 'black_money',
				type = 'item_account',
				amount = blackMoney,
				haveImage = true,
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(1, 74) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and IsPedOnFoot(ped) then
