...
function DrawText3Ds(x , y , z , text)
	...
	DrawRect(_x , _y + 0.0125 , 0.015 + factor , 0.03 , 28 , 28 , 28 , 100)
end
...
AddEventHandler('fuel:refuelFromPump' , function(pumpObject , ped , vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			if Config.UseESX then
				extraString  = "%\n~w~" .. Config.Strings.TotalCost .. ": ~b~$" .. Round(currentCost , 1)
				extraEssence = "\n~w~Nivel de Nafta: ~b~" .. Round(currentFuel , 1)
			end
			
			ShowHelpNotif("¡Presiona ~INPUT_TALK~ para anular la carga!" .. extraEssence .. extraString)
		else
			DrawText3Ds(vehicleCoords.x , vehicleCoords.y , vehicleCoords.z + 0.5 , Config.Strings.CancelFuelingJerryCan .. "\nBidón de Nafta: ~b~" .. Round(GetAmmoInPedWeapon(ped , 883325847) / 4500 * 100 , 1) .. "% | Vehículo: " .. Round(currentFuel , 1) .. "%")
		end
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped) , -1) == ped then
				...
				DrawText3Ds(playerX , playerY , playerZ , '¡Bájate del ~b~vehículo~w~ para cargar combustible!')
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle , -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								DrawText3Ds(stringCoords.x , stringCoords.y , stringCoords.z + 1.2 , '¡Presiona ~b~E~w~ para cargar combustible!')
								DrawText3Ds(stringCoords.x , stringCoords.y , stringCoords.z + 1.0 , 'Nivel actual de combustible: ~b~' .. GetVehicleFuelLevel(vehicle) .. ' ')
								...
								DrawText3Ds(stringCoords.x , stringCoords.y , stringCoords.z + 1.2 , '¡No tienes suficiente ~b~dinero~w~!')
							end
						elseif not canFuel then
							DrawText3Ds(stringCoords.x , stringCoords.y , stringCoords.z + 1.2 , Config.Strings.JerryCanEmpty)
						else
							DrawText3Ds(stringCoords.x , stringCoords.y , stringCoords.z + 1.2 , '¡El tanque de tu ~b~vehículo~w~ está ~b~lleno~w~!')
						end
					end
...
function CreateBlip(coords)
	...
	AddTextComponentString("Estación de Servicio")
