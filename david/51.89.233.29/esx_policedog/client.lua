    ...
    if PlayerData.job.name == Config.Job then
        ...
        ESX.UI.Menu.Open(
                'default', GetCurrentResourceName(), 'buy_storage',
            {
                ...
                align = 'top-left',
                elements = {{label = Strings['take_out_remove'], value = 'take_out_remove'}, {label = Strings['sit_stand'], value = 'sit_stand'}, {label = Strings['search_drugs'], value = 'search_drugs'}, {label = Strings['attack_closest'], value = 'attack_closest'}, {label = Strings['get_ped_down'], value = 'vehicle'}, {label = Strings['get_ped_inside'], value = 'vehicle'}}
            },
            function(data, menu)
		        ...
		        elseif data.current.value == 'vehicle' then
			        ...
			        local coords2  = GetEntityCoords(policeDog)
				        ...
				        if IsPedSittingInAnyVehicle(playerPed) then
					        if distance < 8 then
						        ...
						        if IsVehicleSeatFree(vehicle, 1) then
							        SetPedIntoVehicle(policeDog, vehicle, 1)
							        isInVehicle = true
						        elseif IsVehicleSeatFree(vehicle, 2) then
							        isInVehicle = true
							        SetPedIntoVehicle(policeDog, vehicle, 2)
						        elseif IsVehicleSeatFree(vehicle, 0) then
							        isInVehicle = true
							        SetPedIntoVehicle(policeDog, vehicle, 0)
						        end
						        menu.close()
					        else
						        ESX.ShowNotification(Strings['toofarfromcar'])
					        end

				        else
					        ESX.ShowNotification(Strings['youneedtobeincar'])
				        end
			        else
				        if not IsPedSittingInAnyVehicle(playerPed) then
					        SetEntityCoords(policeDog, coords,1,0,0,1)
					        Citizen.Wait(100)
					        detached()
					        isInVehicle = false
					        menu.close()
				        else
					        ESX.ShowNotification(Strings['yourstillinacar'])
                        end
                    end
                end
