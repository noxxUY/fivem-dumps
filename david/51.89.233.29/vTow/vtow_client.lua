...
RegisterCommand("tow", function(source, args)
    ...
    if towing then
        ...
        elseif not towSetupMode then
            ShowNotification("Ve hacia tu ~r~plataforma ~s~y cuando veas el", "~y~marcador ~s~amarillo, pulsa ~r~~h~INICIO~h~", "~s~para selecionar el vehículo.")
            towSetupMode = true
            towtruck = nil
            target = nil
            helpstate = 1
            ...
            if helpstate ~= 0 then
                ...
                if hit and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if (IsControlJustPressed(0, 213)) then
                        if helpstate == 1 then
                            ShowNotification("Ve hacia el ~o~vehículo que quieras~s~ y cuando veas el", "~y~marcador ~s~amarillo, pulsa ~r~~h~INICIO~h~", "~s~para confirmar el vehículo.")
                            towtruck = veh
                            helpstate = 2
                        elseif helpstate == 2 and veh ~= towtruck then
                            ShowNotification("El vehículo ha sido confirmado.", "~r~/tow ~s~para subirlo, o pulsa", "~r~~h~INICIO~h~~s~ para cancelar.")
                            target = veh
                            helpstate = 3
                        end
                    end
