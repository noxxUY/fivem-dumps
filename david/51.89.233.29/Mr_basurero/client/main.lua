...
AddEventHandler('esx_garbagejob:startpayrequest', function(platenumber, amount)
	if isInService then
		if platenumb == platenumber then
			TriggerServerEvent('esx_garbagejob:pay132', amount)
			platenumb = nil
		end
	end
end)
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							if iscurrentboss then
								...
								trashcollection = false
								truckdeposit = false
								ESX.ShowNotification("¡Recogida finalizada, vuelve al camion!")
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Tirar la bolsa en el camion.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Limpiar contenedor", 1.0)		
					else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Recogida finalizada, espera en el camion...", 1.0)		
					end
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Recoge la bolsa ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
