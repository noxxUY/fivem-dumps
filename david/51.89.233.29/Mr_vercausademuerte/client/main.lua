  ...
  function Start(ped)
	  ...
	  while checking do
		  ...
		  if distance < 2.0 and PlayerData.job.name == 'ambulance' then
			  DrawText3D(x,y,z, 'Pulsa [~g~E~s~] para gestion de salud', 0.4)
  ...
  function Notification(x,y,z)
	  ...
	  while (timestamp + 4500) > GetGameTimer() do
		  Citizen.Wait(0)
		  DrawText3D(x, y, z, 'El daño parece ocurrir aquí', 0.4)
		  checking = false
	  end
  end
  ...
  function OpenDeathMenu(player)
	  ...
	  table.insert(elements, {label = 'Identificar causa', value = 'deathcause'})
	  table.insert(elements, {label = 'Identificar cuando ha sido la muerte', value = 'damage'})
	  ...
	  function(data, menu)
		  ...
		  if ac == 'damage' then
			  ...
			  if success then
				  ...
				  Notify('No se pudo identificar cuando se produjo el daño')
			  end
		  end
