...
function OpenCloakroomMenu()
	...
	elseif grade == 'intendent' then
		table.insert(elements, {label = _U('sheriff_wear'), value = 'intendent_wear'})
	elseif grade == 'lieutenant' then
		table.insert(elements, {label = _U('sheriff_wear'), value = 'lieutenant_wear'})
	elseif grade == 'chef' then
		table.insert(elements, {label = _U('sheriff_wear'), value = 'chef_wear'})
	...
	ESX.TriggerServerCallback('esx_sheriffjob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill132', GetPlayerServerId(player), 'society_sheriff', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('esx_billing:sendBill132', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end
