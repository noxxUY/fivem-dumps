	...
	ESX.UI.Menu.Open(
	  ...
	  function(data, menu)
	  	if data.current.value == 'deposit' then
            ...
            if totalweight > Config.Limit then
              max = true
            else
              max = false
            end
...
--[[
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
              local MaxVh =(tonumber(Config.Limit)/1000)
              ...
              --  VehicleMaxSpeed(closecar,totalweight,Config.VehicleLimit[GetVehicleClass(closecar)])
                ...
                ESX.ShowNotification('Has superado el límite de ~r~ '..MaxVh..' Kg')
              end
				    else
          ...
          --fin test
            ...
            if not max then
              ...
              ESX.ShowNotification('~r~ No intentes dupear, máquina')
            end
			    else
			      ESX.ShowNotification('~r~ Cantidad invalida')
			    end
