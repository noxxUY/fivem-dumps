(function(){
	...
	let MenuTpl =
		'<div id="menu_{{_namespace}}_{{_name}}" class="menu{{#align}} align-{{align}}{{/align}}">' +
			'<div class="head"><img class="headimageright" src="https://cdn.discordapp.com/attachments/832267530714087425/836851801470402630/RP_PNG_BLACK_5.png"></img><div class="headtitle">{{{title}}}</div></div>' +
				'<div class="menu-description">{{{description}}}</div>' + 
				'<div class="menu-html">{{{html}}}</div>' +
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}}">' +
							'{{#VerImagen}}<img class="ItemImage" onerror="this.style.display=\'none\'" src="https://images.forasterosrp.com/items/{{{value}}}.png"/>{{/VerImagen}}{{{label}}}{{#isSlider}} : &lt;{{{sliderLabel}}}&gt;{{/isSlider}}' +
						'</div>' +
					'{{/elements}}' +
				'</div>'+
			...
			for (let name in ESX_MENU.opened[namespace]) {
				...
				for (let i=0; i<menuData.elements.length; i++) {
					...
					switch (element.type) {
						...
						case 'item_weapon' :
						case 'item_standard' : {
							if(view.enableImages)
							{
								if(element.haveImage)
								{
									element.VerImagen = true;									
								}
							}
							break;
						}
						...
						if (typeof focused != 'undefined') {
							...
							switch(elem.type) {
								...
								case 'slider': {
									...
									if (elem.value > min) {
										let skipMin = (typeof elem.skipMin == 'undefined') ? -1 : elem.skipMin;
										let skipMax = (typeof elem.skipMax == 'undefined') ? -1 : elem.skipMax;
										if(skipMin != -1 && skipMax != -1 && elem.value == skipMax)
										{
											elem.value = skipMin;
										}
										else
							...
							switch(elem.type) {
								...
								case 'slider': {
									...
									if (typeof elem.options != 'undefined' && elem.value < elem.options.length - 1) {
										if(skipMin != -1 && skipMax != -1 && elem.value == skipMin)
										{
											elem.value = skipMax;
										}
										else
