	...
	if DoesEntityExist(vehicle) then
		if IsPedInAnyVehicle(player) or IsPedInAnyHeli(player) then
			if vehicleplate ~= nil and clostestvehicleplate ~= nil then
				if not table.empty(VehicleList) then
					...
					if isvehiclefound then
						if (islocked == 1)then
							...
							TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, "alarm", 1.0)
							Citizen.CreateThread(function()
						...
						if (islocked == 1)then
							SetVehicleDoorsLocked(vehicle, 2)
							Notify("Cogiste las llaves de ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))))
				...
				Notify("~r~No hay vehículos cerca de usted.!")
			end
		else
			if not table.empty(VehicleList) then
