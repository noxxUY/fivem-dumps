    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Saliste de la tienda, tu changito esta vacio.!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("No tenes nada en el changito!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            ...
            {label = 'Monto a pagar: <span style="color:green">$' .. payAmount ..'</span>'},
        }
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("No tenes suficiente dinero!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Poner " .. data.current.value .. " piezas " .. data.current["realLabel"] .. " en el changito", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        local elements = {
            {label = 'Monto a pagar: <span style="color:green">$' .. payAmount},
        }
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
        ...
        table.insert(elements, {label = '<span style="color:red">Vaciar Changito', value = "empty"})
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Sacar todo del changito.", "error", 2500)
                end
                if data.current.value == "item_menu" then
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        {
                            title    = "Changito - " .. shopItem["label"] .. " - " .. shopItem["amount"] .. "pcs",
                            align    = 'center',
                            elements = {
                                {label = shopItem["label"] .. " - $" .. shopItem["price"] * shopItem["amount"]},
                                {label = '<span style="color:red">Elminar Objeto</span>', value = "deleteItem"},
                            },
                        },
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Sacaste " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " del changito", "alert", 2500)
