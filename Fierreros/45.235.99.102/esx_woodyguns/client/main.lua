			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'woodyguns')

						TriggerServerEvent('esx_service:disableService', 'woodyguns')
						TriggerEvent('esx_woodygunsjob:updateBlip')
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_woodyguns', owner = station})
...
function OpenwoodygunsActionsMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-woodyguns", title = "woodyguns"},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_woodygunsjob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_woodygunsjob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_woodygunsjob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_woodygunsjob:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_woodygunsjob:getOtherPlayerData', function(data)
		local elements = {
	...
	ESX.TriggerServerCallback('esx_woodygunsjob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('esx_woodygunsjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_woodygunsjob:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_woodygunsjob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('esx_woodygunsjob:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_woodygunsjob:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('esx_woodygunsjob:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('esx_woodygunsjob:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_woodygunsjob:getStockItems', function(items)
		local elements = {
			{unselectable = true, icon = "fas fa-box", title = TranslateCap('woodyguns_stock')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_woodygunsjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_woodygunsjob:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_woodygunsjob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'woodyguns' then
		Wait(1000)
		TriggerServerEvent('esx_woodygunsjob:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = TranslateCap('phone_woodyguns'),
		number     = 'woodyguns',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'woodyguns' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_woodygunsjob:hasEnteredMarker', function(station, part, partNum)
	if part == 'Armory' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = TranslateCap('open_armory')
		CurrentActionData = {station = station}
...
AddEventHandler('esx_woodygunsjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_woodygunsjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'woodyguns' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_woodygunsjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_woodygunsjob:handcuff')
AddEventHandler('esx_woodygunsjob:handcuff', function()
...
RegisterNetEvent('esx_woodygunsjob:unrestrain')
AddEventHandler('esx_woodygunsjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('esx_woodygunsjob:drag')
AddEventHandler('esx_woodygunsjob:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('esx_woodygunsjob:putInVehicle')
AddEventHandler('esx_woodygunsjob:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_woodygunsjob:OutVehicle')
AddEventHandler('esx_woodygunsjob:OutVehicle', function()
...
CreateThread(function()
	while true do
		local Sleep = 1500
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'woodyguns' then
			Sleep = 500
			local playerPed = PlayerPedId()
			...
			for k,v in pairs(Config.woodygunsStations) do
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_woodygunsjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_woodygunsjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('esx_woodygunsjob:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('esx_woodygunsjob:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("woodyguns:interact", "(ESX woodygunsJob) Interact", "keyboard", "E", function()
	if not CurrentAction then 
		return 
	end

	if not ESX.PlayerData.job or (ESX.PlayerData.job and not ESX.PlayerData.job.name == 'woodyguns') then
		return
	end
...
ESX.RegisterInput("woodyguns:quickactions", "(ESX woodygunsJob) Quick Actions", "keyboard", "F6", function()
	if not ESX.PlayerData.job or (ESX.PlayerData.job.name ~= 'woodyguns') or isDead then
		return
	end
...
RegisterNetEvent('esx_woodygunsjob:updateBlip')
AddEventHandler('esx_woodygunsjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'woodyguns' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'woodyguns' then
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	isDead = false
	TriggerEvent('esx_woodygunsjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_woodygunsjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_woodygunsjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'woodyguns')
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'woodyguns' then
