			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'lamafilia')

						TriggerServerEvent('esx_service:disableService', 'lamafilia')
						TriggerEvent('esx_lamafiliajob:updateBlip')
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_lamafilia', owner = station})
...
function OpenlamafiliaActionsMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-lamafilia", title = "lamafilia"},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_lamafiliajob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_lamafiliajob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_lamafiliajob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_lamafiliajob:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_lamafiliajob:getOtherPlayerData', function(data)
		local elements = {
	...
	ESX.TriggerServerCallback('esx_lamafiliajob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('esx_lamafiliajob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_lamafiliajob:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_lamafiliajob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('esx_lamafiliajob:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_lamafiliajob:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('esx_lamafiliajob:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('esx_lamafiliajob:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_lamafiliajob:getStockItems', function(items)
		local elements = {
			{unselectable = true, icon = "fas fa-box", title = TranslateCap('lamafilia_stock')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_lamafiliajob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_lamafiliajob:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_lamafiliajob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'lamafilia' then
		Wait(1000)
		TriggerServerEvent('esx_lamafiliajob:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = TranslateCap('phone_lamafilia'),
		number     = 'lamafilia',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'lamafilia' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_lamafiliajob:hasEnteredMarker', function(station, part, partNum)
	if part == 'Armory' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = TranslateCap('open_armory')
		CurrentActionData = {station = station}
...
AddEventHandler('esx_lamafiliajob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_lamafiliajob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'lamafilia' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_lamafiliajob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_lamafiliajob:handcuff')
AddEventHandler('esx_lamafiliajob:handcuff', function()
...
RegisterNetEvent('esx_lamafiliajob:unrestrain')
AddEventHandler('esx_lamafiliajob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('esx_lamafiliajob:drag')
AddEventHandler('esx_lamafiliajob:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('esx_lamafiliajob:putInVehicle')
AddEventHandler('esx_lamafiliajob:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_lamafiliajob:OutVehicle')
AddEventHandler('esx_lamafiliajob:OutVehicle', function()
...
CreateThread(function()
	while true do
		local Sleep = 1500
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'lamafilia' then
			Sleep = 500
			local playerPed = PlayerPedId()
			...
			for k,v in pairs(Config.lamafiliaStations) do
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_lamafiliajob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_lamafiliajob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('esx_lamafiliajob:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('esx_lamafiliajob:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("lamafilia:interact", "(ESX lamafiliaJob) Interact", "keyboard", "E", function()
	if not CurrentAction then 
		return 
	end

	if not ESX.PlayerData.job or (ESX.PlayerData.job and not ESX.PlayerData.job.name == 'lamafilia') then
		return
	end
	...
	elseif CurrentAction == 'menu_boss_actions' then
		ESX.CloseContext()
		TriggerEvent('esx_society:openBossMenu', 'lamafilia', function(data, menu)
			menu.close()
...
ESX.RegisterInput("lamafilia:quickactions", "(ESX lamafiliaJob) Quick Actions", "keyboard", "F6", function()
	if not ESX.PlayerData.job or (ESX.PlayerData.job.name ~= 'lamafilia') or isDead then
		return
	end
...
RegisterNetEvent('esx_lamafiliajob:updateBlip')
AddEventHandler('esx_lamafiliajob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'lamafilia' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'lamafilia' then
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	isDead = false
	TriggerEvent('esx_lamafiliajob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_lamafiliajob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_lamafiliajob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'lamafilia')
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'lamafilia' then
