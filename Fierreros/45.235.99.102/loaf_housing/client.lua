    ...
    while true do
        ...
        for k, v in pairs(Config.Houses) do
            if v['garage'] ~= nil then 
                if #(GetEntityCoords(PlayerPedId()) - v['garage'].xyz) <= 3 then 
                    if OwnedHouse.houseId == k then 
                        if IsPedInAnyVehicle(PlayerPedId(), false) then 
                            msec = 0
                            ESX.Game.Utils.DrawText3D(v['garage'].xyz, '[~b~Garaje~w~]\nPresiona ~b~E~w~ para guardar.', 0.7)
                            if IsControlJustPressed(0, 38) then 
                                
                                local Garage = 'Propiedad: '..OwnedHouse.houseId 
                                TriggerEvent('garage:house:save', Garage)
                            end
                        end
                    end 
                end
                ...
                while Vdist2(GetEntityCoords(PlayerPedId()), v['door']) <= 2.5 do
                    ...
                    if IsControlJustReleased(0, 38) then
                        if OwnedHouse.houseId == k then
                            ...
                            if Config.Houses[OwnedHouse.houseId]['garage'] ~= nil then
                            ...
                            ESX.UI.Menu.Open(
                                ...
                                function(data, menu)
                                    ...
                                    elseif data.current.value == 'garage' then
                                        menu.close()
                                        local Garage = 'Propiedad: '..OwnedHouse.houseId 
                                        TriggerEvent('garage:house:open', Garage, Config.Houses[OwnedHouse.houseId]['garage'])
                                        --[[
                        ...
                        while #(GetEntityCoords(PlayerPedId()) - coords) <= 45.0 and IsPedInAnyVehicle(PlayerPedId(), false) and Config.EnableGarage do
                            ...
                            if #(GetEntityCoords(PlayerPedId()) - coords) <= 5.0 and IsPedInAnyVehicle(PlayerPedId(), false) and Config.EnableGarage then
                                ...
                                if IsControlJustReleased(0, 38) then
                                   ...
                                   TriggerEvent('loaf_garage:store')
                                end
                            end
...
HelpText = function(msg, coords)
    if not coords or not Config.Use3DText then
        ...
        DrawText3D(coords, string.gsub(msg, "~b~E~w~", "~b~[~w~E~b~]~w~"), 0.35)
    end
end
