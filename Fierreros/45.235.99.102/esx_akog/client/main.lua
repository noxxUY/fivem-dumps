			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'akog')

						TriggerServerEvent('esx_service:disableService', 'akog')
						TriggerEvent('esx_akogjob:updateBlip')
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_akog', owner = station})
...
function OpenakogActionsMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-akog", title = "akog"},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_akogjob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_akogjob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_akogjob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_akogjob:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_akogjob:getOtherPlayerData', function(data)
		local elements = {
	...
	ESX.TriggerServerCallback('esx_akogjob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('esx_akogjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_akogjob:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_akogjob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('esx_akogjob:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_akogjob:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('esx_akogjob:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('esx_akogjob:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_akogjob:getStockItems', function(items)
		local elements = {
			{unselectable = true, icon = "fas fa-box", title = TranslateCap('akog_stock')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_akogjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_akogjob:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_akogjob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'akog' then
		Wait(1000)
		TriggerServerEvent('esx_akogjob:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'akog' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_akogjob:hasEnteredMarker', function(station, part, partNum)
	if part == 'Armory' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = TranslateCap('open_armory')
		CurrentActionData = {station = station}
...
AddEventHandler('esx_akogjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_akogjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'akog' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_akogjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_akogjob:handcuff')
AddEventHandler('esx_akogjob:handcuff', function()
...
RegisterNetEvent('esx_akogjob:unrestrain')
AddEventHandler('esx_akogjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('esx_akogjob:drag')
AddEventHandler('esx_akogjob:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('esx_akogjob:putInVehicle')
AddEventHandler('esx_akogjob:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_akogjob:OutVehicle')
AddEventHandler('esx_akogjob:OutVehicle', function()
...
CreateThread(function()
	while true do
		local Sleep = 1500
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'akog' then
			Sleep = 500
			local playerPed = PlayerPedId()
			...
			for k,v in pairs(Config.akogStations) do
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_akogjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_akogjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('esx_akogjob:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('esx_akogjob:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("akog:interact", "(ESX akogJob) Interact", "keyboard", "E", function()
	if not CurrentAction then 
		return 
	end

	if not ESX.PlayerData.job or (ESX.PlayerData.job and not ESX.PlayerData.job.name == 'akog') then
		return
	end
	...
	elseif CurrentAction == 'menu_boss_actions' then
		ESX.CloseContext()
		TriggerEvent('esx_society:openBossMenu', 'akog', function(data, menu)
			menu.close()
...
ESX.RegisterInput("akog:quickactions", "(ESX akogJob) Quick Actions", "keyboard", "F6", function()
	if not ESX.PlayerData.job or (ESX.PlayerData.job.name ~= 'akog') or isDead then
		return
	end
...
RegisterNetEvent('esx_akogjob:updateBlip')
AddEventHandler('esx_akogjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'akog' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'akog' then
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	isDead = false
	TriggerEvent('esx_akogjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_akogjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_akogjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'akog')
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'akog' then
