...
Citizen.CreateThread(function()
    while isLoading == true do 
        ...
        if ESX.IsPlayerLoaded(PlayerId) and playerData.job.name == "police" then 
            ...
            TriggerServerEvent("sp_police-tracker:getActivePlates")
            isLoading = false
        end
    end 
...
RegisterNetEvent("sp_police-tracker:updateTimer")
AddEventHandler("sp_police-tracker:updateTimer", function(plate)
    time_out[plate] = time_out[nil]
end)

RegisterNetEvent("sp_police-tracker:updateActivePlate")
AddEventHandler("sp_police-tracker:updateActivePlate", function(plate)
...
RegisterNetEvent("sp_police-tracker:getActivePlates")
AddEventHandler("sp_police-tracker:getActivePlates", function(plates)
    time_out = plates
    for v,k in pairs(time_out) do
        checkVehicle(v)
    end
...
RegisterNetEvent('sp_police-tracker:plate')
AddEventHandler('sp_police-tracker:plate', function(plate)
    checkVehicle(plate)
end)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == data.plate then 
            ...
            if nCheck >= Config.maxTracker then 
                ...
                SetNuiFocus(false)
                TriggerServerEvent("sp_police-tracker", data.plate)
            end
        else 
            miss = miss + 1 
        end 
...
RegisterNUICallback("removeSearch", function(data, cb)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == data.plate then 
            TriggerServerEvent("sp_police-tracker:removeActivePlate", data.plate)
            SendNUIMessage({
                type = "ui",
                display = false
              })
...
function checkVehicle(plate)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == plate then 
            TriggerServerEvent("sp_police-tracker:setActivePlates", plate)
...
function triggerTimer(plate)
    TriggerServerEvent("sp_police-tracker:triggerTimer", plate)
end
        ...
        Citizen.CreateThread(function()
            while time_out[plate] == false do
                ...
                if DoesEntityExist(vehicle) then 
                    ...
                    time_out[plate] = time_out[nil]
                    TriggerServerEvent("sp_police-tracker:removeActivePlate", plate)
