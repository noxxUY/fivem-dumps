	...
	if isDead and spam then
		ESX.TriggerServerCallback('hhfw:docOnline', function(EMSOnline, hasEnoughMoney)
			...
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				...
				exports['mythic_notify']:SendAlert('inform', 'El medico esta yendo!', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
			else
				if EMSOnline > Config.Doctor then
					exports['mythic_notify']:SendAlert('error', 'Ya hay SAME Online!', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
				elseif not hasEnoughMoney then
					exports['mythic_notify']:SendAlert('error', 'No tienes suficiente dinero', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
				else
					exports['mythic_notify']:SendAlert('inform', 'Espera al SAME, esta en camino', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
				end	
			end
		end)
	else
...
function DoctorNPC()
	...
	TriggerEvent("mythic_progbar:client:progress", {
        ...
        label = "El doctor te esta reanimando",
        useWhileDead = true,
        canCancel = false,
        controlDisables = {
	...
	exports['mythic_notify']:SendAlert('inform', 'El medico ya te reanimo', 2500, { ['background-color'] = '#ffffff', ['color'] = '#000000' })
