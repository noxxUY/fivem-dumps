			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'thepeakyblinders')

						TriggerServerEvent('esx_service:disableService', 'thepeakyblinders')
						TriggerEvent('esx_thepeakyblindersjob:updateBlip')
						ESX.ShowNotification(TranslateCap('service_out'))
					end
				end, 'thepeakyblinders')
			end
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_thepeakyblinders', owner = station})
...
function OpenthepeakyblindersActionsMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-thepeakyblinders", title = "thepeakyblinders"},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_thepeakyblindersjob:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_thepeakyblindersjob:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_thepeakyblindersjob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_thepeakyblindersjob:OutVehicle', GetPlayerServerId(closestPlayer))
				...
				OpenthepeakyblindersActionsMenu()
			end)
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_thepeakyblindersjob:getOtherPlayerData', function(data)
		local elements = {
	...
	ESX.TriggerServerCallback('esx_thepeakyblindersjob:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('esx_thepeakyblindersjob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_thepeakyblindersjob:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_thepeakyblindersjob:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('esx_thepeakyblindersjob:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_thepeakyblindersjob:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('esx_thepeakyblindersjob:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('esx_thepeakyblindersjob:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_thepeakyblindersjob:getStockItems', function(items)
		local elements = {
			{unselectable = true, icon = "fas fa-box", title = TranslateCap('thepeakyblinders_stock')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_thepeakyblindersjob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_thepeakyblindersjob:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_thepeakyblindersjob:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'thepeakyblinders' then
		Wait(1000)
		TriggerServerEvent('esx_thepeakyblindersjob:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = TranslateCap('phone_thepeakyblinders'),
		number     = 'thepeakyblinders',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'thepeakyblinders' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_thepeakyblindersjob:hasEnteredMarker', function(station, part, partNum)
	if part == 'Armory' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = TranslateCap('open_armory')
		CurrentActionData = {station = station}
...
AddEventHandler('esx_thepeakyblindersjob:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_thepeakyblindersjob:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'thepeakyblinders' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_thepeakyblindersjob:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_thepeakyblindersjob:handcuff')
AddEventHandler('esx_thepeakyblindersjob:handcuff', function()
...
RegisterNetEvent('esx_thepeakyblindersjob:unrestrain')
AddEventHandler('esx_thepeakyblindersjob:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('esx_thepeakyblindersjob:drag')
AddEventHandler('esx_thepeakyblindersjob:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('esx_thepeakyblindersjob:putInVehicle')
AddEventHandler('esx_thepeakyblindersjob:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_thepeakyblindersjob:OutVehicle')
AddEventHandler('esx_thepeakyblindersjob:OutVehicle', function()
...
CreateThread(function()
	while true do
		local Sleep = 1500
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'thepeakyblinders' then
			Sleep = 500
			local playerPed = PlayerPedId()
			...
			for k,v in pairs(Config.thepeakyblindersStations) do
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_thepeakyblindersjob:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_thepeakyblindersjob:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('esx_thepeakyblindersjob:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('esx_thepeakyblindersjob:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("thepeakyblinders:interact", "(ESX thepeakyblindersJob) Interact", "keyboard", "E", function()
	if not CurrentAction then 
		return 
	end

	if not ESX.PlayerData.job or (ESX.PlayerData.job and not ESX.PlayerData.job.name == 'thepeakyblinders') then
		return
	end
	...
	elseif CurrentAction == 'menu_boss_actions' then
		ESX.CloseContext()
		TriggerEvent('esx_society:openBossMenu', 'thepeakyblinders', function(data, menu)
			menu.close()
...
ESX.RegisterInput("thepeakyblinders:quickactions", "(ESX thepeakyblindersJob) Quick Actions", "keyboard", "F6", function()
	if not ESX.PlayerData.job or (ESX.PlayerData.job.name ~= 'thepeakyblinders') or isDead then
		return
	end
...
RegisterNetEvent('esx_thepeakyblindersjob:updateBlip')
AddEventHandler('esx_thepeakyblindersjob:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'thepeakyblinders' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'thepeakyblinders' then
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	isDead = false
	TriggerEvent('esx_thepeakyblindersjob:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_thepeakyblindersjob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_thepeakyblindersjob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'thepeakyblinders')
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'thepeakyblinders' then
