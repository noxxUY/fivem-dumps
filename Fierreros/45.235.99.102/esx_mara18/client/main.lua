			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'mara18')

						TriggerServerEvent('esx_service:disableService', 'mara18')
						TriggerEvent('esx_mara18job:updateBlip')
...
function OpenArmoryMenu(station)
	...
	if Config.OxInventory then
		exports.ox_inventory:openInventory('stash', {id = 'society_mara18', owner = station})
...
function Openmara18ActionsMenu()
	local elements = {
		{unselectable = true, icon = "fas fa-mara18", title = "mara18"},
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_mara18job:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_mara18job:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_mara18job:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_mara18job:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_mara18job:getOtherPlayerData', function(data)
		local elements = {
	...
	ESX.TriggerServerCallback('esx_mara18job:getOtherPlayerData', function(data)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			if data.current.value then
				TriggerServerEvent('esx_mara18job:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle(elementF)
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		if not data.value or length < 2 or length > 8 then
			...
			ESX.TriggerServerCallback('esx_mara18job:getVehicleInfos', function(retrivedInfo)
				local elements = {
					{unselectable = true, icon = "fas fa-car", title = element.title},
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_mara18job:getOtherPlayerData', function(playerData)
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			TriggerServerEvent('esx_mara18job:message', GetPlayerServerId(player), TranslateCap('license_revoked', data.current.label))
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_mara18job:getArmoryWeapons', function(weapons)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			local data = {current = element}
			ESX.TriggerServerCallback('esx_mara18job:removeArmoryWeapon', function()
				ESX.CloseContext()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.OpenContext("right", elements, function(menu,element)
		local data = {current = element}
		ESX.TriggerServerCallback('esx_mara18job:addArmoryWeapon', function()
			ESX.CloseContext()
			OpenPutWeaponMenu()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_mara18job:getStockItems', function(items)
		local elements = {
			{unselectable = true, icon = "fas fa-box", title = TranslateCap('mara18_stock')}
		}
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_mara18job:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_mara18job:getPlayerInventory', function(inventory)
		local elements = {
		...
		ESX.OpenContext("right", elements, function(menu,element)
			...
			ESX.OpenContext("right", elements2, function(menu2,element2)
				...
				if not count then
					...
					ESX.CloseContext()
					TriggerServerEvent('esx_mara18job:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	if job.name == 'mara18' then
		Wait(1000)
		TriggerServerEvent('esx_mara18job:forceBlip')
	end
end)
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = TranslateCap('phone_mara18'),
		number     = 'mara18',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mara18' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_mara18job:hasEnteredMarker', function(station, part, partNum)
	if part == 'Armory' then
		CurrentAction     = 'menu_armory'
		CurrentActionMsg  = TranslateCap('open_armory')
		CurrentActionData = {station = station}
...
AddEventHandler('esx_mara18job:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_mara18job:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mara18' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_mara18job:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_mara18job:handcuff')
AddEventHandler('esx_mara18job:handcuff', function()
...
RegisterNetEvent('esx_mara18job:unrestrain')
AddEventHandler('esx_mara18job:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
RegisterNetEvent('esx_mara18job:drag')
AddEventHandler('esx_mara18job:drag', function(copId)
	if isHandcuffed then
...
RegisterNetEvent('esx_mara18job:putInVehicle')
AddEventHandler('esx_mara18job:putInVehicle', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_mara18job:OutVehicle')
AddEventHandler('esx_mara18job:OutVehicle', function()
...
CreateThread(function()
	while true do
		local Sleep = 1500
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mara18' then
			Sleep = 500
			local playerPed = PlayerPedId()
			...
			for k,v in pairs(Config.mara18Stations) do
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_mara18job:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_mara18job:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
		...
		local Sleep = 1500
			...
			if closestDistance ~= -1 and closestDistance <= 3.0 then
				if LastEntity ~= closestEntity then
					TriggerEvent('esx_mara18job:hasEnteredEntityZone', closestEntity)
					LastEntity = closestEntity
				end
			else
				if LastEntity then
					TriggerEvent('esx_mara18job:hasExitedEntityZone', LastEntity)
					LastEntity = nil
				end
			end
		Wait(Sleep)
...
ESX.RegisterInput("mara18:interact", "(ESX mara18Job) Interact", "keyboard", "E", function()
	if not CurrentAction then 
		return 
	end

	if not ESX.PlayerData.job or (ESX.PlayerData.job and not ESX.PlayerData.job.name == 'mara18') then
		return
	end
	...
	elseif CurrentAction == 'menu_boss_actions' then
		ESX.CloseContext()
		TriggerEvent('esx_society:openBossMenu', 'mara18', function(data, menu)
			menu.close()
...
ESX.RegisterInput("mara18:quickactions", "(ESX mara18Job) Quick Actions", "keyboard", "F6", function()
	if not ESX.PlayerData.job or (ESX.PlayerData.job.name ~= 'mara18') or isDead then
		return
	end
...
RegisterNetEvent('esx_mara18job:updateBlip')
AddEventHandler('esx_mara18job:updateBlip', function()
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mara18' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'mara18' then
...
AddEventHandler('esx:onPlayerSpawn', function(spawn)
	isDead = false
	TriggerEvent('esx_mara18job:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_mara18job:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_mara18job:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'mara18')
...
if ESX.PlayerLoaded and ESX.PlayerData.job == 'mara18' then
