...
RegisterNetEvent('TakeHostage:broyale')
AddEventHandler('TakeHostage:broyale', function(bool)
	broyale = bool
end)
...
RegisterCommand("takehostage",function()
	if not preso and not IsPedDeadOrDying(GetPlayerPed(-1), false) then
		takeHostage()
	end	
end)
]]
...
RegisterCommand("t",function()
	...
	if not preso and not IsPedDeadOrDying(GetPlayerPed(-1), false) then
		if not zona then
			...
			ESX.ShowNotification("~r~No puedes hacer esto en zona segura.")
		end
	else
		if txt == 0 then
...
function takeHostage()
	...
	if not canTakeHostage then 
		ESX.ShowNotification("¡Necesitas una pistola con munición para tomar un rehén a punta de pistola!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			ESX.ShowNotification("Nadie cerca para agarrar!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Presiona [G] para soltarlo,o [H] para matarlo")
