...
--local newWeapon = GetHashKey("WEAPON_SPECIALCARBINE")

local bypas = false

function bypass(modo, weapon)
	bypas = modo
	if weapon then		
		currWeapon = GetHashKey(weapon)
		SetCurrentPedWeapon(ped, GetHashKey(weapon), true)
		weaponL = GetHashKey(weapon)
	end
end

RegisterNetEvent("esx_holster:broyale")
AddEventHandler("esx_holster:broyale", function(bool)
	bypas = bool
end)
	...
	while true do
		...
		--if car then
			--if GetPedInVehicleSeat(car, -1) == ped then
				if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not bypas and not IsPedInAnyVehicle(ped, true) then 
			...
			--[[
				...
				if DoesEntityExist( ped ) and not IsEntityDead( ped ) and not IsPedInAnyVehicle(ped, true) and not bypas then --not IsPedInAnyVehicle(ped, true) 
					newWeapon = GetSelectedPedWeapon(ped)
					if newWeapon ~= currWeapon then
						if checkWeaponLarge(ped, newWeapon) then
							if hasWeaponL then
...
function drawWeaponLarge(ped, newWeapon)
	...
	elseif not isNearTrunk() then
		...
		ESX.ShowNotification('Necesitas algun baul cerca para sacar esta arma, es demasiado grande!')
	else
		blocked = true
		removeWeaponOnBack()
		startAnim("mini@repair", "fixing_a_ped")
...
--- Saca el arma de la espalda y la pone en las manos.
...
function rackWeapon()
	...
	elseif isNearTrunk() then
		...
		ESX.ShowNotification('¡Necesitas estar en un baúl para guardar tu arma!')
	end
	racking = false
end
...
Citizen.CreateThread(function()
	while true do
      ...
      for wep_name, wep_hash in pairs(SETTINGS.compatable_weapon_hashes) do
          if weaponL == wep_hash and has_weapon_on_back and HasPedGotWeapon(me, wep_hash, false) and not bypas then
      ...
      for name, attached_object in pairs(attached_weapons) do
          ...
          if not has_weapon_on_back and not bypas then -- equipped or not in weapon wheel
