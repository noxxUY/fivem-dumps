local loadFile= LoadResourceFile(GetCurrentResourceName(), "./prioridad.json") -- you only have to do this once in your code, i just put it in since it wont get confusing.
...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", 200)
...
for i=1,#extract,1 do	
	_Queue.Priority[string_lower(extract[i].steam)] = extract[i].usd
end
...
function Queue:CanJoin(src, cb)
    ...
    for _, data in ipairs(_Queue.JoinCbs) do
        ...
        while await do Citizen.Wait(5) end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    while allow == nil do Citizen.Wait(5) end
    if not allow then return end
