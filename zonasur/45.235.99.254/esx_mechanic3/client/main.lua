...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		--TriggerServerEvent('esx_mechanic3:onNPCJobCompleted')
	end
end
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic3_actions', {
		title    = _U('mechanic'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		if data.current.value == 'boss_actions' then
			...
			TriggerEvent('esx_society:openBossMenu_zs', 'mechanic3', function(data, menu)
				menu.close()
			end, options)
		end
	...
	if Config.EnablePlayerManagement and PlayerData.job then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_harvest', {
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mechanic3', vehicleProps)
					end, function(data, menu)
						menu.close()
					end)
				end, 'mechanic3')
			...
			elseif data.current.value == 'gaz_bottle' then
				TriggerServerEvent('esx_mechanic3:startHarvest')
			elseif data.current.value == 'fix_tool' then
				TriggerServerEvent('esx_mechanic3:startHarvest2')
			elseif data.current.value == 'caro_tool' then
				TriggerServerEvent('esx_mechanic3:startHarvest3')
			end
		end, function(data, menu)
			menu.close()
	...
	if Config.EnablePlayerManagement and PlayerData.job then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_mechanic3:startCraft_zs')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_mechanic3:startCraft2_zs')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('esx_mechanic3:startCraft3_zs')
			end
		end, function(data, menu)
			menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill_zs', GetPlayerServerId(closestPlayer), 'society_mechanic3', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
				...
				if GetPedInVehicleSeat(vehicle, -1) == playerPed then
					...
					if vehicleLabel == 'BFMECA' or vehicleLabel == 'RPMECA' or vehicleLabel == 'Grúa' or vehicleLabel == 'Flatbed' or vehicleLabel == 'CADDYMECA' or model == -493410377 then
						...
						ESX.TriggerServerCallback('1974_drogas:check', function(serial)
							if serial then
								TriggerServerEvent("esx_mechanic3:money", serial)
							end
						end)
					end	
				else
				...
				if DoesEntityExist(vehicle) then
					if IsVehicleSeatFree(vehicle, -1) then
						...
						if vehicleLabel == 'BFMECA' or vehicleLabel == 'RPMECA' or vehicleLabel == 'Grúa' or vehicleLabel == 'Flatbed' or vehicleLabel == 'CADDYMECA' or model == -493410377 then
							...
							TriggerServerEvent("esx_mechanic3:money", 200)
						end	
					else
					...
					if NPCOnJob then
						if NPCTargetDeleterZone then
							...
							if CurrentlyTowedVehicle == NPCTargetTowable then
								...
								TriggerServerEvent('esx_mechanic3:onNPCJobMissionCompleted')
								StopNPCJob()
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_mechanic3:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_mechanic3:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_mechanic3:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_mechanic3:putStockItems', itemName, count)
...
RegisterNetEvent('esx_mechanic3:onHijack')
AddEventHandler('esx_mechanic3:onHijack', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mechanic3:onCarokit')
AddEventHandler('esx_mechanic3:onCarokit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_mechanic3:onFixkit')
AddEventHandler('esx_mechanic3:onFixkit', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, true)
			Citizen.CreateThread(function()
				TriggerServerEvent("esx_mechanic3:fixkit")
				Citizen.Wait(20000)
				SetVehicleFixed(vehicle)
				SetVehicleDeformationFixed(vehicle)
				SetVehicleUndriveable(vehicle, false)
...
RegisterNetEvent('esx_mechanic3:franela')
AddEventHandler('esx_mechanic3:franela', function()
	local playerPed = PlayerPedId()
	--local vehicle   = ESX.Game.GetVehicleInDirection()
	local coords    = GetEntityCoords(playerPed)
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			TaskStartScenarioInPlace(playerPed, 'WORLD_HUMAN_MAID_CLEAN', 0, true)
			Citizen.CreateThread(function()
				TriggerServerEvent("esx_mechanic3:franela")
				Citizen.Wait(10000)
				SetVehicleDirtLevel(vehicle, 0)
				ClearPedTasksImmediately(playerPed)
				ESX.ShowNotification(_U('vehicle_cleaned'))
...
AddEventHandler('esx_mechanic3:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_mechanic3:hasExitedMarker', function(station, part, partNum)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

AddEventHandler('esx_mechanic3:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job and PlayerData.job.name == 'mechanic3' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_mechanic3:hasExitedEntityZone', function(entity)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'mechanic3' then
			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)
		...
		if PlayerData.job and PlayerData.job.name == 'mechanic3' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				if (LastStation ~= nil and LastPart ~= nil and LastPartNum ~= nil) and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) then
					TriggerEvent('esx_mechanic3:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_mechanic3:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_mechanic3:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_mechanic3:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'mechanic3' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage_zs', 'mechanic3', vehicleProps)
					else
						if
						...
						then
							TriggerServerEvent('esx_service:disableService', 'mechanic3')
						end
					end
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'mechanic3' then
