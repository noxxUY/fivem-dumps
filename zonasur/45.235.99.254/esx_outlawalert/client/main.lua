...
Citizen.CreateThread(function()
	while true do
		...
		elseif IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert and GetSelectedPedWeapon(playerPed) ~= GetHashKey('WEAPON_MUSKET') then
			local weapon = WeaponNames[tostring(GetSelectedPedWeapon(playerPed))]
			...
			if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
				...
				TriggerServerEvent('esx_outlawalert:gunshotInProgress', {
					...
					y = ESX.Math.Round(playerCoords.y, 1),
					z = ESX.Math.Round(playerCoords.z, 1)
				}, streetName, playerGender, weapon)
			end
