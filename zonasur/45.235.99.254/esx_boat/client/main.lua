			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_boat:buyBoat', function(bought)
					if bought then
						...
						ESX.ShowNotification(_U('boat_shop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price), plate))
						...
						ESX.Game.SpawnVehicle(data.current.model, shop.Inside, shop.Inside.w, function(vehicle)
				...
				end, props, plate, data.current.category)
			else
				menu2.close()
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boat_garage', {
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, garage.SpawnPoint, garage.SpawnPoint.w, function(vehicle)
						TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						ESX.Game.SetVehicleProperties(vehicle, { plate = vehicleProps.plate })
