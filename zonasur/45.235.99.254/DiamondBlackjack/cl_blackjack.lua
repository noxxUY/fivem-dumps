...
cfg.blackjackTables = {
    ...
    [2] = {
        dealerPos = vector3(1149.291, 252.3449, -51.03),
        dealerHeading = 135.11,
        tablePos = vector3(1148.74, 251.6947, -52.04094),
        tableHeading = 45.309341430664,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [3] = {
...
Citizen.CreateThread(function()
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.7 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Presiona ~INPUT_PICKUP~ para jugar BlackJack")
                    else 
                        drawNativeNotification("Este asiento está ocupado.")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("~w~Tienes en tu mano: ~g~["..tostring(currentHand).."] ~w~La Banca tiene ~g~["..tostring(dealersHand).."]")
    ...
    while true do 
        if waitingForBetState then
            if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                local tmpInput = getGenericTextInput("cantidad a apostar")
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "El objetivo del Blackjack es vencer la mano del repartidor sin pasar de 21. Este juego usa cuatro mazos de 52 cartas, que se barajan al comienzo de cada mano.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
Citizen.CreateThread(function()
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                DrawRect(0.923, 0.928, 0.112, 0.032, 0, 0, 0, 150) 
                DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "CANTIDAD:", 255, 255, 255, 255, 0, 0)
        ...
        if drawTimerBar then
            DrawRect(0.938, 0.886, 0.081, 0.032, 0, 0, 0, 150)
            DrawAdvancedNativeText(1.013, 0.892, 0.005, 0.0028, 0.29, "TIEMPO:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Haga sus apuestas")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Sin apuestas, ronda omitida")
        end
    end)
end)
        ...
        if closestChair == chairId then
            ...
            Citizen.CreateThread(function()
                ...
                if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,globalNextCardCount)
                    declineCard()
                    drawNativeNotification("Error al pararse / golpear a tiempo, de pie.")
                end
            end)
        else 
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Esperando que comience el próximo juego ...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        angryILost()
        canExitBlackjack = true
        drawNativeNotification("~r~¡Perdiste!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Empataste!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("~g~¡Ganaste!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 194, true)) -- The button to display
    ButtonMessage("Plantarse") --BACKSPACE
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("Pedir Otra Carta") --Space
    PopScaleformMovieFunctionVoid()  
...
function getGenericTextInput(type)
	...
	AddTextEntry('FMMC_MPM_NA', "Digite " .. tostring(type))
	DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "Digite " .. tostring(type) .. " message", "", "", "", "", 30)
