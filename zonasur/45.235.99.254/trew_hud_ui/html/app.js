const wrenchWrap = document.getElementById('wrench-wrap')
...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				if (config.showId == true) { document.querySelector('#id').style.display = 'block'; }
			...
			updateVehicle: function(data) {
				...
				var vehicleWheel = document.querySelector('#wheel');
				...
				if (data.status == true) {
					if (vehicleInfo.classList.contains('inactive')) {
						...
						vehicleWheel.style.display = 'none';
						...
						if (vehiclesCars.indexOf(data.type) > -1) {
							...
							vehicleWheel.style.display = 'block';
					...
					vehicleWheel.querySelector('span').style.height = data.wheel+'%';
