...
RegisterCommand('gendarme', function()
	PlayerData = ESX.GetPlayerData()
	local playerPed = PlayerPedId()
	local grade = PlayerData.job.grade_name
	if PlayerData.job.name == "gendarme" then
		setUniform(grade, playerPed, 'Sandy')
	end
end, false)
...
function OpenCloakroomMenu(station)
	...
	if grade == 'recruit' then
		table.insert(elements, {label = _U('police_wear'), value = 'recruit'})
	elseif grade == 'officer' then
		table.insert(elements, {label = _U('police_wear'), value = 'officer'})
	elseif grade == 'sergeant' then
		table.insert(elements, {label = _U('police_wear'), value = 'sergeant'})
	elseif grade == 'lieutenant' then
		table.insert(elements, {label = _U('police_wear'), value = 'lieutenant'})
	elseif grade == 'inspect' then
		table.insert(elements, {label = _U('police_wear'), value = 'inspect'})
	elseif grade == 'subcomi' then
		table.insert(elements, {label = _U('police_wear'), value = 'subcomi'})
	elseif grade == 'comi' then
		table.insert(elements, {label = _U('police_wear'), value = 'comi'})	
			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'gendarme')
						TriggerServerEvent('esx_service:disableService', 'gendarme')
				...
				if not isInService then
					...
					ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
						if not canTakeService then
							...
							TriggerServerEvent('esx_service:notifyAllInService', notification, 'gendarme')
							--TriggerEvent('esx_gendarmeria:updateBlip')
			...
			elseif data.current.value == 'open_weapons' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory2',
				{
					...
					ESX.TriggerServerCallback('1974_drogas:check', function(serial)
						if serial then
							TriggerServerEvent('esx_gendarmeria:giveWeapon', weapon, 250, serial)
						end
					end)
					--TriggerServerEvent('esx_gendarmeria:giveWeapon', weapon, 250)
				end,
				function(data2, menu2)
					menu2.close()
				end)	
...
function DropWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		TriggerServerEvent('esx_gendarmeria:removeWeapon', data.current.value)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if foundSpawnPoint then
					ESX.Game.SpawnVehicle(vehicleProps.model, spawnPoint, spawnPoint.heading, function(vehicle)
						...
						ESX.Game.SetVehicleProperties(vehicle, { plate = 'GNAZS'})
					end)

					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'gendarme', vehicleProps)
				end
			end, function(data, menu)
				menu.close()
...
RegisterNetEvent('esx_gendarmeria:refuerzo')
AddEventHandler('esx_gendarmeria:refuerzo', function(coords)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		if data.current.value == 'dar_carnet' then
			...
			if closestPlayer ~= -1 and closestDistance <= 3.0 then
				...
				ESX.ShowNotification("NINGUN GENDARME CERCA!")
			end	
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_gendarmeria:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
		...
		elseif data.current.value == 'refuerzo' then
			TriggerServerEvent('esx_gendarmeria:refuerzo', GetEntityCoords(GetPlayerPed(PlayerId()), true))
...
RegisterNetEvent('esx_gendarmeria:hardcuff')
AddEventHandler('esx_gendarmeria:hardcuff', function()
	local playerPed = PlayerPedId()
	Citizen.CreateThread(function()
		if IsShackles then
...
RegisterNetEvent('esx_gendarmeria:doarrested')
AddEventHandler('esx_gendarmeria:doarrested', function()
	Citizen.Wait(250)
...
RegisterNetEvent('esx_gendarmeria:getarrestedhard')
AddEventHandler('esx_gendarmeria:getarrestedhard', function(playerheading, playercoords, playerlocation)
	...
	TriggerEvent('esx_gendarmeria:hardcuff')
	LoadAnimDict('mp_arresting')
	TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
	FreezeEntityPosition(playerPed, true)
end)

RegisterNetEvent('esx_gendarmeria:douncuffing')
AddEventHandler('esx_gendarmeria:douncuffing', function()
	Citizen.Wait(250)
	LoadAnimDict('mp_arresting')
...
RegisterNetEvent('esx_gendarmeria:getuncuffed')
AddEventHandler('esx_gendarmeria:getuncuffed', function(playerheading, playercoords, playerlocation)
	...
	IsShackles = false
	--FreezeEntityPosition(playerPed, false)
	TriggerEvent('esx_gendarmeria:handcuff')
	ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('esx_gendarmeria:handcuff')
AddEventHandler('esx_gendarmeria:handcuff', function()
	local playerPed = PlayerPedId()
	Citizen.CreateThread(function()
...
function OpenIdentityCardMenu(player)
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getOtherPlayerData', function(data)
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getOtherPlayerData', function(data)
		...
		function(data, menu)
			...
			if data.current.value ~= nil then
				TriggerServerEvent('esx_gendarmeria:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		{
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill_zs', GetPlayerServerId(player), 'society_gendarmeria', _U('fine_total', label), amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_gendarmeria:getVehicleFromPlate', function(owner, found)
				if found then
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			if data.current.value ~= 'gym' then
				...
				TriggerServerEvent('esx_gendarmeria:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon',
		{
			...
			ESX.TriggerServerCallback('esx_gendarmeria:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		ESX.TriggerServerCallback('esx_gendarmeria:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu(station)
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getArmoryWeapons', function(weapons)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons',
		{
			...
			ESX.TriggerServerCallback('esx_gendarmeria:buy', function(hasEnoughMoney)
...
function OpenGetStocksMenu()
	if PlayerData.job.grade_name == 'boss' then
	ESX.TriggerServerCallback('esx_gendarmeria:getStockItems', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('esx_gendarmeria:getStockItem', data.current.type, data.current.value, data.current.index)
				ESX.SetTimeout(300, function()
					OpenGetStocksMenu()
				end)
			else
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_gendarmeria:getStockItem', data.current.type, data.current.value, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_gendarmeria:getPlayerInventory', function(inventory)
		...
		if inventory.Money > 0 then
			table.insert(elements, {
				label = 'Dinero Sucio: <span style="color:green;">$ '..ESX.Math.GroupDigits(inventory.Money)..'</span> ';
				type  = 'item_money',
				value = 'money'
			})
		end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			if data.current.type == 'item_weapon' then
				menu.close()
				TriggerServerEvent('esx_gendarmeria:putStockItems', data.current.type, data.current.value, data.current.ammo)
				ESX.SetTimeout(300, function()
					OpenPutStocksMenu()
				end)
			else
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_gendarmeria:putStockItems', data.current.type, itemName, count)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'gendarme' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_gendarmeria:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_gendarmeria:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('esx_gendarmeria:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job ~= nil and (PlayerData.job.name == 'gendarme') and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_gendarmeria:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_gendarmeria:handcuff_zs')
AddEventHandler('esx_gendarmeria:handcuff_zs', function()
...
RegisterNetEvent('esx_gendarmeria:desesposar')
AddEventHandler('esx_gendarmeria:desesposar', function()
	local playerPed = PlayerPedId()
	IsHandcuffed = false
...
RegisterNetEvent('esx_gendarmeria:unrestrain')
AddEventHandler('esx_gendarmeria:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
		ClearPedSecondaryTask(playerPed)
...
RegisterNetEvent('esx_gendarmeria:drag')
AddEventHandler('esx_gendarmeria:drag', function(copID)
...
RegisterNetEvent('esx_gendarmeria:putInVehicle_zs')
AddEventHandler('esx_gendarmeria:putInVehicle_zs', function()
    local playerPed = PlayerPedId()
    if not IsHandcuffed then
        return
    end
...
RegisterNetEvent('esx_gendarmeria:OutVehicle_zs')
AddEventHandler('esx_gendarmeria:OutVehicle_zs', function()
	local playerPed = PlayerPedId()
	...
	while IsPedInVehicle(playerPed, vehicle, false) do 
		TriggerServerEvent('esx_gendarmeria:handcuff_zs', GetPlayerServerId(playerPed))
		break
	end	
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'gendarme' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'gendarme' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_gendarmeria:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_gendarmeria:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_gendarmeria:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('esx_gendarmeria:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and (PlayerData.job.name == 'gendarme') then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage_zs', 'gendarme', vehicleProps)
					end
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenu_zs', 'gendarme', function(data, menu)
						menu.close()
						CurrentAction     = 'menu_boss_actions'
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'gendarme' then
...
RegisterNetEvent('esx_gendarmeria:updateBlip')
AddEventHandler('esx_gendarmeria:updateBlip', function(policias)
...
AddEventHandler('playerSpawned', function(spawn)
	...
	--[[
	TriggerEvent('esx_gendarmeria:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_gendarmeria:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_gendarmeria:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'gendarme')
