...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
	if PlayerData.job and PlayerData.job.grade_name == 'boss' and 'society_' .. PlayerData.job.name == society then
		UpdateSocietyMoneyHUDElement(money)
	end
end)
...
function OpenBossMenu(society, close, options)
	...
	--[[
	ESX.TriggerServerCallback('LegacyFuel:getStockItems', function(inventory)
		money = inventory.blackMoney
	end, 'society_'..society)
	]]--
	
	ESX.TriggerServerCallback('esx_addonaccount:getSharedAccount2', function(account)
		money = account.money
	end, 'society_'..society)
...
function OpenRecruitMenu(society)
	...
	if #playersInArea <= 0 then
        return ESX.ShowNotification("Nadie cerca!")
    end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				...
				if data2.current.value == 'yes' then
					...
					ESX.TriggerServerCallback('esx_society:setJob', function()
						OpenRecruitMenu(society)
					end, data.current.value, society, 0, 'hire')
				end
			end, function(data2, menu2)
				menu2.close()
			end)
...
function OpenPromoteMenu(society, employee)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		for i=1, #job.grades, 1 do
			...
			if job.grades[i].name ~= 'boss' then
				table.insert(elements, {
					label = job.grades[i].label,
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			...
			if data.current.name ~= 'boss' then
				ESX.TriggerServerCallback('esx_society:setJob', function()
					OpenEmployeeList(society)
				end, employee.identifier, society, data.current.value, 'promote')
			else
				ESX.ShowNotification('~r~No puedes ascender a Jefe!')
			end
		end, function(data, menu)
			menu.close()
			OpenEmployeeList(society)
...
AddEventHandler('esx_society:openBossMenu_zs', function(society, close, options)
