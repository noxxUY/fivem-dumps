...
AddEventHandler('tm1_mafias:openCreator',function(rango)
	EnableGui(true, rango)
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	ESX.TriggerServerCallback('1974_drogas:check', function(serial)
		if serial then
			TriggerServerEvent('tm1_mafias:createGroup', serial, data.groupName, data.rango)
			EnableGui(false)
			Wait(500)
		end
	end)	
...
--==                 FUNCTIONS                 ==
--===============================================
function EnableGui(enable, rango)
    SetNuiFocus(enable)
    guiEnabled = enable
