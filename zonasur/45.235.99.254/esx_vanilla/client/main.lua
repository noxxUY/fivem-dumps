...
function IsGradeBoss()
    if PlayerData ~= nil then
        ...
        if PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'viceboss' or PlayerData.identifier == Config.Owner then
            IsGradeBoss = true
        end
        return IsGradeBoss
    end
...
function OpenPutArmas()
	ESX.TriggerServerCallback('esx_vanilla:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			menu.close()
			TriggerServerEvent('esx_vanilla:putStockItems', data.current.type, data.current.value, data.current.ammo)
			Citizen.Wait(300)
			OpenPutArmas()
...
function OpenGetStocksMenuArmas()
	ESX.TriggerServerCallback('esx_vanilla:getStockItems', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu_armas',
		{
			...
			menu.close()
			TriggerServerEvent('esx_vanilla:getStockItem', data.current.type, data.current.value, data.current.ammo)
			Citizen.Wait(300)
			OpenGetStocksMenuArmas()
		end, function(data, menu)
			menu.close()
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'vanilla', vehicleProps)
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              ...
              function(data2, menu2)
                ...
                TriggerServerEvent('esx_vanilla:craftingCoktails', data2.current.value)
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill_zs', GetPlayerServerId(player), 'society_vanilla_boss', _U('billing'), amount)
        end
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu.close()
					TriggerServerEvent('esx_vanilla:getStockItem', tipo, itemName, count)
					Citizen.Wait(300)
					OpenGetStocksMenu()
				end
			end, function(data, menu)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
					...
					if count == nil then
						...
						--menu2.close()
						menu.close()
						TriggerServerEvent('esx_vanilla:putStockItems', tipo, itemName, count)
						Citizen.Wait(300)
						OpenPutStocksMenu(enclosure)
					end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_vanilla:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_vanilla:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_vanilla:putFridgeStockItems', itemName, count)
            end
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_vanilla:getVaultWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_vanilla:removeVaultWeapon', function()
          OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_vanilla:addVaultWeapon', function()
        OpenPutWeaponMenu()
      end, data.current.value)
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'vanilla_shop',
        {
            title    = _U('shop'),
			align = 'bottom-right',
            elements = elements
        ...
        function(data, menu)
            TriggerServerEvent('esx_vanilla:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_vanilla:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_vanilla:hasExitedMarker', function(zone)
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_vanilla:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent('esx_vanilla:hasExitedMarker', LastZone)
            end
...
Citizen.CreateThread(function()
  while true do
      ...
      if IsControlJustReleased(0,  38) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'vanilla', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'vanilla')
            end
        ...
        if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
          ...
          ESX.UI.Menu.CloseAll()
			...
			TriggerEvent('esx_society:openBossMenu_zs', 'vanilla', function(data, menu)
				menu.close()
				CurrentAction     = 'menu_boss_actions'
				CurrentActionMsg  = _U('open_bossmenu')
				CurrentActionData = {}
	...
	--[[
    if IsControlJustReleased(0,  167) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'vanilla_actions') then
...
AddEventHandler('esx_vanilla:teleportMarkers', function(position)
  ...
  while true do
    ...
    if IsJobTrue() then
        ...
        if IsControlJustReleased(0, 38) and isInPublicMarker then
          TriggerEvent('esx_vanilla:teleportMarkers', position)
        end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if data.current.value == 'comprar' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'compra_menu',
			{
				...
				TriggerServerEvent('esx_vanilla:comprar_item', data2.current.value, data2.current.price)	
				menu2.close()
...
--[[
exports["bt-polyzone"]:AddPolyzone("vanillastrip", {
		vector2(103.99387359619, -1296.7551269531),
		vector2(105.95928192139, -1295.6644287109),
		vector2(106.41999053955, -1294.9302978516),
		vector2(106.45307159424, -1293.5916748047),
		vector2(106.05648803711, -1292.7203369141),
		vector2(111.95678710938, -1289.2670898438),
		vector2(112.52867126465, -1290.0380859375),
		vector2(115.22596740723, -1288.4938964844),
		vector2(112.77897644043, -1284.0172119141),
		vector2(109.97137451172, -1285.6044921875),
		vector2(110.29742431641, -1286.5456542969),
		vector2(104.1125869751, -1289.7525634766),
		vector2(103.79721069336, -1289.2777099609),
		vector2(102.19856262207, -1288.6629638672),
		vector2(99.865966796875, -1289.9997558594)
	}, {
	name="vanillastrip",
	debugPoly=false,
	minZ = 26.258722305298,
	maxZ = 32.457263946533
})
]]--
...
CreateThread(function()
	...
	for i = 1, #Config.Sillones do
		local sillon = CreateObject(GetHashKey('v_club_vuarmchair'), Config.Sillones[i].x, Config.Sillones[i].y, Config.Sillones[i].z, false, false, false)
		SetEntityHeading(sillon, Config.Sillones[i].w)
		FreezeEntityPosition(sillon, true)
    end
	...
	CreateThread(function()
		while true do
			Wait(500)
			if notifIn then
				local stripping_pole = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 2.0, GetHashKey("prop_strip_pole_01"), false, false, false) --v_19_strpfrntpl --prop_strip_pole_01
				if DoesEntityExist(stripping_pole) then
					while #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(stripping_pole)) <= 1.5 do
						Wait(0)
						DrawTxt3D(GetEntityCoords(stripping_pole) + vector3(0.0, 0.0, 1.0), 'Presiona ~INPUT_CONTEXT~ para bailar en el tubo')
						if IsControlJustReleased(0, 51) then
							...
							while true do
								Wait(0)  
								HelpText('Presiona ~INPUT_VEH_DUCK~ para detener el baile', true)
	...
	CreateThread(function()
		while true do
            Wait(500)
			if notifIn then
				local chair = GetClosestObjectOfType(GetEntityCoords(PlayerPedId()), 2.0, GetHashKey("v_club_vuarmchair"), false, false, false) --v_club_vuarmchair --apa_mp_h_stn_chairarm_01
				if DoesEntityExist(chair) then
					while #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(chair)) <= 2.0 do
						Wait(0)
						DrawTxt3D(GetEntityCoords(chair) + vector3(0.0, 0.0, 1.0), 'Presiona ~INPUT_CONTEXT~ para dar un baile erotico\nPresiona   ~INPUT_VEH_EXIT~para sentarte')
						if IsControlJustReleased(0, 51) then
...
--[[
RegisterCommand('sillon', function()
    --if not Cache.furnishing then
        ...
        --if Model.loaded then
            local object = CreateObject(GetHashKey('apa_mp_h_stn_chairarm_01'), GetEntityCoords(PlayerPedId()), false, false, false)
            ...
            while furnishing do
                ...
                --DrawText3D(GetEntityCoords(object) + vec3(0.0, 0.0, 0.5), string.format("Rotacion: %s", RoundInt(GetEntityHeading(object))))
				 if IsControlPressed(0, 191) then
                    local objCoords = GetEntityCoords(object)
                    local heading = GetEntityHeading(object)
                    
					print('COORDS = '..objCoords..' Heading: '..heading)
