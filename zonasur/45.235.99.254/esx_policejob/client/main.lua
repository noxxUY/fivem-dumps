...
RegisterNetEvent('esx_policejob:alerta')
AddEventHandler('esx_policejob:alerta', function(niv)
	nivel = niv
end)

RegisterNetEvent('esx_policejob:dispo')
AddEventHandler('esx_policejob:dispo', function(Cantidad)
	PoliciasDispo = Cantidad
end)

RegisterNetEvent('esx_policejob:Servicio')
AddEventHandler('esx_policejob:Servicio', function(bool)
	EnServicio = bool
end)
...
RegisterNetEvent('esx_policejob:hud')
AddEventHandler('esx_policejob:hud', function(bool)
	hudNivel = bool
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if hud then
			if nivel == 3 then
				for k, v in ipairs(armas_blacklist) do
					SetCanPedEquipWeapon(GetPlayerPed(-1), GetHashKey(v), true)
				end
			else
				for k, v in ipairs(armas_blacklist) do
					SetCanPedEquipWeapon(GetPlayerPed(-1), GetHashKey(v), false)
				end	
			end
			if EnServicio then
				drawTxt('DISPONIBLE: '..PoliciasDispo,2, 1, 0.940, 0.925, 0.50, 0, 255, 0,255)
				if IsPedDeadOrDying(PlayerPedId(), true) then
					TriggerServerEvent('esx_policejob:nodispo')
				end
			else
...
RegisterCommand('police', function()
	...
	if PlayerData.job.name == "police" then
		setUniform(grade, playerPed, 'LSPD')
	end
end, false)
...
RegisterCommand('swat', function()
	...
	if PlayerData.job.name == "police" or PlayerData.job.name == "police2" then
		setUniform('swat', playerPed, 'LSPD')
	end	
end, false)
			...
			elseif data.current.value == 'open_weapons' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory2',
				{
					...
					ESX.TriggerServerCallback('1974_drogas:check', function(serial)
						if serial then
							TriggerServerEvent('esx_policejob:giveWeapon', weapon, 250, serial)
						end
					end)
				end,
				function(data2, menu2)
...
RegisterNetEvent('esx_policejob:refuerzo')
AddEventHandler('esx_policejob:refuerzo', function(coords)
...
function OpenPoliceActionsMenu()
	...
	if IsGradeBoss() or PlayerData.job.grade_name == 'comi' then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elseif data.current.value == 'refuerzo' then
			TriggerServerEvent('esx_policejob:refuerzo', GetEntityCoords(GetPlayerPed(PlayerId()), true))
...
AddEventHandler('esx_policejob:getarrestedhard', function(playerheading, playercoords, playerlocation)
	...
	IsHandcuffed = true
	IsShackles = true
	exports["tm1_mafias"]:Esposado(true)
...
AddEventHandler('esx_policejob:getuncuffed', function(playerheading, playercoords, playerlocation)
	...
	IsHandcuffed = false
	IsShackles = false
	exports["tm1_mafias"]:Esposado(false)
	--FreezeEntityPosition(playerPed, false)
	...
	ESX.TriggerServerCallback('esx_policejob:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		{
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill_zs', GetPlayerServerId(player), 'society_police', _U('fine_total', label), amount)
			else
...
AddEventHandler('esx_policejob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'police2' or PlayerData.job.name == 'transito' or PlayerData.job.name == 'seguridad' or  PlayerData.job.name == 'gendarme') and IsPedOnFoot(playerPed) then --or PlayerData.job.name == 'ambulance'
...
RegisterNetEvent('esx_policejob:handcuff_zs')
AddEventHandler('esx_policejob:handcuff_zs', function()
...
RegisterNetEvent('esx_policejob:desesposar')
AddEventHandler('esx_policejob:desesposar', function()
	local playerPed = PlayerPedId()
	IsHandcuffed = false
...
RegisterNetEvent('esx_policejob:putInVehicle_zs')
AddEventHandler('esx_policejob:putInVehicle_zs', function()
    local playerPed = PlayerPedId()
    if not IsHandcuffed then
        return
    end
...
RegisterNetEvent('esx_policejob:OutVehicle_zs')
AddEventHandler('esx_policejob:OutVehicle_zs', function()
	local playerPed = PlayerPedId()
	...
	while IsPedInVehicle(playerPed, vehicle, false) do 
		TriggerServerEvent('esx_policejob:handcuff_zs', GetPlayerServerId(playerPed))
		break
	end	
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'gendarme' or PlayerData.job.name == 'seguridad' or PlayerData.job.name == 'transito' or PlayerData.job.name == 'police2') then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and (PlayerData.job.name == 'police' or PlayerData.job.name == 'police2' or PlayerData.job.name == 'seguridad' or PlayerData.job.name == 'gendarme' or PlayerData.job.name == 'transito') then --or PlayerData.job.name == 'ambulance'
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if pa == 'POLICIA' then
						if Config.EnableSocietyOwnedVehicles then
							...
							TriggerServerEvent('esx_society:putVehicleInGarage_zs', 'police', vehicleProps)
						end
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenu_zs', 'police', function(data, menu)
						menu.close()
						CurrentAction     = 'menu_boss_actions'
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
...
AddEventHandler('esx_policejob:updateBlip', function(policias)
