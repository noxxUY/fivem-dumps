...
createRulettAsztal = function(index, data)
    ...
    self.enableCamera = function(state)
        if state then
                ...
                function()
                    while selectedRulett ~= nil do
                        ...
                        if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                            local tmpInput = getGenericTextInput(_U('input_place_bet'))
			...
			currentBetAmount = 0
			--ESX.ShowNotification('Dejaste la mesa con una puesta de: '..currentBetAmount)
                                        ...
                                        if currentBetAmount > 0 then
                                            if Config.RulettTables[selectedRulett] ~= nil then
                                                if currentBetAmount >= Config.RulettTables[selectedRulett].minBet and currentBetAmount <= Config.RulettTables[selectedRulett].maxBet then
													--ESX.TriggerServerCallback('1974_ruleta:getMoney', function(amount)
														--if amount then
...
Citizen.CreateThread(
    function()
        while true do
            ...
            closetoRulett = false
            for i=1, #Config.RulettTables do
                if #(playerCoords - Config.RulettTables[i].position) < 100.0 then
                    closetoRulett = true
                end
            end
            Wait(1000)
...
Citizen.CreateThread(
    function()
        ...
        for i=1, #Config.RulettTables do
            createRulettAsztal(i, Config.RulettTables[i])
        end
                ...
                for k, v in pairs(Rulettek) do
                    if DoesEntityExist(v.tableObject) then
                        ...
                        if dist < 2.5 then
							...
							DrawText3D(closestChairData.position.x, closestChairData.position.y, closestChairData.position.z + 1.5, "Presiona ~g~[E]~w~ para sentarte.")

                            --Utils.ShowHelpNotification(_U('help_sit_rulett'))
