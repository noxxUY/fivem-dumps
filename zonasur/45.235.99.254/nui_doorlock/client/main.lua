...
RegisterCommand('doorlock', function()
	if not isDead and not isCuffed and closestDoor then
		...
		if mafia then
			TriggerServerEvent('nui_doorlock:updateState', closestDoor, locked, src, mafia.enclosure) -- Broadcast new state of the door to everyone
		else
			TriggerServerEvent('nui_doorlock:updateState', closestDoor, locked, src, false) -- Broadcast new state of the door to everyone
		end
	end
end)
