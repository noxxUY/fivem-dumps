...
function OpenCloakroomMenu()
	...
	if grade == 'abogado' then
		table.insert(elements, {label = 'Abogado', value = 'abogado'})
	elseif grade == 'fiscal' then
		table.insert(elements, {label = 'Fiscal', value = 'fiscal'})
	elseif grade == 'boss' then
		table.insert(elements, {label = 'Juez', value = 'boss'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if
			data.current.value == 'abogado' or
			data.current.value == 'fiscal' or
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory',
		{
			...
			elseif data.current.value == 'open_weapons' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory2',
				{
					...
					TriggerServerEvent('esx_judicial:giveWeapon', weapon, 1000)
				end,
				function(data2, menu2)
					menu2.close()
				end)	
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_judicial:buyWeapon', function(bought)
				if bought then
...
function DropWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		TriggerServerEvent('esx_judicial:removeWeapon', data.current.value)
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if foundSpawnPoint then
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'judicial', vehicleProps)
				end
			end, function(data, menu)
				menu.close()
...
RegisterNetEvent('esx_judicial:refuerzo')
AddEventHandler('esx_judicial:refuerzo', function(coords)
...
function OpenPoliceActionsMenu()
	...
	table.insert(elements, {label = 'Facturacion', value = 'billing'})
	
	if PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = "Dar Matricula Abogado", value = 'dar_matricula'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		if data.current.value == "dar_matricula" then
			...
			if closestPlayer ~= -1 and closestDistance <= 3.0 then
				...
				{
					title  = 'Entregar matricula a '..GetPlayerName(closestPlayer),
					align    = 'bottom-right',
					elements = elements
				},
				function(data, menu)
					if data.current.value == "yes" then
						TriggerServerEvent('esx_license:addLicenseAbogado', GetPlayerServerId(closestPlayer), 'abogado')
						menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill_zs', GetPlayerServerId(closestPlayer), 'society_judicial', 'Judicial', amount)
					end
				end
			end, function(data, menu)
				menu.close()
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_judicial:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
					...
					elseif data2.current.value == 'hardcuff' then
						...
						ESX.TriggerServerCallback('1974_drogas:check', function(can)
							if can then
								TriggerServerEvent('esx_policejob:requesthard', target_id, playerheading, playerCoords, playerlocation, can)
							end
						end)	
					...
					elseif action == 'drag' then
						TriggerServerEvent('esx_judicial:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_judicial:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_judicial:OutVehicle', GetPlayerServerId(closestPlayer))
		...
		elseif data.current.value == 'refuerzo' then
			TriggerServerEvent('esx_judicial:refuerzo', GetEntityCoords(GetPlayerPed(PlayerId()), true))			
		end
...
function OpenIdentityCardMenu(player)
	...
	ESX.TriggerServerCallback('esx_judicial:getOtherPlayerData', function(data)
	...
	ESX.TriggerServerCallback('esx_judicial:getOtherPlayerData', function(data)
		...
		function(data, menu)
			...
			if data.current.value ~= nil then
				TriggerServerEvent('esx_judicial:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
	...
	ESX.TriggerServerCallback('esx_judicial:getFineList', function(fines)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category',
		{
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:sendBill_zs', GetPlayerServerId(player), 'society_judicial', _U('fine_total', label), amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_judicial:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_judicial:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('esx_judicial:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('esx_judicial:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('esx_judicial:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon',
		{
			...
			ESX.TriggerServerCallback('esx_judicial:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon',
	{
		...
		ESX.TriggerServerCallback('esx_judicial:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu(station)
	...
	ESX.TriggerServerCallback('esx_judicial:getArmoryWeapons', function(weapons)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons',
		{
			...
			ESX.TriggerServerCallback('esx_judicial:buy', function(hasEnoughMoney)
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_judicial:getStockItems', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			if data.current.type == 'item_weapon' then
				...
				TriggerServerEvent('esx_judicial:getStockItem', data.current.type, data.current.value, data.current.index)
				ESX.SetTimeout(300, function()
					OpenGetStocksMenu()
				end)
			else
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_judicial:getStockItem', data.current.type, data.current.value, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_judicial:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			if data.current.type == 'item_weapon' then
				menu.close()
				TriggerServerEvent('esx_judicial:putStockItems', data.current.type, data.current.value, data.current.ammo)
				ESX.SetTimeout(300, function()
					OpenPutStocksMenu()
				end)
			else
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_judicial:putStockItems', data.current.type, itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_judicial:forceBlip')
end)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'judicial' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_judicial:hasEnteredMarker', function(station, part, partNum)
	...
	elseif part == 'VehicleDeleter' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			ESX.ShowNotification('~r~No es un auto de judicial!')
		end
...
AddEventHandler('esx_judicial:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('esx_judicial:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job ~= nil and PlayerData.job.name == 'judicial' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_judicial:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_judicial:handcuff_zs')
AddEventHandler('esx_judicial:handcuff_zs', function()
...
RegisterNetEvent('esx_judicial:unrestrain')
AddEventHandler('esx_judicial:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
		ClearPedSecondaryTask(playerPed)
...
RegisterNetEvent('esx_judicial:drag')
AddEventHandler('esx_judicial:drag', function(copID)
...
--[[
RegisterNetEvent('esx_judicial:putInVehicle')
AddEventHandler('esx_judicial:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_judicial:OutVehicle')
AddEventHandler('esx_judicial:OutVehicle', function()
	local playerPed = PlayerPedId()
	...
	while isPedInVehicle(playerPed, vehicle, false) do 
		TriggerServerEvent('esx_judicial:handcuff_zs', GetPlayerServerId(playerPed))
		break
	end	
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'judicial' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'judicial' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_judicial:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_judicial:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_judicial:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity ~= nil then
				TriggerEvent('esx_judicial:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'judicial' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage_zs', 'judicial', vehicleProps)
					end
					--TriggerEvent('persistent-vehicles/forget-vehicle', CurrentActionData.vehicle)
					ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenu_zs', 'judicial', function(data, menu)
						menu.close()
						CurrentAction     = 'menu_boss_actions'
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'judicial' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'judicial_actions') then
			if Config.MaxInService == -1 then
...
RegisterNetEvent('esx_judicial:updateBlip')
AddEventHandler('esx_judicial:updateBlip', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'judicial' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers_zs', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'judicial' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_judicial:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_judicial:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_judicial:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'judicial')

		if Config.MaxInService ~= -1 then
			TriggerServerEvent('esx_service:disableService', 'judicial')
		end
