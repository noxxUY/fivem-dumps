...
AddEventHandler('eden_accesories:silenciador', function(duration)
--[[
	local inventory = ESX.GetPlayerData().inventory
	local silenciador = 0

		for i=1, #inventory, 1 do
		  if inventory[i].name == 'silenciador' then
			silenciador = inventory[i].count
		  end
		end
]]
...
local currentWeaponHash = GetSelectedPedWeapon(ped)
		...
		--if used < silenciador then
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		...
		  		ESX.ShowNotification(("Te acabas de equipar con un silenciador. Tendrá que ser reequipado cada vez que regrese a la ciudad."))
				TriggerServerEvent('eden_accesories:removeItem', 'silenciador')
			...
			elseif currentWeaponHash == GetHashKey("WEAPON_SPECIALCARBINE_MK2") then
		  		GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey("WEAPON_SPECIALCARBINE_MK2"), GetHashKey("COMPONENT_AT_AR_SUPP"))  
		  		ESX.ShowNotification(("Te acabas de equipar con un silenciador. Tendrá que ser reequipado cada vez que regrese a la ciudad.")) 
				TriggerServerEvent('eden_accesories:removeItem', 'silenciador')
		  	else 
		  		ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede soportar un silenciador."))	
			end
		--else
			--ESX.ShowNotification(("Has utilizado todos tus silenciadores.")) 
		--end
end)
...
local currentWeaponHash = GetSelectedPedWeapon(ped)
		...
		--if used2 < flashlight then
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("Te acabas de equipar con una lámpara. Tendrá que ser reequipado cada vez que regrese a la ciudad.")) 
		  		 	--used2 = used2 + 1
	...
	--used2 = used2 + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede soportar una linterna."))
			end
		--else
...
local currentWeaponHash = GetSelectedPedWeapon(ped)
		...
		if used3 < grip then
			if currentWeaponHash == GetHashKey("WEAPON_COMBATPDW") then
		  		 ...
		  		 ESX.ShowNotification(("Te acabas de equipar con un asa. Tendrá que ser reequipado cada vez que regrese a la ciudad.")) 
		  				used3 = used3 + 1
	...
	used3 = used3 + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no puede sostener un mango."))

			end
		else
				  ESX.ShowNotification(("Has usado todas tus asas."))
		end
end)
...
local currentWeaponHash = GetSelectedPedWeapon(ped)
		...
		--if used4 < yusuf then
			if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
		  		 ...
		  		 ESX.ShowNotification(("Acabas de equipar un SKIN en tu Arma. Tendrá que ser reequipado cada vez que regrese a la ciudad.")) 
		  		 	--used4 = used4 + 1
	...
	--used4 = used4 + 1
		  		  ...
		  		  ESX.ShowNotification(("No tienes un arma en la mano o tu arma no soporta una apariencia elegante."))
			end
		--else
				 -- ESX.ShowNotification(("Has usado todas tus máscaras de lujo."))
		--end
end)
