...
AddEventHandler('esx_vangelico_robbery:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
	--PlaySoundFromCoord(soundid, "VEHICLES_HORNS_AMBULANCE_WARNING", -629.99, -236.542, 38.05) --['x'] = -629.99, ['y'] = -236.542, ['z'] = 38.05
end)
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not holdingup then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						...
						if IsPedShooting(GetPlayerPed(-1)) and not incircle then
							incircle = true
							if Config.NeedBag then
							    if borsa == 40 or borsa == 41 or borsa == 44 or borsa == 45 then
							        ESX.TriggerServerCallback('esx_policejob:getCops', function(CopsConnected)
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
						...
						ESX.TriggerServerCallback('esx_vangelico_robbery:check', function(can)
							TriggerServerEvent('esx_vangelico_robbery:gioielli', can)
						end)
					...
					if IsControlJustReleased(1, 51) then
						...
						ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
							if quantity >= Config.MaxJewelsSell then
								ESX.TriggerServerCallback('esx_policejob:getCopsinServicio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsSell then
										...
										blip = false
										ExecuteCommand('entorno Se veria sujeto vendiendo Joyas.')
									else
										blip = false
