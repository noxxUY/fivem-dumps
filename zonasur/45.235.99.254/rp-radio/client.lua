...
Radio.Commands = {
	...
	},
	{
		...
		Help = "Cambiar de frecuencia", -- Command help shown in chatbox when typing the command
		Params = {
			{name = "numbero", "Digite Frecuencia"}
		},
		Handler = function(src, args, raw)
			if Radio.Has then
				if args[1] then
...
function Radio:Toggle(toggle)
	...
	if self.Open then
		...
		RequestModel(self.Prop2)

		while not HasModelLoaded(self.Prop2) do
			Citizen.Wait(150)
		end

		self.Handle = CreateObject(self.Prop2, 0.0, 0.0, 0.0, true, true, false)

		local bone = GetPedBoneIndex(playerPed, self.Bone2)
...
RegisterNetEvent('rp-radio:GivePlayerAccessToFrequency')
AddEventHandler('rp-radio:GivePlayerAccessToFrequency', function(value)
	local frequency = tonumber(value)
...
RegisterNetEvent('rp-radio:RemovePlayerAccessToFrequency')
AddEventHandler('rp-radio:RemovePlayerAccessToFrequency', function(value)
	local frequency = tonumber(value)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if Radio.Open then
			...
			if Radio.On then
				if isBroadcasting and not isPlayingBroadcastAnim and not rola then
			...
			if not isBroadcasting and not isPlayingOpenAnim and not rola then
			...
			if radioConfig.AllowRadioWhenClosed and not rola then
...
RegisterNetEvent("rp-radio:ToggleOrg")
AddEventHandler("rp-radio:Toggleorg", function(freq)
	local playerPed = PlayerPedId()
	local isFalling = IsPedFalling(playerPed)
	local isDead = IsEntityDead(playerPed)
...
RegisterNetEvent("rp-radio:SetRadioOff")
AddEventHandler("rp-radio:SetRadioOff", function()
	Radio:Toggle(false)
	Radio.On = false
	Radio:Remove()
	exports["mumble-voip"]:SetMumbleProperty("radioEnabled", false)
