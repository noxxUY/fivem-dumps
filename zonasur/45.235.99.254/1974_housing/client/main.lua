                    ...
                    while Cache.inshell do
                        ...
                        if #(selfCoords - entrance_coords) <= 2.0 and not Cache.furnishing then
                            ...
                            if IsControlJustReleased(0, ((Config.Options and Config.Options.MainAction) or (51))) then
                                ...
                                if (not Config.Options.RequireKeyFurnish) or exports["loaf_keysystem"]:HasKey(tostring(success.data.id)) then
                    ...
                    for k, v in pairs(Config.Houses) do
                        ...
                        if Cache.owned_houses[sk] then
                            if Config.Blips.Owned then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 3, Strings["owned_" .. v.Type], 10)
                            end
                        elseif Cache.bought_houses[sk] then
                            if houseKeys[sk] and Config.Blips.Access then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 7, Strings["access_" .. v.Type], 10)
                            elseif Config.Blips.Others then
                                Cache["blip_house_" .. sk] = AddBlip(v.Entrance.xyz, 40, 2, Strings["purchased_" .. v.Type], 10)
                            end
                        else
            ...
            if Cache.NearbyHouses and #Cache.NearbyHouses > 0 then
                ...
                for k, v in pairs(Cache.NearbyHouses) do
                    if not Cache.owned_houses[v.houseid] and not Cache.bought_houses[v.houseid] then
                        if (not Config.REA or Config.AllowCircle) and Marker(29, v.data.Entrance.xyz, string.format(Strings["buy_house"], v.data.Label, v.houseid, FormatNumber(v.data.Price))) then
                        ...
                        elseif Config.REA and not Config.AllowCircle then
                            if Config.PreviewHouse.Enabled then
                                if Marker(0, v.data.Entrance.xyz, Strings["contact_rea"]:format(v.data.Label, v.houseid)) then
                                    PreviewHouse(v.data)
                                end
                            else
                                Marker(0, v.data.Entrance.xyz, Strings["contact_rea_nopreview"]:format(v.data.Label, v.houseid))
                            end
                        end
                    else
                        if Config.EnableGarage and (Cache.owned_houses[v.houseid] or exports["loaf_keysystem"]:HasKey(v.houseid) and Config.Options.Key.Garage) and v.data.Garage ~= nil then
                            --local vehicle = GetVehiclePedIsUsing(PlayerPedId())
							local pedin = IsPedInAnyVehicle(PlayerPedId())
                            if pedin then --DoesEntityExist(vehicle) then
                                if Marker(0, v.data.Garage.Exit.xyz + vec3(0.0, 0.0, 0.5), Strings["store_garage"]) then
                                    --TriggerEvent("loaf_garage:store", "Housing [" .. v.houseid .. "]")
									if #(v.data.Garage.Exit.xyz - GetEntityCoords(PlayerPedId())) <= 3.0 then
										StoreOwnedCarsMenu()
									end
                                end
                            else
                                if Marker(36, v.data.Garage.Entrance, Strings["enter_garage"]) then
									ListOwnedCarsMenu(v.data)
                                    --[[
									local viewing = true
                ...
                while #(GetEntityCoords(PlayerPedId()) - Config.IKEA.EnterIKEA.xyz) <= 25.0 do
                    ...
                    if Marker(29, Config.IKEA.EnterIKEA.xyz, Strings["enter_ikea"]) then
                        EnterIKEA()
...
function ListOwnedCarsMenu(datos)
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				menu.close()
				SpawnVehicle(data.current.value.vehicle, data.current.value.plate, datos)
			else
...
function SpawnVehicle(vehicle, plate, datos)
	ESX.Game.SpawnVehicle(vehicle.model, datos.Garage.Exit.xyz, datos.Garage.Exit.w, function(callback_vehicle)
