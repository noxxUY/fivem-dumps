...
Marker = function(type, position, text, control)
    DrawMarker(type, position, vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), vec3(0.4, 0.4, 0.4), 255, 255, 0, 100, false, true, 2, true, nil, nil, false)
    if #(GetEntityCoords(PlayerPedId()) - position) <= 2.0 and text then
...
RegisterNetEvent('loaf_housing:menu_transferir')
AddEventHandler('loaf_housing:menu_transferir', function()
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), "menu_transfer", {
		title = 'Que desea Transferir?',
		align = ((Config.Options and Config.Options.MenuAlign) or ("bottom-right")), 
		elements = {
			{label = 'Propiedad', value = "prop"},
			{label = 'Tienda', value = "tienda"},
		},
	}, function(data, menu)
		if data.current.value == "prop" then
			ESX.UI.Menu.CloseAll()
			TriggerServerEvent('loaf_housing:escritura')
		else
			ESX.UI.Menu.CloseAll()
			TriggerServerEvent('esx_kr_shops:escritura')
		end
	end, function(data, menu)
		menu.close()
	end)
...
RegisterNetEvent('loaf_housing:transferir')
AddEventHandler('loaf_housing:transferir', function(houseid)
	local elements = {}
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "select_player", {
        ...
        if player ~= "none" then
			local casas = {}
			for k, v in pairs(houseid) do
                table.insert(casas, {label = v.label..' - '..v.id, value = v.id})
            end
		
			ESX.UI.Menu.Open("default", GetCurrentResourceName(), "seleccionar_casa", {
                title = 'Selecionar Propiedad',
                align = ((Config.Options and Config.Options.MenuAlign) or ("bottom-right")), 
                elements = casas,
            }, function(data3, menu3)
                ESX.UI.Menu.Open("default", GetCurrentResourceName(), "confirm_transfer", {
					title = 'Confirma transferir: '..data3.current.label..' a '..data2.current.label,
					align = ((Config.Options and Config.Options.MenuAlign) or ("bottom-right")), 
					elements = {
						{label = Strings["yes"], value = "yes"},
						{label = Strings["no"], value = "no"},
					...
					if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(data2.current.ped)) <= 5.0 then
						if data4.current.value == "yes" then
							...
							end, player, data3.current.value)
						else
							menu4.close()
						end
					else
...
CreateThread(function()
    while true do
        ...
        if Cache.ManageTimeAndWeather then
            ...
            TriggerEvent("ToggleWeatherSync", false) -- desactiva la sincronización meteorológica. Deberá implementar este evento en su recurso de sincronización meteorológica.
			SetRainFxIntensity(0.0) -- tratando de eliminar charcos pero esto no parece hacerlo
            ...
            TriggerEvent("ToggleWeatherSync", true) --  activa la sincronización meteorológica. Deberá implementar este evento en su recurso de sincronización meteorológica.
        end
    end
end)
