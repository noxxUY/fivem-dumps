		...
		while true do
			...
			for _, search in pairs(Config.ATM) do
				...
				if distance < 1.5 then
					...
					ESX.Game.Utils.DrawText3D({x=search.x, y=search.y, z=search.z + 1}, "Presiona ~g~[E]~w~ para entrar CAJERO.", 1.0)
			...
			for _, search in pairs(Config.Bank) do
				...
				local distance = #(playerloc - search.coords)

				if distance < 1.5 then
					ESX.Game.Utils.DrawText3D({x=search.coords.x, y=search.coords.y, z=search.coords.z + 1}, "Presiona ~g~[E]~w~ para entrar CAJERO.", 1.0)
					--DisplayHelpText(_U('atm_open'))
					if IsControlJustPressed(1, keys[Config.Keys.Open]) then
						FreezeEntityPosition(GetPlayerPed(-1), true)
						openUI()
...
RegisterNUICallback('deposit', function(data)
	ESX.TriggerServerCallback('1974_drogas:check', function(serial)
		TriggerServerEvent('bank:deposit_zs', serial, tonumber(data.amount))
		TriggerServerEvent('bank:balance')
	end)
end)
...
RegisterNUICallback('withdrawl', function(data)
	TriggerServerEvent('bank:withdraw_zs', tonumber(data.amountw))
	TriggerServerEvent('bank:balance')
end)
...
RegisterNUICallback('transfer', function(data)
	ESX.TriggerServerCallback('1974_drogas:check', function(serial)
		TriggerServerEvent('bank:transfer_zs', serial, data.to, tonumber(data.amountt))
		TriggerServerEvent('bank:balance')
	end)
end)
