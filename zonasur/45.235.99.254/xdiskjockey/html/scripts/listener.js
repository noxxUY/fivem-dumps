...
function stopMusic() {
    $.post('http://xdiskjockey/stopMusic', JSON.stringify({}));
...
var app = new Vue({
    ...
    methods: {
        play: function (song) {
            ...
            song.playing = true;
            $.post('http://xdiskjockey/playMusic', JSON.stringify({song: song.url}));
        },
        pause: function (song) {
            if (song.playing) {
                song.playing = false;
        ...
        removeSong: function () {
            ...
            $.post('http://xdiskjockey/removeMusic', JSON.stringify(song));
        ...
        addSong: function () {
            ...
            if (this.validAddValues == true) {
                ...
                $.post('http://xdiskjockey/addMusic', JSON.stringify(song));
        ...
        editSong: function () {
            ...
            if (this.validEditValues) {
                ...
                $.post('http://xdiskjockey/editMusic', JSON.stringify(song));
            } else {
                sendAlert(locales.dataAreNotCorret, 'danger')
                return
            }
        ...
        updateVolume: function (up) {
            ...
            $.post('http://xdiskjockey/updateVolume', JSON.stringify({type: type}));
        }
    },
});
...
$(document).keyup(function (e) {
    if (e.key === "Escape") {
        if (isInBoard()) {
            $.post('http://xdiskjockey/exit', JSON.stringify({}));
            app.visible = false;
        } else {
            if(!isInModal()){
                setInBoard(true);
