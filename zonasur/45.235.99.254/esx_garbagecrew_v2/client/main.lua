...
AddEventHandler('esx_garbagecrew:enteredarea', function(zone)
	...
	if CurrentAction == 'collection' and not albetogetbags then
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == work_truck then
		...
		while CurrentAction ~= nil and CurrentActionMsg ~= nil do
			...
			if IsControlJustReleased(0, 38) then
				...
				if CurrentAction == 'endmission' then
					if GetVehiclePedIsIn(ped, false) == work_truck and GetPedInVehicleSeat(work_truck, -1 ) == ped then
						ESX.TriggerServerCallback('1974_drogas:check', function(serial)
							TriggerServerEvent('esx_garbagecrew:dropMoney', serial)
						end)
						DeleteEntity(work_truck)
						ESX.ShowNotification("Te devolvimos: ~g~$"..Config.Deposito.." ~w~del deposito del camion.")
					else
						ESX.ShowNotification('~r~No es la camion que te dimos! o no estas sentado en el asiento Conductor!')
					end
...
function MenuVehicleSpawner()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclespawner', {
			...
			ESX.TriggerServerCallback('esx_garbagecrew:getMoney', function(money)
				if money then
					ESX.Game.SpawnVehicle(data.current.value, Config.VehicleSpawn.pos, 270.0, function(vehicle)
						...
						currentstop = 0
						FindDeliveryLoc()
						ESX.ShowNotification('~g~CUANDO DEVUELVAS EL CAMION TE DEVOLVEMOS EL DEPOSITO!')
					end)
				end	
			end)
			menu.close()
