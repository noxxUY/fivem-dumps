...
function openmenuvehicle()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		if IsPedSittingInAnyVehicle(playerPed) then
			ESX.ShowNotification('No puedes abrir el baul desde adentro del vehiculo!')
			return
		end
		...
		if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
			CloseToVehicle = true
			--local vehFront = ESX.Game.GetVehicleInDirection()
			--local x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
			--local closecar = ESX.Game.GetVehicleInDirection()

			--if vehFront ~= nil and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
			if DoesEntityExist(vehicle) then
				...
				if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					...
					if locked == 1 or class == 15 or class == 16 or class == 14 then
						...
						ESX.ShowNotification('Este cofre está ~r~ cerrado')
					end
				end
			else
			...
			ESX.ShowNotification('~r~Este vehiculo tiene el baul cerrado por la empresa.')
		end
	end
end
local count = 0
  ...
  ESX.TriggerServerCallback('esx_truck:getPlayerInventory', function(inventory)
	...
	for i=1, #inventory.weapons, 1 do
		...
		table.insert(elements, {
			label = weapon.label .. ' [' .. weapon.ammo .. '] - ('..(getItemyWeight(weapon.name)/1000)..' Kg)',
			type  = 'item_weapon',
			value = weapon.name,
			ammo  = weapon.ammo
		})
