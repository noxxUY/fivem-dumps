...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(PlayerPedId(), false) and IsInAnimation then
			EmoteCancel()
		end
		
		if IsPedShooting(PlayerPedId()) and IsInAnimation and not broyale then
			EmoteCancel()
		end

		if PtfxPrompt and not broyale then
		...
		if Config.MenuKeybindEnabled and not broyale then 
			if IsControlPressed(0, Config.MenuKeybind) then
				if not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedArmed(PlayerPedId(), 4 | 2) then
					OpenEmoteMenu()
				else
					ESX.ShowNotification('~r~No puedes hacer esto dentro de un Vehiculo ni con Armas!')
				end
			end 
		end
		if Config.EnableXtoCancel and not broyale then 
			if IsControlPressed(0, 73) then
				if not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedArmed(PlayerPedId(), 4 | 2) and IsInAnimation then
					EmoteCancel()
				else
					ESX.ShowNotification('~r~No puedes hacer esto dentro de un Vehiculo ni con Armas!')
				end
...
RegisterCommand('e', function(source, args, raw)
	if not IsControlPressed(0, 73) and not broyale then
		EmoteCommandStart(args[1], raw)
	end
end)

RegisterNetEvent('dpemotes:emoteid')
AddEventHandler('dpemotes:emoteid', function(name)
	EmoteCommandStartId(name, false)
end)

RegisterCommand('emote', function(source, args, raw) EmoteCommandStart(args, raw) end)
if Config.SqlKeybinding then
...
--RegisterCommand('cancel', function(source, args, raw) EmoteCancel() end)
...
------ Functions and stuff --------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
RegisterNetEvent('dpemotes:cancel')
AddEventHandler('dpemotes:cancel', function()
	EmoteCancel()
end)
...
RegisterNetEvent('dpemotes:broyale')
AddEventHandler('dpemotes:broyale', function(bool)
	broyale = bool
end)

function EmoteCommandStartId(args, raw)
	if #args > 0 and not broyale then
		local name = string.lower(args)
...
function EmoteCommandStart(args, raw)
	if not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedArmed(PlayerPedId(), 4 | 2) then
		if #args > 0 and not broyale then
			local name = string.lower(args)
