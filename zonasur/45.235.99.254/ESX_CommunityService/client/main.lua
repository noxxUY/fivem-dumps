...
AddEventHandler('esx_communityservice:inCommunityService', function(actions_remaining)
	...
	ESX.Game.Teleport(GetPlayerPed(-1), Config.ServiceLocation)
	...
	while actionsRemaining > 0 and not communityServiceFinished do
		...
		if not communityServiceFinished then
			local coords = GetEntityCoords(GetPlayerPed(-1))
			local distance = Vdist(coords.x, coords.y, coords.z, Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z)	
			if distance > 45 then
				ESX.Game.Teleport(GetPlayerPed(-1), Config.ServiceLocation)
...
AddEventHandler('esx_communityservice:finishCommunityService', function(source)
	...
	isSentenced = false
	actionsRemaining = 0
	--ESX.Game.Teleport(GetPlayerPed(-1), Config.ReleaseLocation)
	SetEntityCoords(ped, Config.ReleaseLocation.x, Config.ReleaseLocation.y, Config.ReleaseLocation.z - 1.0)
...
Citizen.CreateThread(function()
	while true do
		...
		if actionsRemaining > 0 and isSentenced then
			--DrawMissionText( _U('remaining_msg', ESX.Math.Round(actionsRemaining)), 5)
			draw2dText( _U('remaining_msg', ESX.Math.Round(actionsRemaining)), { 0.350, 0.955 } )
