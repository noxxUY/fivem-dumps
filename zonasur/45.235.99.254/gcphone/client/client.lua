...
local reportCooldown = 1000

local onTimer = GetGameTimer()
local onTimerSMS = GetGameTimer()
...
RegisterNUICallback('reponseText', function(data, cb)
	...
	--[[
	if onTimerSMS > GetGameTimer() then
		local timeLeft = (onTimerSMS - GetGameTimer()) / 1000
		ESX.ShowNotification('Debes esperar ~y~'..ESX.Math.Round(timeLeft)..'~w~ segundos antes de enviar otro SMS.')
		return
	end
	
	onTimerSMS = GetGameTimer() + (reportCooldown * 10)
	]]--
...
RegisterNUICallback('sendMessage', function(data, cb)
	if data.message == '%pos%' then
		if onTimer > GetGameTimer() then
			local timeLeft = (onTimer - GetGameTimer()) / 1000
			ESX.ShowNotification('Debes esperar ~y~'..ESX.Math.Round(timeLeft)..'~w~ segundos para volver a enviar UBICACION.')
			return
		end
	
		onTimer = GetGameTimer() + (reportCooldown * 60)
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
		ESX.TriggerServerCallback('gcphone:getwebhook', function(webhook)
			exports['screenshot-basic']:requestScreenshotUpload(webhook, data.field, function(data)
