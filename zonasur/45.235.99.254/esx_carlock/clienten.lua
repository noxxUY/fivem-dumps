...
RegisterCommand("lock", function()
    local coords = GetEntityCoords(GetPlayerPed(-1))
		...
		cars = ESX.Game.GetVehiclesInArea(coords, 2.5) --5 seria la distancia ajustar a gusto
		local carstrie = {}
		local cars_dist = {}		
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("No hay vehículos para cerrar cerca.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Has ~r~BLOQUEADO~s~ tu ~y~'..vehicleLabel)
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Has ~g~DESBLOQUEADO~s~ tu ~y~'..vehicleLabel)
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							SetVehicleLights(carstrie[i], 2)
...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		local coords = GetEntityCoords(GetPlayerPed(-1))
		local hasAlreadyLocked = false
		local enclosure = exports["tm1_mafias"]:GetGroup()
		cars = ESX.Game.GetVehiclesInArea(coords, 5) --5 seria la distancia ajustar a gusto
		local carstrie = {}
		local cars_dist = {}		
		notowned = 0
		if #cars == 0 then
			...
			for i=1, #carstrie, 1 do
				...
				local vehicleLabel = GetDisplayNameFromVehicleModel(GetEntityModel(carstrie[i]))
				vehicleLabel = GetLabelText(vehicleLabel)
				--ESX.ShowNotification(vehicleLabel)
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						local lock = GetVehicleDoorLockStatus(carstrie[i])
						if lock == 1 or lock == 0 then
					...
					elseif vehicleLabel == 'AMBULAN' and hasAlreadyLocked ~= true then
					...
					elseif vehicleLabel == 'Taxi' and hasAlreadyLocked ~= true then
						if PlayerData ~= nil and PlayerData.job.name == "taxi" then
							local vehicleLabel = GetDisplayNameFromVehicleModel(GetEntityModel(carstrie[i]))
							vehicleLabel = GetLabelText(vehicleLabel)
							local lock = GetVehicleDoorLockStatus(carstrie[i])
							if lock == 1 or lock == 0 then
					...
					elseif carstrie[i] == camion and hasAlreadyLocked ~= true then
							local vehicleLabel = GetDisplayNameFromVehicleModel(GetEntityModel(carstrie[i]))
							vehicleLabel = GetLabelText(vehicleLabel)
							local lock = GetVehicleDoorLockStatus(carstrie[i])
							if lock == 1 or lock == 0 then
					...
					elseif string.upper(plate) == "POLICIA" then
					...
					elseif string.sub(platetrim, 1, -2) == string.upper(enclosure.enclosure) and hasAlreadyLocked ~= true then
						local vehicleLabel = GetDisplayNameFromVehicleModel(GetEntityModel(carstrie[i]))
						vehicleLabel = GetLabelText(vehicleLabel)
						local lock = GetVehicleDoorLockStatus(carstrie[i])
						if lock == 1 or lock == 0 then
					...
					elseif string.upper(plate) == "GNAZS" then
						if PlayerData ~= nil and PlayerData.job.name == "gendarme" then
							local vehicleLabel = GetDisplayNameFromVehicleModel(GetEntityModel(carstrie[i]))
							vehicleLabel = GetLabelText(vehicleLabel)
							local lock = GetVehicleDoorLockStatus(carstrie[i])
							if lock == 1 or lock == 0 then
