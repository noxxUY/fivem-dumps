...
function IsJobTrucker() -- Check si le joueur est bien du métier
	if PlayerData ~= nil then
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'mlibre' then
			isJobTrucker = true
		end
		return isJobTrucker
	end
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.TriggerServerCallback('1974_drogas:check', function(serial)
					TriggerServerEvent('gopostal_job:caution', serial, 'take', Config.Caution)
				end)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'distribution', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'distribution_choice', {
			...
			if data2.current.value == 'pick' then
				ESX.TriggerServerCallback('1974_drogas:check', function(serial)
					if serial then
						TriggerServerEvent('gopostal_job:Item', data.current.item, data.current.value, data.current.label, 'pick', serial)
					end
				end)
			elseif data2.current.value == 'deposit' then
				ESX.TriggerServerCallback('1974_drogas:check', function(serial)
					if serial then
						TriggerServerEvent('gopostal_job:Item', data.current.item, data.current.value, data.current.label, 'deposit', serial)
					end
				end)
			end
...
AddEventHandler('gopostal_job:hasEnteredMarker', function(zone)
	...
	if zone == 'VehicleDeleter' and isInService then
		...
		if GetVehiclePedIsIn(playerPed, false) == camion and GetPedInVehicleSeat(camion, -1) == playerPed then
			...
			ESX.ShowNotification('No es el vehiculo que te dimos, o no estas sentado en el asiento del conductor.')		
		end
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and IsJobTrucker() then -- Touche E
                ...
                if CurrentAction == 'vehicledeleter' then
					...
					ESX.TriggerServerCallback('1974_drogas:check', function(serial)
						TriggerServerEvent('gopostal_job:caution', serial, "give_back", giveBack)
					end)
...
function LetterAndColis()
	...
	if zone.Pos[progress].colis then
		colis = GetRandomIntInRange(Config.MinColis, Config.MaxColis)
	else
		colis = 0
	end
		    	...
		    	local coords = GetEntityCoords(GetPlayerPed(-1))
		      	...
		      	if GetDistanceBetweenCoords(coords, zone.Pos[progress].x, zone.Pos[progress].y, zone.Pos[progress].z, true) < 100.0 then
		      		...
		      		if GetDistanceBetweenCoords(coords, zone.Pos[progress].x, zone.Pos[progress].y, zone.Pos[progress].z, true) < 3.0 then
			        	HelpPromt(_U('pickup'))
			        	if IsControlJustReleased(0, 38) and IsJobTrucker() and isInService and not entregando then
							entregando = true
			        		if not IsPedInAnyVehicle(PlayerPedId(), false) then
								ESX.TriggerServerCallback('1974_drogas:check', function(serial)
									TriggerServerEvent('gopostal_job:end', serial, lettre, colis)
