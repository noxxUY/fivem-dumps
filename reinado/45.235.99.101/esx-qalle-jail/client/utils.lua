RegisterCommand("jailmenu", function(source, args)
	...
	if PlayerData.job.name == "police" then
		...
		ESX.ShowNotification("No sos un policia!")
	end
end)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Boiling Broke" or action == "Security" then
				...
				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("No tienes una llave para ingresar!")
					return
				end
			end
...
Citizen.CreateThread(function()
    ...
    AddTextComponentString('Penitenciaria de Alcatraz')
