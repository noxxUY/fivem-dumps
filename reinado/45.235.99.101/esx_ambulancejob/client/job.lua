...
function OpenAmbulanceActionsMenu()
	...
	{label = 'Deposito',     value = 'inventario'}
	}
...
function OpenCloakroomMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		elseif data.current.value == 'ambulance_wear' then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
			--	if skin.sex == 0 then
                       SetPedComponentVariation(PlayerPedId(), 4, 86, 2, 2)
                       SetPedComponentVariation(PlayerPedId(), 8, 20, 0, 2)
                       SetPedComponentVariation(PlayerPedId(), 6, 77, 1, 2)  
                       SetPedComponentVariation(PlayerPedId(), 11, 244, 0, 2) 
