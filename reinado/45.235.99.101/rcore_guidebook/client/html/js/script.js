    ...
    methods: {
        ...
        initAdminPage(page, callRequest) {
            ...
            if (callRequest) {
                ...
                requestPage(page.key, function (data) {
                    ...
                    app.adminData.editedPage.collision = false;
                    app.loading = false;
                    $('.guide__body').scrollTop(0);
                }, app.showErrorPage);
            } else {
        ...
        initPointPage(point) {
            ...
            if (point) {
                ...
                data.marker_rotation = data.marker_rotation || {
                    x: 0,
                    y: 0,
                    z: 0,
                }
        ...
        saveForm(type) {
            ...
            if (type === 'POINT') {
                ...
                data.marker_rotation.x = parseFloat(data.marker_rotation.x);
                data.marker_rotation.y = parseFloat(data.marker_rotation.y);
                data.marker_rotation.z = parseFloat(data.marker_rotation.z);
            }
    ...
    window.addEventListener("message", function (event) {
        ...
        if (item.type === "REFRESH_PAGE") {
            if (pageCache[item.key] !== undefined) {
                pageCache[item.key] = undefined;
            }

            if (app.adminData.editedPage && app.adminData.editedPage.key === item.key) {
                app.adminData.editedPage.collision = true;
            }
        }
