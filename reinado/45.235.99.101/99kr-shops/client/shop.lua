    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("Saliste de la tienda, tu canasta ahora está vacía!", "error", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("No tenes plata!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Poner " .. data.current.value .. " unidades de " .. data.current["realLabel"] .. " en la canasta", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        ...
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
                label = '<span style="color:red">*</span> ' .. item["label"] .. ': ' .. item["amount"] .. ' unidades (<span style="color:green">$' .. item["price"] * item["amount"] .. '</span>)',
                value = "item_menu",
                index = i
            })
        end
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Eliminó todo de su canasta.", "error", 2500)
                end
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Sacar " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " de la canasta.", "alert", 2500)
