...
cfg.blackjackTables = {
    ...
    [3] = {
        dealerPos = vector3(1014.79,47.24,72.28),
        dealerHeading = 282.89,
        tablePos = vector3(1015.583, 47.486, 72.276),
        tableHeading = 103.304,
        distance = 800.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
}
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Tu mano es ["..tostring(currentHand).."], la mano del Crupier es ["..tostring(dealersHand).."]")
    ...
    while true do 
        if waitingForBetState then
            ...
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
				...
				if tonumber(currentBetAmount) > 0 and tonumber(currentBetAmount) < 5000001 then
                    ...
                    notify("~r~Monto invalido. o superior a 5 millones")
                end
            end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "El objetivo del Blackjack es ganarle a la mano del crupier sin pasar de 21. Este juego usa barajas de 52, que se mezclan al comienzo de cada mano. El crupier siempre se planta en 17 o mas.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
Citizen.CreateThread(function()
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                DrawRect(0.933, 0.928, 0.115, 0.032, 0, 0, 0, 150) 
                DrawAdvancedNativeText(0.991, 0.935, 0.005, 0.0028, 0.29, "Apuesta:", 255, 255, 255, 255, 0, 0)
        ...
        if drawTimerBar then
            DrawRect(0.944, 0.886, 0.091, 0.032, 0, 0, 0, 150)
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("No apostaste, espera a la siguiente ronda.")
        end
    end)
end)
...
function setupBlackjackInstructionalScaleform(scaleform)
    ...
    ButtonMessage("Aumentar apuesta") --Page Up
    PopScaleformMovieFunctionVoid()
...
function getGenericTextInput(type)
	...
	AddTextEntry('FMMC_MPM_NA', "Ingrese " .. tostring(type))
	DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "Ingrese " .. tostring(type) .. " message", "", "", "", "", 30)
