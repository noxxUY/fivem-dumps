			...
			if IsControlJustPressed(0, Config.Hotkey.key) then
				...
				if IsPedArmed(playerPed, 4) then
					if hash then
						...
						logging('debug', ('[HOTKEY] [Add Ammo] Item: %s, Weapon: %s'):format(itemName, weapon.name))
...
AddEventHandler('msk_weaponammo:checkItem', function(clip)
	...
	if IsPedArmed(playerPed, 4) then
		if hash then
			...
			if not Config.Weapons[clip] then 
				logging('debug', ('[ITEM] [Add Ammo] Item %s is not configured in config.lua'):format(clip))
			end
			logging('debug', ('[ITEM] [Add Ammo] Item: %s, Weapon: %s'):format(clip, weapon.name))
