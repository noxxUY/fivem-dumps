	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			if options.deposit then
				...
				table.insert(elements, {label = 'Promover', value = 'subirrango'})
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
				...
				elseif data.current.value == 'subirrango' then
...
-- ADD AGREGADO  MAX PLAYER
RegisterNetEvent('Roda-GetPlayerTotalJobs:xD')
AddEventHandler('Roda-GetPlayerTotalJobs:xD', function(total)
	jobcount = total
	local society = ESX.GetPlayerData().job.name
	if jobcount < Config.MaxSlots[""..society..""] then
		OpenRecruitMenu(society)
	else
		ESX.ShowNotification('Tienes muchos empleados, tu maximo de slots es: '..Config.MaxSlots[society]..'')
	end
end)
-- ADD AGREGADO
...
function OpenManageEmployeesMenu(society)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society, {
		...
		if data.current.value == 'recruit' then
			if Config.ActivarSlots then 
			local trabajo = ESX.GetPlayerData().job.name
			TriggerServerEvent('Roda-GetPlayersJob', GetPlayerServerId(PlayerId()), trabajo)
			else
				OpenRecruitMenu(society)
			end
