...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	ESX.TriggerServerCallback('esx_lscustomvip:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_lscustomvip:installMod')
AddEventHandler('esx_lscustomvip:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_lscustomvip:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_lscustomvip:cancelInstallMod')
AddEventHandler('esx_lscustomvip:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_lscustomvip:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent('esx_lscustomvip:buyMod', price)
		...
		menu.close()
		TriggerEvent('esx_lscustomvip:cancelInstallMod')
...
CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if (ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanicvip') or not Config.IsMechanicJobOnly then
				for k,v in pairs(Config.Zones) do
