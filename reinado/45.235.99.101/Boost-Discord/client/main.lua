...
AddEventHandler('Boost-Discord:SetPresence', function(data)
	...
	if Config.UseESXIdentity then
		SetRichPresence((Config.RichPresence):format(GetPlayerServerId(player), data['IdentityName'], Config.PlayerText, data['ActivePlayers']*2, tostring(Config.PlayerCount)))
	else
		SetRichPresence((Config.RichPresence):format(GetPlayerServerId(player), data['PlayerName'], Config.PlayerText, data['ActivePlayers']*2, tostring(Config.PlayerCount)))
	end
	SetDiscordRichPresenceAssetText('ReinadoRP! 1 Año')
end)
