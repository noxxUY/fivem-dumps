...
Radio.Commands = {
	{
		...
		Help = "Oculta/Muestra Radio Menu", -- Command help shown in chatbox when typing the command
		Params = {},
	...
	},
	{
		...
		Help = "Cambie la frecuencia de la radio", -- Command help shown in chatbox when typing the command
		Params = {
			{name = "number", "Ingrese Frecuencia"}
		},
		Handler = function(src, args, raw)
			if Radio.Has then
				if args[1] then
...
Citizen.CreateThread(function()
	...
	--	if isActivatorPressed and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has and not isDead then
		if isActivatorPressed and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has then
			Radio:Toggle(not Radio.Open)
	--	elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
		elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has)) then
			Radio:Remove()
		...
		if Radio.Open then
			...
			if not Radio.On then
				AddTextComponentSubstringPlayerName(Radio.Clicks and "~r~desactivar~w~" or "~g~activar~w~")
			end
