...
CreateThread(function()
	while true do
		SendNUIMessage({ action = 'setPlayerId', value = GetPlayerServerId(NetworkGetEntityOwner(PlayerPedId())) })
		Wait(1000)
	end
end)
...
Citizen.CreateThread(function()
	   ...
	   local coins
        ...
        for i=1, #ESX.PlayerData.accounts, 1 do
          ...
          elseif ESX.PlayerData.accounts[i].name == 'bank' then
			...
			elseif ESX.PlayerData.accounts[i].name == 'coins' then
                coins = ESX.PlayerData.accounts[i].money
          end
        end
	...
	SendNUIMessage({ action = 'setMoney', id = 'coins', value = coins })
