    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            elseif checkArray (Bullet, death) then
                TriggerServerEvent('playerDied', killername .. " disparo a " .. playerName)
            elseif checkArray (Knife, death) then
                TriggerServerEvent('playerDied', killername .. " apuñalo a " .. playerName)
            ...
            elseif checkArray (FallDamage, death) then
                TriggerServerEvent('playerDied', playerName .. " murio de daño por caida")
            elseif checkArray (Explosion, death) then
                TriggerServerEvent('playerDied', playerName .. " murio por una explosion")
            elseif checkArray (Gas, death) then
                TriggerServerEvent('playerDied', playerName .. " died of gas")
            elseif checkArray (Burn, death) then
                TriggerServerEvent('playerDied', playerName .. " se quemo vivo")
            elseif checkArray (Drown, death) then
                TriggerServerEvent('playerDied', playerName .. " se ahogo")
            else
                TriggerServerEvent('playerDied', playerName .. " murio por una fuerza desconocida")
                --print(death)
            end
