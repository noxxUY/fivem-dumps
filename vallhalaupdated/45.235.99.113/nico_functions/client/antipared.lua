...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if weapon > 0 and IsPlayerFreeAiming(pedid) then
            local hitW, coordsW, entityW = RayCastGamePlayWeapon(weapon, 5.0, 1)
            local hitC, coordsC, entityC = RayCastGamePlayCamera(weapon, 500.0, 1)
...
RegisterCommand('c', function(source, args, rawCommand)
	if args[1] and args[1] == "m" then
		local coords, heading = GetEntityCoords(PlayerPedId()), GetEntityHeading(PlayerPedId())
		SendNUIMessage({
			type = 'clipboard',
			data = '' .. coords.x .. ' ' .. coords.y .. ' ' .. coords.z .. ''
		})
		exports['mythic_notify']:DoCustomHudText('info', ('Coordenadas copiadas correctamente. ('..heading..')'), 3000)
	else
		local coords, heading = GetEntityCoords(PlayerPedId()), GetEntityHeading(PlayerPedId())
		SendNUIMessage({
			type = 'clipboard',
