...
function getPoints()
    ...
    ESX.TriggerServerCallback('guille_gangs:server:getGangsData', function(gang, rank, data, boss, identifier)
        if gang then
            ...
            enablePoints()
            if (mafiaBlip and DoesBlipExist(mafiaBlip)) then
                RemoveBlip(mafiaBlip)
            end

            if (gangData and gangData.points) then
                local coords
                for i,v in pairs(gangData.points) do
                ...
                if (coords) then
                    mafiaBlip = AddBlipForCoord(coords.x, coords.y, coords.z)
                    SetBlipSprite(mafiaBlip, 437)
                    SetBlipColour(mafiaBlip, 1)
                    SetBlipScale(mafiaBlip, 0.8)
                    SetBlipAsShortRange(mafiaBlip, true)
                    BeginTextCommandSetBlipName("STRING")
                    AddTextComponentString("Sede de organización")
            ...
            for k, v in pairs(gangData.points) do
                ...
                if dist < 20 then
                    ...
                    if not editingPoints then 
                        DrawMarker(31, v.coords.x, v.coords.y, v.coords.z+0.9, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, 255, 255, 255, 255, 0, 0, 0, 0, 0, 0, 0)
...
function openBossMenu()
    ...
    for k, v in pairs(gangData.members) do
        if v.member.name and v.member.steam then
...
RegisterNUICallback("vehs", function(cb)
    ...
    SetVehicleCustomSecondaryColour(veh, tonumber(gangData.colors.r), tonumber(gangData.colors.g), tonumber(gangData.colors.b))
    SetVehicleWindowTint(veh, 1)
    SetVehicleMaxMods(veh)
	exports['valhalla_fuel']:SetFuel(veh, 100)
end)
...
function openShop()
    ...
    for k, v in pairs(gangData.shop) do
        table.insert(data, {text = v.label.."<span style='color:red'>"..v.price.."$</span>", toDo = v.name})
    end
...
RegisterCommand('gangmenu', function()
    if gangName ~= nil then
        local ped = PlayerPedId()
        local elements = {}
        table.insert(elements, { label = ('<i class="fas fa-search"></i> Cachear'), value = "search" })
        table.insert(elements, { label = ('<i class="fas fa-lock"></i> Precintar'), value = "handcuff" })
        table.insert(elements, { label = ('<i class="fas fa-user-alt-slash"></i> Desprecintar'), value = "unarrest" })
        table.insert(elements, { label = ('<i class="fas fa-paper-plane"></i> Meter al vehículo'), value = "vehiclein" })
        table.insert(elements, { label = ('<i class="fas fa-minus-circle"></i> Sacar del vehículo'), value = "vehicleout" })
        table.insert(elements, { label = ('<i class="fas fa-handshake"></i> Agarrar / Soltar'), value = "escort" })
...
RegisterKeyMapping('gangmenu', (_U("open_gang_me")), 'keyboard', 'F6')
...
function OpenBodySearchMenu(player)
    TriggerEvent('inventory:openPlayerInventory', GetPlayerServerId(player))
