...
function StartNPCJob()
	...
	exports['mythic_notify']:DoCustomHudText('cajaverde' , ('¡Un nuevo trabajo fue marcado en tu GPS!') , 7000)
    Done = true
end
...
function StopNPCJob(cancel)
    ...
    if cancel then
	  exports['mythic_notify']:DoCustomHudText('cajaroja' , ('¡Cancelaste el trabajo!') , 5000)
	else
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            if data.current.value == 'citizen_wear' then
				...
				CreateBlip()
				menu.close()
                exports['mythic_notify']:DoCustomHudText('cajaverde' , ('¡Jornada finalizada!') , 7000)
            ...
            if data.current.value == 'job_wear' then
				...
				CreateBlip()
                menu.close()
				exports['mythic_notify']:DoCustomHudText('cajaverde' , ('¡Iniciaste la jornada laboral!') , 7000)
				exports['mythic_notify']:DoCustomHudText('cajaverde' , ('¡Presiona F7 para ponerte en servicio!') , 7000)
				local playerPed = PlayerPedId()
				...
				if PlayerData.job.name == Config.nameJob then
					if CurrentAction == 'cloakroom_menu' then
						if IsPedInAnyVehicle(playerPed, 0) then
							exports['mythic_notify']:DoCustomHudText('cajaroja' , ('¡Necesitas estar en el vehículo!') , 5000)
						else
							CloakRoomMenu()
						end
					end
					  ...
					  if string.find (plate, platePrefix) then
						...
						if hash == GetHashKey(truck.Hash) then
							if GetVehicleEngineHealth(vehicle) <= 500 or GetVehicleBodyHealth(vehicle) <= 500 then
								exports['mythic_notify']:DoCustomHudText('cajaroja' , ('¡Necesitas reparar el vehículo para continuar la jornada!') , 5000)
							else
						...
						exports['mythic_notify']:DoCustomHudText('cajaroja' , ('¡Solo puedes guardar el vehículo laboral!') , 5000)
					  end
					end
               	    CurrentAction = nil
				end
...
RegisterCommand('empleadomuni', function()
	if (PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob) then
		if Onjob then
			StopNPCJob(true)
			RemoveBlip(Blips['NPCTargetPool'])
			...
			if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("boxville")) then
				...
				exports['mythic_notify']:DoCustomHudText('cajaroja' , ('¡Necesitas estar en el vehículo laboral!') , 5000)
			end
		end
	end
end)
RegisterKeyMapping('empleadomuni', 'Empleo: Empleado Municipal', 'keyboard', 'F7')
