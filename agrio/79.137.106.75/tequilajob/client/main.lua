  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'cachear' then
        cacheartequilala(closestPlayer)
      end
      ...
      if data.current.value == 'esposar' then
        ExecuteCommand('me Le pone las esposas')
						...
						if closestDistance <= 2.0 then
							TriggerServerEvent('tequilala:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
    ...
    elseif action == 'desesposar' then
      ...
      local target_id = GetPlayerServerId(closestPlayer)
      if closestDistance <= 2.0 then
        TriggerServerEvent('tequilala:requestrelease', target_id, playerheading, playerCoords, playerlocation)
      else
...
function cacheartequilala(player)
	ESX.TriggerServerCallback('tequilala:getOtherPlayerData', function(data)
		local elements = {}
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(player), 'society_tequilala', _U('billing'), amount)
        end
...
-----------------------
----- TELEPORTERS -----
RegisterKeyMapping('menutequilala', 'Menu de tequilala(Only for job)', 'keyboard', 'F6' )
RegisterCommand('menutequilala', function() if IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'tequilala_actions') then
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('tequilala:unrestrain')
...
RegisterNetEvent('tequilala:handcuff132')
AddEventHandler('tequilala:handcuff132', function()
...
RegisterNetEvent('tequilala:unrestrain')
AddEventHandler('tequilala:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
