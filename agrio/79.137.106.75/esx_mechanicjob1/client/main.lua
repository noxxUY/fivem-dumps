...
function OpenMechanicActionsMenu2()
	print('2 jaja')
	local elements = {
		{label = _U('vehicle_list'),   value = 'vehicle_list'},
		{label = 'Coger vehiculo de empresa',  value = 'cogercochegc'},
		{label = _U('work_wear'),      value = 'cloakroom'},
		{label = _U('civ_wear'),       value = 'cloakroom2'},
		{label = _U('deposit_stock'),  value = 'put_stock'},
		{label = 'Radio privada [301]',  value = 'radiofrec'},
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones2.VehicleSpawnPoint22.Pos, 270.0, function(vehicle)
				...
				local elements = {
					...
					{label = 'Audi prestado', value = 'AAQ4'}
				}
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones2.VehicleSpawnPoint2.Pos, 90.0, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						end)
					else
		...
		elseif data.current.value == 'radiofrec' then
			exports["rp-radioja"]:GivePlayerAccessToFrequencies(301)
			ESX.ShowNotification('Concedido el acceso de radio [301]')
		elseif data.current.value == 'cogercochegc' then
		local elements = {
			{label = 'Audi prestado', value = 'AAQ4'}
		}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
			...
			if Config.MaxInService == -1 then
				ESX.Game.SpawnVehicle(data.current.value, Config.Zones2.VehicleSpawnPoint2.Pos, 90.0, function(vehicle)
					local playerPed = PlayerPedId()
					TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
					ESX.ShowNotification('Este ~b~vehiculo es de la ~r~empresa,úsalo para desplazarte hasta el ~g~Garage más ~r~cercano ~w~y coger tu ~y~vehículo personal.')
				end)
			else
				ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
					if canTakeService then
		...
		CurrentAction     = 'mechanic_actions_menu2'
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'recrue' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_craft', {
			...
			if data.current.value == 'blow_pipe' then
				TriggerServerEvent('esx_mechanicjobgfda:startCraft')
			elseif data.current.value == 'fix_kit' then
				TriggerServerEvent('esx_mechanicjobgfda:startCraft2')
			elseif data.current.value == 'caro_kit' then
				TriggerServerEvent('esx_mechanicjobgfda:startCraft3')
			end
		end, function(data, menu)
			menu.close()
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = 'Acceso a frecuencia de radio [300]',      value = 'frec'},
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(closestPlayer), 'society_mechanic', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
			end)
		elseif data.current.value == 'frec' then
			exports["rp-radioja"]:GivePlayerAccessToFrequencies(300)
			ESX.ShowNotification('Concedido el acceso de radio de MECÁNICOS [300]')
		elseif data.current.value == 'esposar' then
			if ESX.GetPlayerData().job.grade == 4 or ESX.GetPlayerData().job.grade == 7 and poderhacer == true then
		...
		elseif data.current.value == 'object_spawner' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions_spawn', {
				...
				elements = {
					...
					{label = _U('barrier'),  value = 'prop_mp_barrier_02'}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_mechanicjob:getStockItem', itemName, count);OpenGetStocksMenu()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_mechanicjob:putStockItems', itemName, count);OpenPutStocksMenu()
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones2.MechanicActions2.Pos.x, Config.Zones2.MechanicActions2.Pos.y, Config.Zones2.MechanicActions2.Pos.z)
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				if CurrentAction == 'mechanic_actions_menu' then
					OpenMechanicActionsMenu()
				elseif CurrentAction == 'mechanic_actions_menu2' then
					OpenMechanicActionsMenu2()
