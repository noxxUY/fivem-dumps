...
local dui2 = GetDuiHandle(CreateDui('https://i.giphy.com/Hv0MRrML4RiHm.gif', 920, 450))
CreateRuntimeTextureFromDuiHandle(CreateRuntimeTxd('kkTatos'), 'Gradient_Bgd',dui)
CreateRuntimeTextureFromDuiHandle(CreateRuntimeTxd('kkTatos1'), 'Gradient_Bgd',dui2)
--CreateRuntimeTextureFromDuiHandle(CreateRuntimeTxd('kkTatos'), 'shop_arrows_upanddown',dui2)
...
local function drawTitle()
    if menus[currentMenu] then
		...
		elseif menus[currentMenu].titleFont == "~sprite~" then
			...
			HasStreamedTextureDictLoaded(textDict)
			DrawSprite(textDict, sprite, x, y, menuWidth, titleHeight, 0, color[1], color[2], color[3], color[4])
			drawText(menus[currentMenu].title, x, y - titleHeight / 2 + titleYOffset, 1, {245, 90, 18, 100}, titleScale, true)
		else
            if HasStreamedTextureDictLoaded("kkTatos") then
                SetUiLayer(0)
                DrawSprite("kkTatos", "Gradient_Bgd", x, y, menuWidth, titleHeight, 0.0, 245, 90, 18, 100, 0)
            end
			--drawRect(x, y, menuWidth, titleHeight, menus[currentMenu].titleBackgroundColor)
			drawText(menus[currentMenu].title, x, y - titleHeight / 2 + titleYOffset,2, {245, 210, 18, 230}, titleScale, true)
        end
...
local function drawDescription()
    if menus[currentMenu] then
        ...
        if HasStreamedTextureDictLoaded("kkTatos") then
			...
			DrawSprite("kkTatos", "Gradient_Bgd", x, y, menuWidth, descriptionHeight, 0.0, 255, 255, 255, 220, 0)
            ...
            RequestStreamedTextureDict("kkTatos")
        end
...
local function drawMenuBackground()
    if menus[currentMenu] then
        ...
        if HasStreamedTextureDictLoaded("kkTatos1") then
            SetUiLayer(0)
            DrawSprite("kkTatos1", "Gradient_Bgd", x, y, menuWidth, menuHeight, 0.0, 255, 205, 10, 100, 0)
        else
            RequestStreamedTextureDict("kkTatos1")
        end
...
local function drawArrows()
    ...
    if HasStreamedTextureDictLoaded("kkTatos") then
        ...
        DrawSprite("kkTatos", "Gradient_Bgd", x, y, 0.0265, 0.05, 0.0, 255, 200, 0, 255, 0)
    else
        RequestStreamedTextureDict("kkTatos")
    end
...
local function drawButton(text, subText)
    ...
    if multiplier then
        ...
        if text ~= "!!separator!!" then
            if menus[currentMenu].currentOption == optionCount and HasStreamedTextureDictLoaded("kkTatos") then
                SetUiLayer(1)
                DrawSprite("kkTatos", "Gradient_Nav", x, y, menuWidth, buttonHeight, 0.0, 255, 255, 10, 105, 0)
            end
...
local function drawDisabledButton(text, subText)
    ...
    if multiplier then
        ...
        --[[if menus[currentMenu].currentOption == optionCount and HasStreamedTextureDictLoaded("kkTatos") then
            SetUiLayer(1)
            DrawSprite("kkTatos", "Gradient_Nav", x, y, menuWidth, buttonHeight, 0.0, 255, 255, 255, 255, 0)
        end]]
        --drawRect(x, y, menuWidth, buttonHeight, backgroundColor)
        drawText(text, menus[currentMenu].x + buttonTextXOffset, y - (buttonHeight / 2) + buttonTextYOffset, buttonFont, textColor, buttonScale, false, shadow)
...
local function drawSpriteButton(text, textDict, sprite, focusSprite)
    ...
    if multiplier then
        ...
        drawText(text, menus[currentMenu].x + buttonTextXOffset, y - (buttonHeight / 2) + buttonTextYOffset, buttonFont, {100,250,0,150}, buttonScale, false, shadow)
...
function JayMenu.CreateMenu(id, title, closeCallback)
    ...
    menus[id].menuFocusBackgroundColor = {245, 245, 245, 100}
...
function JayMenu.SetDescription(text)
 ...
 function JayMenu.CheckBox(text, bool, callback)
    ...
    if JayMenu.SpriteButton(text, "kkTatos", sprite, focusSprite) then
        bool = not bool
