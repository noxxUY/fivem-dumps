...
ESX.ShowNotification = function(msg, job)
    TriggerEvent("co_notify:ShowNotification", msg)
end
...
ESX.ShowHelpNotification = function(msg, thisFrame, beep, duration)
    TriggerEvent("co_notify:ShowHelpNotification", msg)
end

ESX.ShowFloatingHelpNotification = function(msg, thisFrame, beep, duration)
    TriggerEvent("co_notify:ShowFloatingHelpNotification", msg)
end
...
--	})
  --end
  --[[ESX.ShowNotification = function(msg, job)
   --[[ if exports.pk_notifications ~= nil then
        exports.pk_notifications:PkShowNotification(msg, job)
    else
        SetNotificationTextEntry('STRING')
        AddTextComponentSubstringPlayerName(msg)
        DrawNotification(false, true)
    end]]--
--	TriggerEvent('snshowNotify', msg)
--end]]--
...
ESX.ShowFloatingHelpNotification = function(msg, coords)
	if tipe == nil then tipe = 3 end
   
	if coords == nil then coords = GetEntityCoords(GetPlayerPed(-1)) end
	AddTextEntry('esxFloatingHelpNotification', msg)
	SetFloatingHelpTextWorldPosition(1, coords)
   
	SetFloatingHelpTextStyle(1, 1, 12, -1, tipe, 1)
...
ESX.ShowInventory = function()
	...
	table.insert(elements, {
		label     = 'Trabajo: ' .. ESX.PlayerData.job.label .. ' - ' .. ESX.PlayerData.job.grade_label	
	})

	for k,v in pairs(ESX.PlayerData.accounts) do
		if v.money > 0 then
			local formattedMoney = _U('locale_currency', ESX.Math.GroupDigits(v.money))
			local canDrop = v.name ~= 'bank' and v.name ~= 'agrio_coins'

			table.insert(elements, {
				label = ('<span style="color:yelow;">%s:</span> <span style="color:green;">%s</span>'):format(v.label, formattedMoney),
				count = v.money,
				type = 'item_account',
				value = v.name,
				usable = false,
	...
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.count > 0 then
			...
			table.insert(elements, {
				label = ('<span style="color:orange;">%s</span> x%s'):format(v.label, v.count),
				count = v.count,
				type = 'item_standard',
				value = v.name,
				usable = v.usable,
	...
	for i=1, #Config.Weapons, 1 do
		...
		if HasPedGotWeapon(playerPed, weaponHash, false) and Config.Weapons[i].name ~= 'WEAPON_UNARMED' then
			...
			table.insert(elements, {
				label     = '🔫<span style="color:red;">'..Config.Weapons[i].label .. '</span>[' .. ammo .. ']',
				count     = 1,
				type      = 'item_weapon',
			...
			if data1.current.action == 'give' then
				...
				if #playersNearby > 0 then
					...
					ESX.TriggerServerCallback('esx:getPlayerNames', function(returnedPlayers)
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) and not IsPedFalling(selectedPlayerPed) then
									if type == 'item_weapon' then
										...
										menu2.close()
										menu1.close()--]]
										local closestPed = selectedPlayerPed
						...
						TriggerServerEvent('esx:giveInventoryItem', selectedPlayerId, type, item, sourceAmmo)
						menu2.close()
						menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity and quantity > 0 and data.current.count >= quantity then
												TriggerServerEvent("ServerValidEmote", selectedPlayerId, 'give', 'give2')
...
local debugSnowi = false
RegisterCommand('debugEstended', function()
debugSnowi = true
end)
RegisterNetEvent('esx:serverCallback')
AddEventHandler('esx:serverCallback', function(requestId, ...)
	if debugSnowi == true then
	print(tostring(requestId))
	local payload = msgpack.pack({...})
		local pay = msgpack.unpack(payload)
	print('Callback info: '..json.encode(pay))
	end
...
AddEventHandler('esx:showHelpNotification', function(msg, thisFrame, beep, duration)
	ESX.ShowFloatingHelpNotification(msg, thisFrame, beep, duration)
end)
