...
function OpenVaultMenu()
  ...
  if Config.EnableVaultManagement then
    ...
    local elements = {
     -- {label = 'Comprar armas', value = 'buy_weapons'},
...
function OpenSocietyActionsMenu()
  ...
  table.insert(elements, {label = 'Ver DNI', value = 'identity_card'})
	table.insert(elements, {label = 'Cachear', value = 'body_search'})
    ...
    function(data, menu)
      ...
      if data.current.value == 'identity_card' then
        OpenIdentityCardMenu(closestPlayer)
						ExecuteCommand('me Le busca el dni')
						ExecuteCommand('do donde estaria?')
      end
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              'default', GetCurrentResourceName(), 'menu_crafting',
              {
                  ...
                  align = 'bottom-right',
                  elements = {
                      {label = 'Bomba',     value = 'jagerbomb'},
                      {label = 'golem',         value = 'golem'},
                      {label = 'Whisky',    value = 'whiskycoca'},
                      {label = 'Vodka Negro',   value = 'vodkaenergy'},
                      {label = 'Vodka Fruta',    value = 'vodkafruit'},
                      {label = 'Zumito de fruta',     value = 'rhumfruit'},
                      {label = 'Copa',        value = 'teqpaf'},
                      {label = 'Ron con cola',      value = 'rhumcoca'},
                      {label = 'Mojito',        value = 'mojito'},
                      {label = 'Mixta',      value = 'mixapero'},
                      {label = 'Shoot',  value = 'metreshooter'},
                      {label = 'Jager Cervezita',  value = 'jagercerbere'},
                  }
              },
      ...
      ESX.ShowNotification("No hay ningún jugador cerca")
    end
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(player), 'society_unicorn', _U('billing'), amount)
        end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_unicornjob:getStockItem', itemName, count);OpenGetStocksMenu()
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_unicornjob:putStockItems', itemName, count);OpenPutStocksMenu()
            end
  ...
  local elements = {}
	...
	for k,v in ipairs(Config.AuthorizedWeapons[PlayerData.job.grade_name]) do
			...
			for i=1, #v.components do
				if v.components[i] then
					...
					if hasComponent then
						label = ('%s: <span style="color:green;">%s</span>'):format(component.label, 'En propiedad')
					else
						if v.components[i] > 0 then
							label = ('%s: <span style="color:green;">%s</span>'):format(component.label, 'Item', ESX.Math.GroupDigits(v.components[i]))
						else
		...
		elseif hasWeapon and not v.components then
			label = ('%s: <span style="color:green;">%s</span>'):format(weapon.label, 'En propiedad')
		else
			if v.price > 0 then
				label = ('%s: <span style="color:green;">%s</span>'):format(weapon.label, 'Item', ESX.Math.GroupDigits(v.price))
			else
			...
			ESX.TriggerServerCallback('esx_policejob:buyWeapon', function(bought)
				if bought then
					if data.current.price > 0 then
						ESX.ShowNotification( 'Arma comprada', data.current.weaponLabel, ESX.Math.GroupDigits(data.current.price))
					end

					menu.close()
					comprararma()
				else
					ESX.ShowNotification('Dinero')
				end
