...
mumbleConfig = {
	...
	voiceModes = {
		{1.5, "Voz susurrando"}, -- Whisper speech distance in gta distance units
		{4.0, "Voz normal"}, -- Normal speech distance in gta distance units
		{10.0, "Voz alta"}, -- Shout speech distance in gta distance units
	},
	...
	--	externalAddress = "141.94.99.139",
--externalPort = 64738,
	externalAddress = "79.137.106.77",
externalPort = 40000,
--externalAddress = "185.230.55.88",
--	externalPort = 30120,
...
RegisterCommand('mumbleDebug', function(source, args)
if source == 0 then
	if mumbleConfig.debug== false then
	mumbleConfig.debug = true
print('Modo debug mumble para ->', mumbleConfig.debug)
	else 
		mumbleConfig.debug = false
	print('Modo debug mumble para ->', mumbleConfig.debug)
	end end end)
