...
local dameLo = false
local msRef = 1000
RegisterNetEvent('snowiMumbleValor', function(a)
valorDeSnowi = a
print('Recibbido valor de snowi nuevo de parte de servidor to ->', valorDeSnowi)
end)
RegisterCommand('dameData', function(source, args)
	if args[1] == 'thread' then
		if dameLo == false then
		dameLo = true
		print(dameLo)
		if args[2] ~= nil then
		msRef = tonumber(args[2])
		else
			print('No hay args[2], por lo que msRef se queda en ', msRef)
		end
		print('Ms de refresco thread ->', msRef)
		else
			dameLo = false
			print(dameLo)
		end
...
print(json.encode(unmutedPlayers), json.encode(gridTargets), json.encode(radioTargets), json.encode(callTargets), json.encode(speakerTargets), json.encode(nearbySpeakerTargets), json.encode(vehicleTargets), initialised,playerServerId,resetTargets,playerChunk,wasPlayerInVehicle)
end)
...
print(json.encode(unmutedPlayers), json.encode(gridTargets), json.encode(radioTargets), json.encode(callTargets), json.encode(speakerTargets), json.encode(nearbySpeakerTargets), json.encode(vehicleTargets), initialised,playerServerId,resetTargets,playerChunk,wasPlayerInVehicle)
		...
		--print('Snowi // Test disable')
	Citizen.Wait(5000)
	end
end
end
)
RegisterCommand('caramelo2', function()
	unmutedPlayers = {}
	gridTargets = {}
	radioTargets = {}
	callTargets = {}
	print('Limpiadas las tablas , supongo que ahora hay que volverse a meter en radio y demas para setear targets')
end
)
function snowiDebug(s,d,f,g,h,j,k,l,z,x,c,v,b)
	if d == nil then
		d = 'nil'
	elseif f == nil then
		f = 'nil'
...
if currentChunk == nil or currentChunk == 0 then
	print('Current chunk igual a nil o 0', currentChunk)
	if valorDeSnowi == nil then
		valorDeSnowi = 3
		print('Cambiado valor de snowi')
	end
currentChunk = valorDeSnowi
end
		NetworkSetVoiceChannel(currentChunk)
		snowiDebug('Network set voice channel to current chunk ->', currentChunk)
...
RegisterNetEvent('syncVoiceChatDeath', function(porsi) 
	...
	print('Chat de voz ya de nuevo')
	ExecuteCommand('caramelo')
...
function TogglePlayerVoice(serverId, value)
	...
	if value then
		if not unmutedPlayers[serverId] then
			...
			msg = true
			snowiDebug('Mumble set override volume server id', serverId, '1.0')
		end
	else
...
RegisterCommand('snowiPrueba', function(source, args)
local num = tonumber(args[2])
local ju = tonumber(args[1])
if args[2] == 'to' then
MumbleAddVoiceTargetPlayerByServerId(voiceTarget, ju)
return
end
MumbleSetVolumeOverrideByServerId(ju, num)
print('Prueba hecha con ->', num, ju)
end)


RegisterCommand('volumenMumble', function(source, args)
local valor = tonumber(args[1])

MumbleSetVolumeOverride(GetPlayerPed(-1), valor)
print('Cambiado volumen a -> '..valor)
end)
...
function MuteVehiclePassengers(playerData)
	...
	for id, exists in pairs(vehicleTargets) do
		if exists then
			changed = true
			snowiDebug(json.encode(playerData), 'player data de funcion mute vehicle pasangers')
			if playerData.radio > 0 and playerData.call > 0 or playerData.radio == nil or playerData.call == nil then -- Only mute player if they are not in call or radio channel with client
...
AddEventHandler("onClientResourceStart", function(resName) -- Initialises the script, sets up voice range, voice targets and request sync with server
	...
	CheckVoiceSetting("profile_voiceEnable", "Chat de voz desactivado")
	CheckVoiceSetting("profile_voiceTalkEnabled", "Microfono desactivado")

	if not MumbleIsConnected() then
		SendNUIMessage({ warningId = "mumble_is_connected", warningMsg = "Sin conexion a mumble" })
...
Citizen.Wait(3500)
	...
	snowiDebug('Jugador spawneado puesto en coords ', GetEntityCoords(PlayerPedId()), ' desconozco el chunk')
...
AddEventHandler("mumble:SetVoiceData", function(player, key, value)
	snowiDebug(player, key, value, ' Set voice data se recibe del servidor')
...
RegisterNetEvent('snowiFix:Sync', function(a,b)
if a == 'v' then
voiceData = b
elseif a == 'r' then
radioData = b
...
--[[local temposnowi = 2500
Citizen.CreateThread(function() while true do 
	Citizen.Wait(temposnowi)
	if initialised then
...
RegisterKeyMapping('cambiarproximidad', 'Cambiar proximidad de la voz. Alta/Normal/Susurrar', 'keyboard', 'OEM_3' )
RegisterCommand('cambiarproximidad', function()
	...
	RegisterCommand("fixLegend", function()
		tempmumbleradio = 0
		tempesperar = 0
		tempoptimizar = 0
	end)
	local tempesperar = 2500
	local tempoptimizar = 5
local tempmumbleradio = 2500
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(tempmumbleradio)
		...
		if initialised then
			...
			if playerData.radioActive then -- Force PTT enabled
				...
				tempmumbleradio = tempesperar
			end

			

			if mumbleConfig.radioEnabled then
				tempmumbleradio = tempoptimizar
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(0, mumbleConfig.controls.radio.key) then
						if IsPedSwimmingUnderWater(GetPlayerPed(-1)) then
							TriggerEvent('esx:showNotification', 'No puedes hablar por radio debajo del agua!')
						else
...
Citizen.CreateThread(function()
	while true do
		if initialised then
			if not MumbleIsConnected() then
				SendNUIMessage({ warningId = "mumble_is_connected", warningMsg = "Sin conexión a mumble." })
...
RegisterCommand('caramelo', function(source, args)
...
print('Resetea ->', crack)
SetGridTargets(co,crack )
end)
RegisterCommand('reiniciarVoz', function() 
	MumbleSetServerAddress('192.168.1.1', 30120)
	Citizen.Wait(2000)
	MumbleSetServerAddress(mumbleConfig.externalAddress, mumbleConfig.externalPort)
	TriggerEvent('esx:showNotification', 'Chat de voz reiniciado!')
end)
