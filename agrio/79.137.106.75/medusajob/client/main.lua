...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'medusa' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'medusa', vehicleProps)
...
snowi = ESX.GetPlayerData()
 ...
 -- end
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'medusa_actions',
    {
      title    = _U('medusa'),
      align    = 'top-left',
      elements = elements
      ...
      if data.current.value == 'esposar' then
        ExecuteCommand('me Le pone las esposas')
						...
						if closestDistance <= 2.0 then
							TriggerServerEvent('medusa:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
    ...
    elseif action == 'desesposar' then
      ...
      local target_id = GetPlayerServerId(closestPlayer)
      if closestDistance <= 2.0 then
        TriggerServerEvent('medusa:requestrelease', target_id, playerheading, playerCoords, playerlocation)
      else
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              ...
              function(data2, menu2)
                ...
                TriggerServerEvent('esx_medusajob:craftingCoktails', data2.current.value)
...
function cachearmedusa(player)
	ESX.TriggerServerCallback('medusa:getOtherPlayerData', function(data)
		local elements = {}
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(player), 'society_medusa', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_medusajob:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_medusajob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_medusajob:putFridgeStockItems', itemName, count)
            end
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'medusa_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_medusajob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_medusajob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_medusajob:hasExitedMarker', function(zone)
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_medusajob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                otrotonto = 2500
                TriggerEvent('esx_medusajob:hasExitedMarker', LastZone)
            end
          else
            otrotonto = 10000
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'medusa', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'medusa')
            end
        ...
        if CurrentAction == 'menu_boss_actions' then-- and IsGradeBoss() then
          ...
          TriggerEvent('esx_society:openBossMenu', 'medusa', function(data, menu)
...
-----------------------
----- TELEPORTERS -----
RegisterKeyMapping('menumedusa', 'Menu de medusa(Only for job)', 'keyboard', 'F6' )
RegisterCommand('menumedusa', function() if IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'medusa_actions') then
  OpenSocietyActionsMenu()
end
end)

AddEventHandler('esx_medusajob:teleportMarkers', function(position)
      ...
      sdaasd=1000
       --[[ for k,v in pairs(Config.TeleportZones) do
        ...
        if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          TriggerEvent('esx_medusajob:teleportMarkers', position)
        end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('medusa:unrestrain')
...
RegisterNetEvent('medusa:handcuff132')
AddEventHandler('medusa:handcuff132', function()
...
RegisterNetEvent('medusa:unrestrain')
AddEventHandler('medusa:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
