...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('armasSurGetSandy', function(shopItems)
...
while ja do
	...
	if IsControlJustReleased(0, 32) then
		...
		if tra.job.name == "armeriaSandy" then
			print('si armeria')
			love = true
		else
			print('No menor')
...
RegisterCommand('reloadArmeriaSandy', function()
	tra = ESX.GetPlayerData()
	PlayerData = ESX.GetPlayerData()
		if tra.job.name == "armeriaSandy" then
			love = true
...
RegisterNetEvent('armasSandySendShopSandy')
AddEventHandler('armasSandySendShopSandy', function(shopItems)
	for k,v in pairs(shopItems) do
		Config.Zones[k].Items = v
...
print(json.encode(Config.Zones))
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		print('Jola?')
		ESX.TriggerServerCallback('armasSandybuyWeapon', function(bought)
			print('Jola2?')
			if bought then
				DisplayBoughtScaleform(data.current.weaponName, data.current.price)
				OpenShopMenu('GunShop')
				SetCurrentPedWeapon(GetPlayerPed(-1),GetHashKey(data.current.weaponName),true)
			else
...
AddEventHandler('entraEnPointSandy', function(zone)
	if love then
...
AddEventHandler('salePointSandy', function(zone)
	if love then 
	CurrentAction = nil
	...
	for k,v in pairs(Config.Zones) do
		if v.Legal then
			for i = 1, #v.Locations, 1 do
				...
				AddTextComponentSubstringPlayerName('Armeria Sandy')
...
if love then 
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('entraEnPointSandy', currentZone)
		end
		
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('salePointSandy', LastZone)
		end
		if o == true then
			Citizen.Wait(3000)
		end
...
duerme3 = false
            ...
            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent('entraEnPointSandy', LastZone)
            end
...
local GUI = {}
GUI.Time = 0
  RegisterCommand('menuarmeriaSandy', function()
  if PlayerData.job ~= nil and PlayerData.job.name == 'armeriaSandy' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'SnowiMafiasAcciones') and (GetGameTimer() - GUI.Time) > 150 then
	TriggerEvent('snowiEventToMenu', PlayerData.job.name)
	   GUI.Time = GetGameTimer()
  end 
  end)
  RegisterKeyMapping('menuarmeriaSandy', 'Menu armeria sandy', 'keyboard', 'F6' )
