...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'badulake' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
...
function IsGradeBoss()
    if PlayerData ~= nil then
        ...
        if PlayerData.job.grade_name == 'jefe' or PlayerData.job.grade_name == 'encargado' then
            IsGradeBoss = true
        end
        return IsGradeBoss
    end
...
function OpenCloakroomMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'empleado' then
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value == 'get_weapon' then
          if ESX.GetPlayerData().job.grade_name ~= 'enpruebas' then
          OpenGetWeaponMenu()
          end
        end
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'badulake', vehicleProps)
...
snowi = ESX.GetPlayerData()
 ...
 -- end
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'badulake_actions',
    {
      title    = _U('badulake'),
      align    = 'bottom-right',
      elements = elements
      ...
      if data.current.value == 'esposar' then
        ExecuteCommand('me Le pone las esposas')
						...
						if closestDistance <= 2.0 then
							TriggerServerEvent('badulake:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
    ...
    elseif action == 'desesposar' then
      ...
      local target_id = GetPlayerServerId(closestPlayer)
      if closestDistance <= 2.0 then
        TriggerServerEvent('badulake:requestrelease', target_id, playerheading, playerCoords, playerlocation)
      else
...
function cachearbadulake(player)
	ESX.TriggerServerCallback('badulake:getOtherPlayerData', function(data)
		local elements = {}
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(player), 'society_badulake', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_badulakejob:getFridgeStockItems', function(items)
    ...
    ESX.UI.Menu.Open(
      ...
      {
        title    = _U('badulake_fridge_stock'),
        elements = elements
      },
      function(data, menu)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_badulakejob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_badulakejob:putFridgeStockItems', itemName, count)
            end
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'badulake_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_badulakejob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_badulakejob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_badulakejob:hasExitedMarker', function(zone)
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_badulakejob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                otrotonto = 2500
                TriggerEvent('esx_badulakejob:hasExitedMarker', LastZone)
            end
          else
            otrotonto = 10000
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'menu_vehicle_spawner' then
          if ESX.GetPlayerData().job.grade_name == 'jefe' or ESX.GetPlayerData().job.grade_name == 'encargado' then
            print(ESX.GetPlayerData().job.grade_name)
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'badulake', vehicleProps)
...
-----------------------
----- TELEPORTERS -----
RegisterKeyMapping('menubadulake', 'Menu de badulake(Only for job)', 'keyboard', 'F6' )
RegisterCommand('menubadulake', function() if IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'badulake_actions') then
  OpenSocietyActionsMenu()
end
end)

AddEventHandler('esx_badulakejob:teleportMarkers', function(position)
      ...
      sdaasd=1000
       --[[ for k,v in pairs(Config.TeleportZones) do
        ...
        if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          TriggerEvent('esx_badulakejob:teleportMarkers', position)
        end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('badulake:unrestrain')
...
RegisterNetEvent('badulake:handcuff132')
AddEventHandler('badulake:handcuff132', function()
...
RegisterNetEvent('badulake:unrestrain')
AddEventHandler('badulake:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
