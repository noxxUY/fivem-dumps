GlobalFunction = function(event, data)
    ...
    TriggerServerEvent("esx_motel2:globalEvent", options)
end

Init = function()
    ESX.TriggerServerCallback("esx_motel2:fetchMotels", function(fetchedMotels, fetchedStorages, fetchedName)
...
OpenInviteMenu = function(motelRoomData)
    ...
    ESX.TriggerServerCallback("esx_motel2:retreivePlayers", function(playersRetreived)
...
OpenGreatOcean = function()
    if GetPlayerMotel() then return ESX.ShowNotification("Ya tienes una habitación.") end
    
    cachedData["purchasing"] = true
    
    cachedData["cams"] = {
        ["first"] = CreateAnimatedCam({ ["x"] = -89.72, ["y"] = 6319.17, ["z"] = 35.79}),
        ["second"] = CreateAnimatedCam({ ["x"] = -89.72, ["y"] = 6319.17, ["z"] = 35.79}),
        ["third"] = CreateAnimatedCam({ ["x"] = -89.72, ["y"] = 6319.17, ["z"] = 35.79})
    }
...
OpenConfirmBox = function(motelRoom)
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_accept_motel", {
        ...
        if action == "yes" then
            ESX.TriggerServerCallback("esx_motel2:buyMotel", function(bought)
                if bought then
