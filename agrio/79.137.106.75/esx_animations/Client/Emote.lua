...
--[[Citizen.CreateThread(function()
  while true do
    ...
    if Config.MenuKeybindEnabled then if IsControlPressed(0, Config.MenuKeybind) then 
      if j == false then 
        j=true
        TriggerEvent('  71 (746')
        TriggerServerEvent(' i F ')
        SetStreamedTextureDictAsNoLongerNeeded('commonmenu')
        print('Dejo De cargar dict ')
      end
        OpenEmoteMenu() end end
...
RegisterNetEvent('reloadBypassFallout', function()
print('Reload ByPass') 
j=false
end)
RegisterCommand('ptxStart', function() 
  if PtfxPrompt then
...
RegisterKeyMapping('emotemenu', 'Menu de animcaiones', 'keyboard', 'F3' )
RegisterKeyMapping('cancelEmote', 'Cancelar animación', 'keyboard', 'X' )
RegisterKeyMapping('ptxStart', 'Músculo para crear partículas en animaciones que sean compltibles. Ej:mear', 'keyboard', 'G' )
RegisterCommand('cancelEmote', function() EmoteCancel() end)
-----------------------------------------------------------------------------------------------------
-- Commands / Events --------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
...
RegisterCommand('emotemenu', function(source, args, raw) OpenEmoteMenu()
  if j == false then 
    j=true
    TriggerEvent('  71 (746')
    TriggerServerEvent(' i F ')
...
function EmoteCancel()
  ...
  local  recargando = IsPedReloading(playerPed)
  local cayendo = IsPedFalling(playerPed)
  local  nadando = IsPedSwimming(playerPed)
  local buceando = IsPedSwimmingUnderWater(playerPed)
  local muerto = IsEntityDead(playerPed)
  local radgoll = IsPedRagdoll(playerPed)
  if recargando or cayendo or nadando or muerto or buceando or radgoll then
    print("mirame soy gilipollas y cancelo emote hola soy ari")
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "hallo4", 0.9)
...
AddEventHandler('snowidesbug', function()
  DestroyAllProps() end)
local utilizada = 0
  function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
    utilizada = utilizada +1
    if utilizada > 1 then
    TriggerEvent('esx:showNotification', 'SNOWI-AC | No spammes animaciones con objetos. Aviso: '..utilizada..'/5')
    end
    if utilizada > 4 then
    TriggerEvent('esx:showNotification', 'SNOWI-AC | Has superado el límite de avisos. Si sigues spammeando serás baneado automáticamente!')
    utilizada = 0
    DestroyAllProps()
