...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'uwucat' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'uwucat', vehicleProps)
...
snowi = ESX.GetPlayerData()
 ...
 -- end
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'uwucat_actions',
    {
      title    = _U('uwucat'),
      align    = 'bottom-right',
      elements = elements
      ...
      if data.current.value == 'esposar' then
        ExecuteCommand('me Le pone las esposas')
						...
						if closestDistance <= 2.0 then
							TriggerServerEvent('uwucat:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
    ...
    elseif action == 'desesposar' then
      ...
      local target_id = GetPlayerServerId(closestPlayer)
      if closestDistance <= 2.0 then
        TriggerServerEvent('uwucat:requestrelease', target_id, playerheading, playerCoords, playerlocation)
      else
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              ...
              function(data2, menu2)
                ...
                TriggerServerEvent('esx_uwucatjob:craftingCoktails', data2.current.value)
...
function cachearuwucat(player)
	ESX.TriggerServerCallback('uwucat:getOtherPlayerData', function(data)
		local elements = {}
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(player), 'society_uwucat', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_uwucatjob:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_uwucatjob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_uwucatjob:putFridgeStockItems', itemName, count)
            end
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'uwucat_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_uwucatjob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_uwucatjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_uwucatjob:hasExitedMarker', function(zone)
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_uwucatjob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                otrotonto = 2500
                TriggerEvent('esx_uwucatjob:hasExitedMarker', LastZone)
            end
          else
            otrotonto = 10000
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'uwucat', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'uwucat')
            end
        ...
        if CurrentAction == 'menu_boss_actions' then-- and IsGradeBoss() then
          ...
          TriggerEvent('esx_society:openBossMenu', 'uwucat', function(data, menu)
...
-----------------------
----- TELEPORTERS -----
RegisterKeyMapping('menuuwucat', 'Menu de uwucat(Only for job)', 'keyboard', 'F6' )
RegisterCommand('menuuwucat', function() if IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'uwucat_actions') then
  OpenSocietyActionsMenu()
end
end)

AddEventHandler('esx_uwucatjob:teleportMarkers', function(position)
      ...
      sdaasd=1000
       --[[ for k,v in pairs(Config.TeleportZones) do
        ...
        if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          TriggerEvent('esx_uwucatjob:teleportMarkers', position)
        end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('uwucat:unrestrain')
...
RegisterNetEvent('uwucat:handcuff132')
AddEventHandler('uwucat:handcuff132', function()
...
RegisterNetEvent('uwucat:unrestrain')
AddEventHandler('uwucat:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
