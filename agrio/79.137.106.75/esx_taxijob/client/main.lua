...
function OpenTaxiActionsMenu()
	local elements = {
		...
		{label = 'Coger taser', value = 'cogerTaser'},
		{label = 'Dejar taser', value = 'dejarTaser'},
		{label = 'Obtener acceso a frecuencia Taxi [55]', value = 'radiofrec'},
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'taxi_actions',
	{
		...
		elseif data.current.value == 'radiofrec' then
			exports["rp-radioja"]:GivePlayerAccessToFrequencies(55)
			ESX.ShowNotification('Concedido el acceso de radio de TAXI [55]')
		elseif data.current.value == 'cogerTaser' then
			TriggerServerEvent('snowiAmbulanceGetWeapon', 'WEAPON_STUNGUN')
		elseif data.current.value == 'dejarTaser' then
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(closestPlayer), 'society_taxi', 'Taxi', amount)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              menu2.close()
              menu.close()
              TriggerServerEvent('esx_taxijob:getStockItem', itemName, count);OpenGetStocksMenu()
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              menu2.close()
              menu.close()
              TriggerServerEvent('esx_taxijob:putStockItems', itemName, count);OpenPutStocksMenu()
...
RegisterKeyMapping('menutaxi', 'Menu de taxistas', 'keyboard', 'F6' )
RegisterCommand('menutaxi', function() 
if not IsDead and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'taxi' then
