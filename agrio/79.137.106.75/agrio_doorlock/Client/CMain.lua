...
_CreateThread(function()
    _Wait(500)
    ESX['TriggerServerCallback']('corleon_doorlock:cb:getDoors', function(doors, state)
        _enabledDoors = doors
        _doorState = state
    end)
end)

RegisterNetEvent("corleon_doorlock:client:setUpDoor", function()
    local elements = {}
...
RegisterNetEvent("corleon_doorlock:client:deleteDoor", function()
    _CreateThread(function()
        while true do
            local _wait = 0
            ...
            if IsControlJustPressed(1, 38) then
                for k, v in pairs(_enabledDoors) do
                    if v['_type'] ~= "double" then
                        ...
                        if _distTo < 1 then
                            TriggerServerEvent("corleon_doorlock:server:syncRemove", k)
                            _found = true
                        end
                    else
                        local _doorCoords = vector3(v['_textCoords']['x'], v['_textCoords']['y'], v['_textCoords']['z'])
...
RegisterNetEvent("corleon_doorlock:client:removeGlobDoor", function(id)
            ...
            while true do
                ...
                if IsEntityAnObject(entity) then
                    ...
                    if IsControlJustPressed(1, 38) then
                        ...
                        TriggerServerEvent("corleon_doorlock:server:addDoor", _doorCoords, _doorModel, _heading, type, _textCoords, dist, jobs, pin, item)
                ...
                if IsEntityAnObject(entity) then
                    ...
                    if IsControlJustPressed(1, 38) then
                        ...
                        if #_doorsDobule == 2 then
                            ...
                            entities = {}
                            TriggerServerEvent("corleon_doorlock:server:addDoubleDoor", _doorsDobule, type, _textCoords, dist, jobs, pin, item)
                            _doorsDobule = {}
                            break
                        else
...
RegisterNetEvent("corleon_doorlock:client:refreshDoors", function(tableToIns)
            ...
            if v['_type'] == "normal" then
                ...
                if _distTo < v['dist'] then
                    ...
                    if _doorState[k] ~= nil then
                        ...
                        FreezeEntityPosition(door, false)
                        if pulsed then
                            TriggerServerEvent("corleon_doorlock:server:updateDoor", k, nil)
                            animatePlyDoor()
                            pulsed = false
                        end
                    else
                        ...
                        if pulsed then
                            TriggerServerEvent("corleon_doorlock:server:updateDoor", k, "locked")
                            animatePlyDoor()
                            pulsed = false
                        end
                        if v['_type'] == "normal" then
...
RegisterNetEvent("corleon_doorlock:client:updateDoorState", function(id, type, h)
    _doorState[id] = type
end)
...
RegisterCommand("lockdoor", function()
    if show then
        ...
        if not allowed then
            ...
            if object then
                ESX.TriggerServerCallback('corleon_doorlock:cb:hasObj', function(has)
                    if has then
                        pulsed = true
                        object = nil
                    end
