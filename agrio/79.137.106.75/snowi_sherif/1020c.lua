...
--[[RegisterNetEvent('sheriff:blip1020')
AddEventHandler('sheriff:blip1020', function(x, y, z)
	loadESX()
...
RegisterCommand('1020', function(source, args)
    ...
    if PlayerData.job and PlayerData.job.name == 'sheriff' then
		...
		TriggerServerEvent('1020:sheriff', location, msg, x, y, z, tipo)
		local msg = '10-20'
	local job = PlayerData.job.name
    TriggerServerEvent('esx_jobChat:chat', job, msg)
    else
		ESX.ShowNotification('Esta función solo está permitida para unidades sheriffles!')
	end
end)--]]