...
Radio.Labels = {        
    { "FRZL_RADIO_HELP", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " para ocultar la radio.~n~~" .. radioConfig.Controls.Toggle.Name .. "~ para ~g~encender~s~ la radio.~n~~" .. radioConfig.Controls.Decrease.Name .. "~ o ~" .. radioConfig.Controls.Increase.Name .. "~ para cambiar de frecuencia.~n~~" .. radioConfig.Controls.Input.Name .. "~ para elegir la frecuencia.~n~~" .. radioConfig.Controls.ToggleClicks.Name .. "~ para ~a~ los sonidos de la radio.~n~Frecuencia: ~1~ MHz" },
    { "FRZL_RADIO_HELP2", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " para ocultar la radio.~n~~" .. radioConfig.Controls.Toggle.Name .. "~ para ~r~apagar~s~ la radio.~n~~" .. radioConfig.Controls.Broadcast.Name .. "~ para emitir.~n~Frecuencia: ~1~ MHz" },
    { "FRZL_RADIO_INPUT", "Introduce la frecuencia" },
}
...
Citizen.CreateThread(function()
	...
	for i=1, #xPlayer.inventory, 1 do
	  	if xPlayer.inventory[i].name == 'radio' and  xPlayer.inventory[i].count > 0 then
			Radio.Has = true
		  break
	  	end
	end
...
local latiene = false
function tieneRadio()
	ESX.TriggerServerCallback('snowiHasRadio', function(tiene)
		if tiene then
			latiene = true
		else
			TriggerEvent('esx:showNotification', 'Debes de tener un walki encima!')
			latiene = false
		end
end)
end
...
function hasRadio (cb)
	ESX.TriggerServerCallback('snowiHasRadio', function(qtty)
	  cb(qtty)
	end)
  end
-- Create/Destroy handheld radio object
local canAbrir = false
...
function Radio:Toggle(toggle)
    ...
    if not self.Has or IsEntityDead(playerPed) or not canAbrir then
        self.Open = false
    ...
    while true do
        ...
        if Radio.Open then
            ...
            if not Radio.On then
                AddTextComponentSubstringPlayerName(Radio.Clicks and "~r~deshabilitar~w~" or "~g~habilitar~w~")
            end
        ...
        if Radio.Open == false and Radio.On == false then
tempradio = 2500
        end
    end
end)
