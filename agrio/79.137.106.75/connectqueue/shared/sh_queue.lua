...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", 256)
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] Tiene prioridad en la cola y esta de  %d/%d ", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] Esta en el puesto %d/%d de la cola", tmp.name, ids[1], _pos, queueCount))
    end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            ...
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] Fue eliminado de la cola por que no tenía datos válidos"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            ...
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] Fué eliminado de la cola por que tuvo tiempo de espera agotado")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] Fué eliminado de la cola por que tuvo tiempo de espera agotado")
            else
                i = i + 1
            end
        end
...
commands.setpos = function(args)
    ...
    Queue:DebugPrint("Poner " .. data.name .. " en posición : " .. args[2])
end
