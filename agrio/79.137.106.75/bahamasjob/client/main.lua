    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'bahamas', vehicleProps)
...
snowi = ESX.GetPlayerData()
  ...
  if snowi.job.grade_name ~= 'cama' or snowi.job.grade_name ~= 'dsncer' or snowi.job.grade_name ~= 'barman'  then
    table.insert(elements, {label = 'Cachear',    value = 'cachear'})
    table.insert(elements, {label = 'Esposar',    value = 'esposar'})
    table.insert(elements, {label = 'Desesposar',    value = 'desesposar'})
  end
 ...
 -- end
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'bahamas_actions',
    {
      title    = _U('bahamas'),
      align    = 'top-left',
      elements = elements
      ...
      if data.current.value == 'esposar' then
        ExecuteCommand('me Le pone las esposas')
						...
						if closestDistance <= 2.0 then
							TriggerServerEvent('bahamas:requestarrest', target_id, playerheading, playerCoords, playerlocation)
						else
    ...
    elseif action == 'desesposar' then
      ...
      local target_id = GetPlayerServerId(closestPlayer)
      if closestDistance <= 2.0 then
        TriggerServerEvent('bahamas:requestrelease', target_id, playerheading, playerCoords, playerlocation)
      else
      ...
      if data.current.value == 'menu_crafting' then
          ...
          ESX.UI.Menu.Open(
              ...
              function(data2, menu2)
                ...
                TriggerServerEvent('esx_bahamasjob:craftingCoktails', data2.current.value)
...
function cachearbahamas(player)
	ESX.TriggerServerCallback('bahamas:getOtherPlayerData', function(data)
		local elements = {}
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:senhfgdBill132', GetPlayerServerId(player), 'society_bahamas', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_bahamasjob:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_bahamasjob:getFridgeStockItem', itemName, count)
            end
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_bahamasjob:putFridgeStockItems', itemName, count)
            end
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'bahamas_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_bahamasjob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_bahamasjob:hasExitedMarker', function(zone)
...
duerme2 = false
        ...
        if duerme2 == true then
          Citizen.Wait(5000)
        elseif duerme == true then
          Citizen.Wait(2500)
        end
...
duerme3 = false
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_bahamasjob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent('esx_bahamasjob:hasExitedMarker', LastZone)
            end
...
local duerme5 = true
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'bahamas', vehicleProps)
    ...
    if IsControlJustReleased(0,  Keys['F6']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'bahamas_actions') then
...
AddEventHandler('esx_bahamasjob:teleportMarkers', function(position)
...
if duerme6 == true then
  Citizen.Wait(5000)
elseif duerme7 == true then
  Citizen.Wait(2500)
end
  end
end)
...
duerme8 = false
        ...
        if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          TriggerEvent('esx_bahamasjob:teleportMarkers', position)
        end
    ...
    if duerme8 == true then
      Citizen.Wait(5000)
    elseif duerme9 == true then
      Citizen.Wait(2500)
    end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('bahamas:unrestrain')
...
RegisterNetEvent('bahamas:handcuff132')
AddEventHandler('bahamas:handcuff132', function()
...
RegisterNetEvent('bahamas:unrestrain')
AddEventHandler('bahamas:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
