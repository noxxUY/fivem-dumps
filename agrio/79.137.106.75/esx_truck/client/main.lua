...
function openmenuvehicle()
	...
	if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
		local elements = {
			{label = 'Abrir maletero', value = 'abrir'},
			{label = 'Cerrar maletero', value = 'cerrar'}
		}
		ESX.UI.Menu.Open(
	  'default', GetCurrentResourceName(), 'check_truck',
	  {
	    title    = 'Maletero OPCIONES',
	    align    = 'bottom-right',
	    elements = elements,
	  },
	  function(data, menu)
		...
		ESX.TriggerServerCallback('esx_truck:checkvehicle',function(valid)
			...
			if (not valid) then
				...
				local closecar = GetClosestVehicle(x, y, z, 7.0, 0, 71)
			  if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= GetPlayerPed(-1) then
					...
					if ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'inventory') then
					  ...
					  if locked == 1 or class == 15 or class == 16 or class == 14 then
						  ...
						  if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
							CloseToVehicle = true
							ExecuteCommand('me Abre el maletero del vehículo('..GetVehicleNumberPlateText(vehFront)..')')
							TriggerServerEvent('esx_truck_inventory:AddVehicleList', GetVehicleNumberPlateText(vehFront))
						  TriggerServerEvent("esx_truck_inventory:getInventory", GetVehicleNumberPlateText(vehFront))
						  end
						else
						   ESX.ShowNotification('Este maletero está ~r~cerrado')
					  end
					end
				else
					ESX.ShowNotification('Debes acercarte más al ~r~maletero~w~ ')
				end
				lastOpen = true
				GUI.Time  = GetGameTimer()
			else
				ExecuteCommand('me Me intenta abrir el maletero')
				ExecuteCommand('do No podria')
				TriggerEvent('esx:showNotification', "Alguien ya está mirando el maletero o no eres el propietario del vehiculo!")
			end
		end, globalplate)
	else
		--ExecuteCommand('me Cierra el maletero del vehículo('..GetVehicleNumberPlateText(vehFront)..')')
		TriggerServerEvent('esx_truck_inventory:RemoveVehicleList', GetVehicleNumberPlateText(vehFront))
		ESX.UI.Menu.CloseAll()
...
RegisterNetEvent('snowiMaleteroSync', function(s) 
	local vehFront = VehicleInFront()
if s == 'secierra' then
	ExecuteCommand('me Cierra el maletero del vehículo('..GetVehicleNumberPlateText(vehFront)..')')
	...
	ExecuteCommand('do Intentaría cerrar el maletero del vehículo('..GetVehicleNumberPlateText(vehFront)..'). No podría')
		ESX.UI.Menu.CloseAll()
end
end)
-- Key controls
--[[Citizen.CreateThread(function()
  while true do
	...
	if IsControlPressed(0, Keys["L"]) and (GetGameTimer() - GUI.Time) > 1000 then
		if count == 0 then
			openmenuvehicle()
			--ExecuteCommand('me Abre el maletero del vehiculo')
			count = count +1
		else
			Wait(2000)
			--print('Cierra maletero')
			count = 0
		end
...
RegisterCommand('abrirveh', function()
	print(GetGameTimer().." / "..GUI.Time)
  ...
  while true do
	...
	if CloseToVehicle then
		...
		if DoesEntityExist(vehFront) then
			...
			lastOpen = false
			ESX.UI.Menu.CloseAll()
			print('Close cierra 1')
			SetVehicleDoorShut(lastVehicle, 5, false)
		end
	else
		locot = 2500
		if lastVehicle ~= nil and globalplate ~= nil then
		TriggerServerEvent('esx_truck_inventory:RemoveVehicleList', globalplate)
			CloseToVehicle = false
			lastOpen = false
			ESX.UI.Menu.CloseAll()
			print('Close cierra 2'..tostring(globalplate))
			SetVehicleDoorShut(lastVehicle, 5, false)
			globalplate = nil 
			lastVehicle=nil
		end
	...
	if inventory ~= nil and #inventory > 0 then
		...
		if inventory[i].type == 'item_standard' then
			...
			elseif inventory[i].type == 'item_weapon' then
			  table.insert(elements, {
				label     = inventory[i].label .. ' | munición: ' .. inventory[i].count,
				count     = inventory[i].count,
				value     = inventory[i].name,
				type	  = inventory[i].type
			  })	
		...
		for i=1, #weaponList, 1 do
		  ...
		  if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
            ...
            if quantity > 0 and quantity <= tonumber(data3.current.count) and vehFront > 0  then
			  ...
			  ESX.ShowNotification('Espera 1,2 segundos mientras coges el producto del maletero')
		...
		elseif data.current.type == 'cancel' then
          ...
          --fin test
            ...
            if not max then
              ...
              ESX.ShowNotification('~r~ No tienes suficiente espacio en tu inventario, prueba a sacar menos cantidad')
            end
			    else
			      ESX.ShowNotification('~r~ Cantidad inválida')
			    end
