...
var open_sound = document.getElementById("abrir-inventario");
open_sound.volume = 0.4;

var boton_sound = document.getElementById("botones");
boton_sound.volume = 0.4;

var cerrar_sound = document.getElementById("cerrarboton");
cerrar_sound.volume = 0.3;
    ...
    if (event.data.action == "display") {
        ...
        if (type === "normal") {
            ...
            $(".menu-lateral .boton").off("click").on("click", function(){
                ...
                $.post('https://agrio_inventory/inventory_options', JSON.stringify({event: event, action: action}));
            });
            $('#cerrarropa').on('click', () => {
                boton_sound.play()
                $('.menu-lateral').hide();
                $('#botonropa').fadeIn(800);
                $('#cerrarropa').hide();
            })
        ...
        $(".nearbyPlayerButton").click(function () {
            ...
            $.post("https://agrio_inventory/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("https://agrio_inventory/NUIFocusOff", JSON.stringify({}));
}
	...
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast") && (itemData.type === 'item_weapon' || itemData.usable === true)) {
                ...
                $.post("https://agrio_inventory/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1
                }));
            }
...
function formatMoney(money) {
    ...
    var stringMoney = formatter.format(money);
    var result = stringMoney.split(".");
    return result[0];
};
...
$(document).ready(function () {
    ...
    $('#use').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.usable) {
                ...
                $.post("https://agrio_inventory/UseItem", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("https://agrio_inventory/GetNearPlayers", JSON.stringify({
                    item: itemData
                }));
            }
        }
  ...
  /*  $("#zone").droppable({
        ...
        drop: function(event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300)
                $.post("https://agrio_inventory/DropItem", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
                $(".zona").css({
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
					owner : ownerHouse
                }));
                ...
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }else if (type === "normal" && itemInventory === "fast") {
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromFast", JSON.stringify({
                    item: itemData
                }));
            } else if (type === "glovebox" && itemInventory === "second") {
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                })); 
            } else if (type === "shop" && itemInventory === "second") {
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "disc-property" && itemInventory === "second") {
                disableInventory(500);
                $.post("https://agrio_inventory/TakeFromDiscProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("https://agrio_inventory/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("https://agrio_inventory/PutIntoProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val()),
					owner : ownerHouse
                }));
                ...
                disableInventory(500);
                $.post("https://agrio_inventory/PutIntoGlovebox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "disc-property" && itemInventory === "main") {
                disableInventory(500);
                $.post("https://agrio_inventory/PutIntoDiscProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("https://agrio_inventory/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
...
function buttonsMenuEvents(){
    ...
    $(".menu-lateral .boton").off("click").on("click", function(){
        let action = $(this).attr("action");
        let event = $(this).attr("event");
        $.post('httpss://agrio_inventory/inventory_options', JSON.stringify({event: event, action: action}));
    });
}
